...
# Directory names that are auto-generated from data_types.json itself and
# therefore mention every struct name by design — scanning them for
# references would produce a tautological 100% "referenced" result.
_AUTOGENERATED_REFERENCE_DIRS = {'checks'}
...
def _collect_file_text(directories, extensions):
    ...
    for scan_dir in directories:
        ...
        for root, dirs, files in os.walk(scan_dir):
            dirs[:] = [d for d in dirs if d not in _AUTOGENERATED_REFERENCE_DIRS]
            for fname in files:
