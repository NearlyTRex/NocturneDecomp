...
def generate_watcom_runtime_inlines():
    ...
    lines.append("inline void* __arr_op(void* dest, void* src, int count, int size, void (*func)(void*, void*)) {")
    lines.append("    char* d = (char*)dest;")
    lines.append("    char* s = (char*)src;")
    ...
    lines.append("inline int __arrfini(void** arr, int count, WatcomTypeInfo* ti) {")
    lines.append("    WatcomTypeArrayInfo info;")
    lines.append("    info.obj_array = arr;")
    lines.append("    info.obj_count = count;")
    lines.append("    info.type_info = ti;")
    lines.append("    __arrdtor(&info);")
    lines.append("    return (int)(size_t)*arr;")
    lines.append("}")
    lines.append("")
    ...
    lines.append("    __arrfini((void**)&ptr, *base, ti);")
    ...
    lines.append("")
    lines.append("#ifndef _MSC_VER")
    lines.append("inline long _findfirst(const char* filespec, struct _find_t* fileinfo) {")
    lines.append("    (void)filespec; (void)fileinfo; return -1;")
    lines.append("}")
    lines.append("inline int _findnext(long handle, struct _find_t* fileinfo) {")
    ...
    lines.append("#endif")
    lines.append("")
    lines.append("// _heapinfo structure for _heapwalk")
    lines.append("#ifndef _HEAPINFO_DEFINED")
    lines.append("#define _HEAPINFO_DEFINED")
    lines.append("typedef struct _heapinfo {")
    lines.append("    void*   _pentry;   // heap pointer")
    lines.append("    size_t  _size;     // heap entry size")
    lines.append("    int     _useflag;  // heap entry 'in-use' flag")
    lines.append("} _HEAPINFO;")
    lines.append("#endif")
    lines.append("")
