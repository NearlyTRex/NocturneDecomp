...
def generate_basetypes_header(pseudocode_dir):
    ...
    content.append("#include <time.h>  // for time_t (long on glibc, long long on any Windows CRT)")
    content.append("")
    content.append("// The Watcom convention macros below blank `__edx`, which is also the name of")
    content.append("// a local in clang's <cpuid.h>. Anything pulling <intrin.h> after this point")
    content.append("// (SDL_cpuinfo.h does) would then fail to parse it, so parse it here first and")
    content.append("// let its include guard absorb the later inclusion. On Windows the entry point")
    content.append("// must be <intrin.h>, not <cpuid.h>: including <cpuid.h> alone defines a 5-arg")
    content.append("// `__cpuid` macro that rewrites mingw's own `void __cpuid(int[4], int)`.")
    content.append("#if defined(_WIN32)")
    content.append("#include <intrin.h>")
    content.append("#endif")
    content.append("")
    content.append("// =============================================================================")
    content.append("// Integer width model: Win32 LLP64 (NOT the host's data model)")
    content.append("// =============================================================================")
    content.append("// The binary was compiled for 32-bit Watcom where `long` == 4 bytes. On a")
    content.append("// 64-bit Linux host `long` == 8 (LP64), which would silently grow every")
    content.append("// struct that has a `long`/`ulong`/`dword`/`DWORD`/`ULONG` field and break")
    content.append("// every baked 32-bit layout, memcpy size, and raw fread/fwrite record.")
    content.append("// So the fixed-width integer typedefs below are pinned to 32 bits (matching")
    content.append("// real Win32 semantics, where DWORD/LONG are 32-bit even on Win64). Only the")
    content.append("// *_PTR / SIZE_T family is genuinely pointer-width and uses intptr_t/size_t.")
    content.append("// A literal `long`/`unsigned long` field cannot be pinned here (it's a builtin")
    content.append("// keyword) — retype those few fields to a fixed-width type in Ghidra instead.")
    content.append("")
    content.append("// =============================================================================")
    ...
    content.append("typedef unsigned long ulong;     // MUST stay `unsigned long`: glibc <sys/types.h> defines")
    content.append("                                 // this BSD alias identically; a mismatch is a redefinition")
    content.append("                                 // error. 8 bytes @64-bit — retype `ulong` struct FIELDS to")
    content.append("                                 // `uint` in Ghidra instead (same as literal `long`).")
    ...
    content.append("typedef unsigned int dword;      // 32-bit (Win32 DWORD width)")
    ...
    content.append("")
    content.append("// Ghidra string types. Typed `const char *` on purpose: Ghidra auto-detects")
    content.append("// some statically-initialized buffers as C string literals and emits them as")
    content.append("// TerminatedCString (e.g. g_RendererDllName = \"trid3d.dll\"). If the game")
    content.append("// actually writes to that buffer at runtime — which it does, because the")
    content.append("// original binary declared it as a fixed-size `char[N]` — we want the WRITE")
    content.append("// call site to fail to compile (via -Wincompatible-pointer-types / -Werror),")
    content.append("// not silently SEGV at runtime when the literal turns out to live in .rodata.")
    content.append("// Real const strings that are only read (e.g. g_AxisLabelChars=\"XYZ\") still")
    content.append("// compile fine. Fix a compile error by retyping the global in Ghidra from")
    content.append("// `char *` to `char[N]`, then re-exporting.")
    ...
    content.append("typedef unsigned short WORD;")
    content.append("typedef unsigned int DWORD;      // 32-bit even on Win64 (LLP64)")
    content.append("typedef short SHORT;")
    content.append("typedef int LONG;               // 32-bit even on Win64 (LLP64)")
    content.append("typedef int INT;")
    content.append("typedef unsigned int UINT;")
    content.append("typedef unsigned short USHORT;")
    content.append("typedef unsigned int ULONG;     // 32-bit even on Win64 (LLP64)")
    content.append("typedef unsigned char UCHAR;")
    ...
    content.append("")
    content.append("// Pointer-sized types (genuinely pointer-width: 4 bytes @32-bit, 8 @64-bit)")
    content.append("typedef intptr_t LONG_PTR;")
    content.append("typedef uintptr_t ULONG_PTR;")
    content.append("typedef uintptr_t DWORD_PTR;")
    content.append("typedef size_t SIZE_T;")
    content.append("typedef uintptr_t UINT_PTR;")
    content.append("typedef intptr_t INT_PTR;")
    content.append("")
...
def generate_watcom_runtime_inlines():
    ...
    lines.append("#endif")
    lines.append("")
    lines.append("// WatcomHeapInfo - forward declaration (full definition from Ghidra types)")
    lines.append("struct WatcomHeapInfo;")
    lines.append("")
    lines.append("extern int _heapchk(void);")
    lines.append("extern int _heapwalk(struct WatcomHeapInfo* entry);")
...
def generate_winsock_runtime_inlines():
    ...
    lines.append("//")
    lines.append("// Implementations are in shims/win32/winsock.cpp (POSIX BSD sockets).")
    lines.append("//")
    lines.append("// =============================================================================")
    lines.append("")
    lines.append("// ---------------------------------------------------------------------------")
...
def export_system_grouped_files(currentProgram, pseudocode_dir, system_grouped_types, type_to_path_map=None):
    ...
    for export_path, types in system_grouped_types.items():
        ...
        if header_name == "stdarg":
            ...
            content.append("//")
            content.append("// An earlier version of VA_START_T used `(char*)(&(last) + 1)` to")
            content.append("// locate the first variadic arg. That works in unsanitized builds")
            content.append("// where parameters live contiguously on the real stack, but under")
            content.append("// AddressSanitizer the compiler copies parameters into a shadow")
            content.append("// stack frame, so `&last` points into the shadow region and `+1`")
            content.append("// walks into ASan-managed zeroed memory. Every vsprintf-style")
            content.append("// callee then reads NULL for every format arg (printed as '(null)').")
            content.append("//")
...
def generate_master_include(pseudocode_dir):
    ...
    content.append("")
    content.append("// Build-time behavior toggles.")
    content.append('#include "shim_config.h"')
    content.append("")
