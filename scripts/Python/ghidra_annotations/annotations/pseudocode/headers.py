...
def generate_watcom_runtime_inlines():
    """Generate Watcom C++ runtime inline function definitions.
    ...
    lines = []
    lines.append("// =============================================================================")
    lines.append("// WATCOM C++ RUNTIME INLINE FUNCTIONS")
    lines.append("// =============================================================================")
    lines.append("//")
    ...
    lines.append("// ---------------------------------------------------------------------------")
    lines.append("")
    lines.append("// __arrinit - Low-level array constructor loop (default ctor)")
    lines.append("inline void* __arrinit(void* array_start, int count, WatcomTypeInfo* ti) {")
    lines.append("    char* p = (char*)array_start;")
    lines.append("    for (int i = 0; i < count; i++, p += ti->instance_size)")
    lines.append("        if (ti->ctor) (*ti->ctor)(p);")
    lines.append("    return array_start;")
    lines.append("}")
    lines.append("")
    lines.append("// __arrinit_ - Low-level array constructor loop (flagged types)")
    lines.append("inline void* __arrinit_(void* array_start, int count, WatcomTypeInfo* ti) {")
    lines.append("    return __arrinit(array_start, count, ti);")
    lines.append("}")
    lines.append("")
    lines.append("// __arrinit_dispatch - Selects flagged vs default ctor based on type_flags")
    lines.append("inline void* __arrinit_dispatch(void* array_ptr, int count, WatcomTypeInfo* ti) {")
    lines.append("    if (!array_ptr) return 0;")
    lines.append("    return __arrinit(array_ptr, count, ti);")
    lines.append("}")
    lines.append("")
    lines.append("// __vec_new_ - Internal: stores count at dest[0], dispatches construction")
    lines.append("inline void* __vec_new_(void* dest, int count, WatcomTypeInfo* ti) {")
    lines.append("    if (!dest) return 0;")
    lines.append("    *(int*)dest = count;")
    lines.append("    return __arrinit_dispatch((char*)dest + 4, count, ti);")
    lines.append("}")
    lines.append("")
    lines.append("// __vec_new - new[] entry point wrapper")
    lines.append("inline void* __vec_new(void* dest, int count, WatcomTypeInfo* ti) {")
    lines.append("    return __vec_new_(dest, count, ti);")
    lines.append("}")
    lines.append("")
    lines.append("// __arrcopy - Array copy-construction loop")
    lines.append("inline void* __arrcopy(void* dest, void* src, int count, WatcomTypeInfo* ti) {")
    lines.append("    char* d = (char*)dest;")
    lines.append("    char* s = (char*)src;")
    lines.append("    for (int i = 0; i < count; i++, d += ti->instance_size, s += ti->instance_size)")
    lines.append("        if (ti->copy) (*ti->copy)(d, s);")
    ...
    lines.append("// ---------------------------------------------------------------------------")
    lines.append("")
    lines.append("// __arrdtor - Low-level array destructor loop (reverse order)")
    lines.append("inline void* __arrdtor(WatcomTypeArrayInfo* info) {")
    lines.append("    char* p = (char*)info->obj_array + (info->obj_count - 1) * info->type_info->instance_size;")
    lines.append("    for (int i = info->obj_count - 1; i >= 0; i--, p -= info->type_info->instance_size)")
    lines.append("        if (info->type_info->dtor) (*info->type_info->dtor)(p, 0);")
    lines.append("    return info->obj_array;")
    lines.append("}")
    lines.append("")
    lines.append("// __arrfini - Array destructor wrapper (packs params, calls __arrdtor)")
    lines.append("inline void* __arrfini(void* obj_array, int count, WatcomTypeInfo* ti) {")
    lines.append("    WatcomTypeArrayInfo info;")
    lines.append("    info.obj_count = count;")
    lines.append("    info.type_info = ti;")
    lines.append("    info.obj_array = (void**)obj_array;")
    lines.append("    __arrdtor(&info);")
    lines.append("    return obj_array;")
    lines.append("}")
    lines.append("")
    lines.append("// __vec_delete - delete[] implementation (reads count from ptr-4)")
    lines.append("inline void* __vec_delete(void* ptr, WatcomTypeInfo* ti) {")
    lines.append("    if (!ptr) return 0;")
    lines.append("    int* base = ((int*)ptr) - 1;")
    lines.append("    __arrfini(ptr, *base, ti);")
    ...
    lines.append("// ---------------------------------------------------------------------------")
    lines.append("")
    lines.append("// __memfill - Optimized memory fill (handles alignment)")
    lines.append("inline void __memfill(void* dest, unsigned int value, unsigned int count) {")
    lines.append("    unsigned char* p = (unsigned char*)dest;")
    lines.append("    for (unsigned int i = 0; i < count; i++) *p++ = (unsigned char)value;")
    lines.append("}")
    lines.append("")
    lines.append("// ---------------------------------------------------------------------------")
    ...
    lines.append("// ---------------------------------------------------------------------------")
    lines.append("")
    lines.append("// notifyAbnormalTermination - Called by error handlers before abort")
    lines.append("void notifyAbnormalTermination();")
    lines.append("")
    lines.append("// __stosd - Bulk DWORD store (unrolled for performance)")
    lines.append("inline void __stosd(void* dest, unsigned int value, unsigned int dword_count) {")
    lines.append("    unsigned int* p = (unsigned int*)dest;")
    lines.append("    for (unsigned int i = 0; i < dword_count; i++) *p++ = value;")
    lines.append("}")
    lines.append("")
    lines.append("// ---------------------------------------------------------------------------")
    ...
    lines.append("// ---------------------------------------------------------------------------")
    lines.append("")
    lines.append("// __STK - Stack probe function (ensures stack pages are committed)")
    lines.append("// Watcom emits this with no args or with an explicit size argument.")
    lines.append("inline void __STK() {}")
    lines.append("inline void __STK(size_t size) {")
    lines.append("    volatile char* p = (volatile char*)&size;")
    lines.append("    while (size > 4096) { p -= 4096; (void)*p; size -= 4096; }")
    lines.append("}")
    ...
    lines.append("#endif")
    lines.append("")
    lines.append("// Watcom __fastcall CRT wrappers (underscore-prefixed variants)")
    lines.append("inline void* _memcpy(void* dest, const void* src, size_t count) { return memcpy(dest, src, count); }")
    lines.append("inline void* _memset(void* dest, int value, size_t count) { return memset(dest, value, count); }")
    lines.append("")
    lines.append("// tell() - get current file position")
    lines.append("#ifndef _MSC_VER")
    lines.append("inline long tell(int fd) { return lseek(fd, 0, SEEK_CUR); }")
    lines.append("#endif")
    lines.append("")
    lines.append("// chsize() - change file size")
    lines.append("#ifndef _MSC_VER")
    lines.append("inline int chsize(int fd, long size) { return ftruncate(fd, size); }")
    lines.append("#endif")
    lines.append("")
    ...
    lines.append("#endif")
    lines.append("")
    lines.append("// _fullpath() - get absolute path")
    lines.append("#ifndef _MSC_VER")
    lines.append("#ifndef PATH_MAX")
    lines.append("#define PATH_MAX 4096")
    lines.append("#endif")
    lines.append("inline char* _fullpath(char* buffer, const char* path, size_t maxlen) {")
    lines.append("    if (buffer == 0) buffer = (char*)malloc(maxlen ? maxlen : PATH_MAX);")
    lines.append("    if (buffer == 0) return 0;")
    lines.append("    if (realpath(path, buffer) == 0) { strcpy(buffer, path); }")
    lines.append("    return buffer;")
    lines.append("}")
    lines.append("#endif")
    lines.append("")
    lines.append("// _getcwd() - get current working directory")
    lines.append("#ifndef _MSC_VER")
    lines.append("inline char* _getcwd(char* buffer, int size) { return getcwd(buffer, size); }")
    lines.append("#endif")
    lines.append("")
    lines.append("// ---------------------------------------------------------------------------")
    ...
    lines.append("")
    lines.append("#ifndef _MSC_VER")
    lines.append("inline long _findfirst(const char* filespec, void* fileinfo) {")
    lines.append("    (void)filespec; (void)fileinfo; return -1;")
    lines.append("}")
    lines.append("inline int _findnext(long handle, void* fileinfo) {")
    lines.append("    (void)handle; (void)fileinfo; return -1;")
    lines.append("}")
    lines.append("inline int _findclose(long handle) {")
    lines.append("    (void)handle; return 0;")
    lines.append("}")
    lines.append("#endif")
    lines.append("")
    lines.append("// ---------------------------------------------------------------------------")
    ...
    lines.append("inline int _mkdir(const char* path) {")
    lines.append("    return mkdir(path, 0755);")
    lines.append("}")
    lines.append("#endif")
    lines.append("")
    lines.append("// ---------------------------------------------------------------------------")
    ...
    lines.append("")
    lines.append("// _heapchk - Check heap consistency")
    lines.append("inline int _heapchk(void) {")
    lines.append("    return _HEAPOK;  // Always report OK for compilation")
    lines.append("}")
    lines.append("")
    lines.append("// _heapwalk - Walk through heap entries")
    lines.append("inline int _heapwalk(struct _heapinfo* entry) {")
    lines.append("    (void)entry;")
    lines.append("    return _HEAPEND;  // Signal end of heap walk")
    lines.append("}")
    lines.append("")
    lines.append("// _memmax - Return largest available memory block")
    lines.append("inline size_t _memmax(void) {")
    lines.append("    return 0x7FFFFFFF;  // Return large value for compilation")
    lines.append("}")
    lines.append("")
    lines.append("// memavl - Return total available memory")
    lines.append("inline size_t memavl(void) {")
    lines.append("    return 0x7FFFFFFF;  // Return large value for compilation")
    lines.append("}")
    lines.append("")
    lines.append("// ---------------------------------------------------------------------------")
    ...
    lines.append("// ---------------------------------------------------------------------------")
    lines.append("")
    lines.append("// __getfileattr - Internal file attribute getter (uses FindFirstFile)")
    lines.append("inline unsigned long __getfileattr(const char* filename) {")
    lines.append("    (void)filename;")
    lines.append("    return 0;  // Return normal file attributes")
    lines.append("}")
    lines.append("")
    lines.append("// __set_errno - Get last Windows error and convert to errno")
    lines.append("inline unsigned long __set_errno(void) {")
    lines.append("    return 0;  // No error")
    lines.append("}")
    lines.append("")
    lines.append("// _errno - Return pointer to errno value (Watcom errno.h)")
    lines.append("inline int* _errno(void) {")
    ...
    lines.append("")
    lines.append("// _atexit - Register static destructor node")
    lines.append("inline void _atexit(WatcomStaticDestructorNode* node) {")
    lines.append("    (void)node;  // No-op stub")
    lines.append("}")
    lines.append("")
    return lines
...
def generate_winsock_runtime_inlines():
    ...
    lines.append("// ---------------------------------------------------------------------------")
    lines.append("")
    lines.append("inline _SOCKET accept(_SOCKET s, struct SOCKADDR* addr, int* addrlen) {")
    lines.append("    (void)s; (void)addr; (void)addrlen;")
    lines.append("    return (_SOCKET)-1;  // INVALID_SOCKET")
    lines.append("}")
    lines.append("")
    lines.append("inline int bind(_SOCKET s, const struct SOCKADDR* addr, int namelen) {")
    lines.append("    (void)s; (void)addr; (void)namelen;")
    lines.append("    return -1;  // SOCKET_ERROR")
    lines.append("}")
    lines.append("")
    lines.append("inline int closesocket(_SOCKET s) {")
    lines.append("    (void)s;")
    lines.append("    return 0;")
    lines.append("}")
    lines.append("")
    lines.append("inline int connect(_SOCKET s, const struct SOCKADDR* name, int namelen) {")
    lines.append("    (void)s; (void)name; (void)namelen;")
    lines.append("    return -1;  // SOCKET_ERROR")
    lines.append("}")
    lines.append("")
    lines.append("inline int getsockname(_SOCKET s, struct SOCKADDR* name, int* namelen) {")
    lines.append("    (void)s; (void)name; (void)namelen;")
    lines.append("    return -1;  // SOCKET_ERROR")
    lines.append("}")
    lines.append("")
    lines.append("inline int ioctlsocket(_SOCKET s, long cmd, uint* argp) {")
    lines.append("    (void)s; (void)cmd; (void)argp;")
    lines.append("    return -1;  // SOCKET_ERROR")
    lines.append("}")
    lines.append("")
    lines.append("inline int listen(_SOCKET s, int backlog) {")
    lines.append("    (void)s; (void)backlog;")
    lines.append("    return -1;  // SOCKET_ERROR")
    lines.append("}")
    lines.append("")
    lines.append("inline int recv(_SOCKET s, char* buf, int len, int flags) {")
    lines.append("    (void)s; (void)buf; (void)len; (void)flags;")
    lines.append("    return -1;  // SOCKET_ERROR")
    lines.append("}")
    lines.append("")
    lines.append("inline int recvfrom(_SOCKET s, char* buf, int len, int flags, struct SOCKADDR* from, int* fromlen) {")
    lines.append("    (void)s; (void)buf; (void)len; (void)flags; (void)from; (void)fromlen;")
    lines.append("    return -1;  // SOCKET_ERROR")
    lines.append("}")
    lines.append("")
    lines.append("inline int send(_SOCKET s, const char* buf, int len, int flags) {")
    lines.append("    (void)s; (void)buf; (void)len; (void)flags;")
    lines.append("    return -1;  // SOCKET_ERROR")
    lines.append("}")
    lines.append("")
    lines.append("inline int sendto(_SOCKET s, const char* buf, int len, int flags, const struct SOCKADDR* to, int tolen) {")
    lines.append("    (void)s; (void)buf; (void)len; (void)flags; (void)to; (void)tolen;")
    lines.append("    return -1;  // SOCKET_ERROR")
    lines.append("}")
    lines.append("")
    lines.append("inline int setsockopt(_SOCKET s, int level, int optname, const char* optval, int optlen) {")
    lines.append("    (void)s; (void)level; (void)optname; (void)optval; (void)optlen;")
    lines.append("    return -1;  // SOCKET_ERROR")
    lines.append("}")
    lines.append("")
    lines.append("inline int shutdown(_SOCKET s, int how) {")
    ...
    lines.append("// ---------------------------------------------------------------------------")
    lines.append("")
    lines.append("inline _SOCKET socket(int af, int type, int protocol) {")
