...
def generate_crt_header(functions_to_process):
    ...
    # Add string comparison wrapper functions
    lines.append("// ---------------------------------------------------------------------------")
    ...
    lines.append("inline double _strtod(const char* str) {")
    lines.append("    return strtod(str, nullptr);")
    lines.append("}")
    lines.append("#endif // _MSC_VER")
    lines.append("")

    # Add path manipulation wrapper functions
    lines.append("// ---------------------------------------------------------------------------")
    ...
    # Add file manipulation wrapper functions
    lines.append("// ---------------------------------------------------------------------------")
    ...
    lines.append("")
    lines.append("inline FILE* _FILE_to_FILE(_FILE* f) {")
    lines.append("    return reinterpret_cast<FILE*>(f);")
    lines.append("}")
    lines.append("")
    lines.append("inline _FILE* _fopen(const char* filename, const char* mode) {")
    lines.append("    return reinterpret_cast<_FILE*>(fopen(filename, mode));")
    lines.append("}")
    lines.append("")
    lines.append("inline int _fclose(_FILE* f) {")
    lines.append("    return fclose(_FILE_to_FILE(f));")
    lines.append("}")
    lines.append("")
    lines.append("inline int _fflush(_FILE* f) {")
    lines.append("    return fflush(_FILE_to_FILE(f));")
    lines.append("}")
    lines.append("")
    lines.append("inline int _feof(_FILE* f) {")
    lines.append("    return feof(_FILE_to_FILE(f));")
    lines.append("}")
    lines.append("")
    lines.append("inline int _ferror(_FILE* f) {")
    lines.append("    return ferror(_FILE_to_FILE(f));")
    lines.append("}")
    lines.append("")
    lines.append("inline long _ftell(_FILE* f) {")
    lines.append("    return ftell(_FILE_to_FILE(f));")
    lines.append("}")
    lines.append("")
    lines.append("inline void _rewind(_FILE* f) {")
    lines.append("    rewind(_FILE_to_FILE(f));")
    lines.append("}")
    lines.append("")
    lines.append("inline int _fgetc(_FILE* f) {")
    lines.append("    return fgetc(_FILE_to_FILE(f));")
    lines.append("}")
    lines.append("")
    lines.append("inline int _fputc(int c, _FILE* f) {")
    lines.append("    return fputc(c, _FILE_to_FILE(f));")
    lines.append("}")
    lines.append("")
    lines.append("inline int _ungetc(int c, _FILE* f) {")
    lines.append("    return ungetc(c, _FILE_to_FILE(f));")
    lines.append("}")
    lines.append("")
    lines.append("inline char* _fgets(char* s, int n, _FILE* f) {")
    lines.append("    return fgets(s, n, _FILE_to_FILE(f));")
    lines.append("}")
    lines.append("")
    lines.append("inline int _fputs(const char* s, _FILE* f) {")
    lines.append("    return fputs(s, _FILE_to_FILE(f));")
    lines.append("}")
    lines.append("")
    lines.append("inline size_t _fread(void* ptr, size_t size, size_t count, _FILE* f) {")
    lines.append("    return fread(ptr, size, count, _FILE_to_FILE(f));")
    lines.append("}")
    lines.append("")
    lines.append("inline size_t _fwrite(const void* ptr, size_t size, size_t count, _FILE* f) {")
    lines.append("    return fwrite(ptr, size, count, _FILE_to_FILE(f));")
    lines.append("}")
    lines.append("")
    lines.append("inline int _fseek(_FILE* f, long offset, int whence) {")
    lines.append("    return fseek(_FILE_to_FILE(f), offset, whence);")
    lines.append("}")
    lines.append("")
    lines.append("inline int _fsetpos(_FILE* f, const fpos_t* pos) {")
    lines.append("    return fsetpos(_FILE_to_FILE(f), pos);")
    lines.append("}")
    lines.append("")
    lines.append("inline int _fgetpos(_FILE* f, fpos_t* pos) {")
    lines.append("    return fgetpos(_FILE_to_FILE(f), pos);")
    lines.append("}")
    lines.append("")
    lines.append("inline int _setvbuf(_FILE* f, char* buf, int mode, size_t size) {")
    lines.append("    return setvbuf(_FILE_to_FILE(f), buf, mode, size);")
    lines.append("}")
    lines.append("")
    lines.append("inline void _setbuf(_FILE* f, char* buf) {")
    lines.append("    setbuf(_FILE_to_FILE(f), buf);")
    lines.append("}")
    lines.append("")
    lines.append("__attribute__((format(printf, 2, 3)))")
    lines.append("inline int _fprintf(_FILE* f, const char* format, ...) {")
    lines.append("    va_list args;")
    lines.append("    va_start(args, format);")
    lines.append("    int result = vfprintf(_FILE_to_FILE(f), format, args);")
    lines.append("    va_end(args);")
    lines.append("    return result;")
    lines.append("}")
    lines.append("")
    lines.append("__attribute__((format(scanf, 2, 3)))")
    lines.append("inline int _fscanf(_FILE* f, const char* format, ...) {")
    lines.append("    va_list args;")
    lines.append("    va_start(args, format);")
    lines.append("    int result = vfscanf(_FILE_to_FILE(f), format, args);")
    lines.append("    va_end(args);")
    lines.append("    return result;")
    lines.append("}")
    lines.append("")
    lines.append("inline _FILE* _freopen(const char* filename, const char* mode, _FILE* stream) {")
    lines.append("    return reinterpret_cast<_FILE*>(freopen(filename, mode, _FILE_to_FILE(stream)));")
    lines.append("}")
    lines.append("")
    lines.append("// ---------------------------------------------------------------------------")
    ...
    lines.append("")
    lines.append("inline int _vsprintf(char* buffer, const char* format, va_list_t args) {")
    lines.append("    return vsprintf(buffer, format, reinterpret_cast<va_list&>(args));")
    lines.append("}")
    lines.append("")
    lines.append("__attribute__((format(printf, 2, 3)))")
    lines.append("inline int _sprintf(void* buffer, const char* format, ...) {")
    lines.append("    va_list args;")
    lines.append("    va_start(args, format);")
    lines.append("    int result = vsprintf((char*)buffer, format, args);")
    lines.append("    va_end(args);")
    lines.append("    return result;")
    lines.append("}")
    lines.append("")
    ...
    lines.append("// ---------------------------------------------------------------------------")
    lines.append("")
    lines.append("inline char* _strncpy(void* dest, const void* src, size_t count) {")
    lines.append("    return strncpy((char*)dest, (const char*)src, count);")
    lines.append("}")
    lines.append("")

    # Add time manipulation wrapper functions
    lines.append("// ---------------------------------------------------------------------------")
    ...
    lines.append("")
    lines.append("inline time_t _mktime(_tm* t) {")
    lines.append("    return mktime(reinterpret_cast<tm*>(t));")
    lines.append("}")
    lines.append("")
