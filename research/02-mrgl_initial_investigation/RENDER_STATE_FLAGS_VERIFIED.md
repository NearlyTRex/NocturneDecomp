...
| **0x004** | `RENDER_PRIMITIVE_SPECIAL` | **Color-accumulator source #2 (fog-Z)**: when set (and 0x200 clear), the per-pixel lighting color is derived from the interpolated Z: `clamp(z - 0x100, 0, 0xfff) << 3`, broadcast to all three RGB channels. | keep.cpp:136–150 | ⚠️ **Rename** — name is meaningless. Suggested: `RENDER_COLOR_FROM_FOG` or `RENDER_FOG_COLOR`. |
| **0x008** | `RENDER_ADVANCED_MODE` | In Loop C only: **gates alpha-blend against `g_SolidColorMode`**. Clear → skip alpha setup, emit lit pixel raw. Set → LERP `(lit_pixel, g_SolidColorMode)` with per-pixel alpha via `g_AlphaTable`. Unused in Loops A/B. | keep.cpp:181, 216 | ⚠️ **Rename** — not a generic "advanced mode" toggle. Suggested: `RENDER_SOLID_ALPHA_BLEND` (only meaningful in the solid loop). |
...
else if (flags & 0x004) { color = fog-Z derived gray ramp }
else if (flags & 0x010) { color = lighting-value lookup (g_AlphaTable) }
else                    { color = full-bright (g_AlphaTable[0xff]) }   // default
```

If a primitive has 0x200 **and** 0x004 set in its flags, only 0x200 takes effect — the fog bit is dead. Setting 0x004 **and** 0x010 together similarly drops the lighting-value path. This is important when reasoning about which writer combos produce which runtime color mode.
