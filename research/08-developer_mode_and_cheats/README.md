...
Mapping of every developer/debug/options surface in `nocedit.exe`: env-var gates, runtime flags, typed-cheat-codes, ini configuration, and the script event flags that live in the mission data rather than the binary. Split into the topic docs below.
...
| [`script_event_flags.md`](script_event_flags.md) | The second debug surface, which lives in the mission data rather than the executable: the ~50 dormant developer warps the shipped `.SCR` scripts test for and nothing raises, the three event stores (`events` / `persistent_events` / `game_flags`) and when each is the right one, and the game's one shipped easter egg. |
...
| [`retail_vs_editor_mysteries.md`](retail_vs_editor_mysteries.md) | Open mysteries about retail-vs-editor build differences. Currently covers: (1) why D3D is permanently disabled in Graphics Options (definitive — hardcoded clobber every frame), (2) why cutscene voices don't play despite subtitle render (partial — orphan `loadStreamingSoundFile`), (3) why START shows a flat `.msn` picker instead of the curated chapter list (definitive — `pod.ini` presence sentinel), (4) why resolution is capped at 640×480 and graphics-options changes don't persist (definitive cap from §1; Linux-port persistence fails on un-normalized `remove`/`rename`), (5) why the editor probes 60 `APIDLL*` entry points when every shipped renderer DLL has 37 (definitive — an unshipped hardware-T&L renderer; the 23 extras are bound but never called). |
