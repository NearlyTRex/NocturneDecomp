...
- **Access is gated by `NOCTURNE_EDITOR_BUILD`** (`shims/config/shim_config_authentic.h`, default `0`). That flag is on the which-binary axis rather than the shipped-versus-improved one, and it covers the whole editor surface — this menu, the branding banner, and the editor-only cheats — so neither of its settings is less faithful than the other. At `1` the developer-tools menu is reachable two ways: a `Developer tools` entry on the Options screen — main-menu route only, since `showDeveloperToolsMenu` resets the sound system and under `NOCTURNE_AUTHENTIC_D3D_OPTIONS` kills acceleration — and the original `Ctrl+D` / `Ctrl+L` hotkeys. At `0` neither exists and the build presents as retail `nocturne.exe`. Affected keeps: `showOptionsScreen_FUN_00512d30.keep.cpp`, `enterMainGameMenu_FUN_00507a50.keep.c`, `showMainGameMenu_FUN_00512f40.keep.cpp` (orphan; no callers).
