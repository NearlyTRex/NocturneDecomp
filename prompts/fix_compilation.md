...
- `subfield_vector_pun` — §26 (Split contiguous Watcom stack block via field-pun). Two shapes. The **vector** shape is `(CVector3f *)&LOCAL.y` / `&LOCAL.z` — a 3-component access starting mid-vector. The **struct** shape is `(T *)&LOCAL.f1.f2` where the byte offset and both type sizes are known and `sizeof(T) > sizeof(typeof(LOCAL)) - offset`; the dominant case is *self-typed* (`(CBoundingBox3D *)&local_box.max.y` — a full-object access starting 16 bytes into a 24-byte box), which Ghidra emits when its stack-local partition disagrees with Watcom's real frame layout. Both are guaranteed ASan `stack-buffer-overflow`. Fix by passing a real contiguous local — for a pure scratch out-param (written by the callee, consumed immediately) that is usually just `&LOCAL` itself; confirm the intended slot in the `.asm` first. The struct shape needs the type maps, so it is inert if `data_types.json` didn't load. Comparison-only puns (`&x != (T *)&x.max`) never dereference and are deliberately not flagged — those are dead address-identity guards (§23-adjacent), not overruns.
