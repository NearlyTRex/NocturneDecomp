...
- **Report missed-suspect siblings.** When you collapse a flagged suspect and notice the same pattern elsewhere in the function (or a near-identical shape that should match the same detector), tell the user before moving on: cite the function, line, and the fingerprint that the existing detector misses. Suggest updating the suspect detector so every other function with the same shape gets flagged on the next re-export. Fixing the local instance in the current `.keep` is fine; the detector gap is the actual bug, since one function-at-a-time cleanup can't catch the rest of the codebase.
