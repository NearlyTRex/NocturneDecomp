...
- **One suspect, one edit — but fix all of them in the pass.** After resolving the suspect the user named, re-run `test_suspects.sh` on the `.keep` and fix every other eligible suspect it still flags in the same function. Each fix is still its own targeted edit (don't bundle unrelated rewrites into one diff), but the goal of the pass is to drive the function's flagged-suspect count to zero, not to leave siblings behind for a future round. Skip only suspects that are ineligible for `.keep` cleanup (see the ineligibility list below) or that you cannot rewrite *unambiguously* from the asm — explain each skip to the user.
