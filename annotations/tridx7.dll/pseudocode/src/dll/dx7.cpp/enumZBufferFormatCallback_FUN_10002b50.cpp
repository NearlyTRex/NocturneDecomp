...
// Signature: HRESULT __stdcall dll_dx7_cpp_enumZBufferFormatCallback_FUN_10002b50(DDPIXELFORMAT *lp_pixel_format,DDPIXELFORMAT *lp_context)
  ...
  if (lp_pixel_format->dwFlags == 0x400) {
    if (g_ScreenBitDepth != 0x20) {
      for (iVar1 = 8; iVar1 != 0; iVar1 = iVar1 + -1) {
        lp_context->dwSize = lp_pixel_format->dwSize;
        lp_pixel_format = (DDPIXELFORMAT *)&lp_pixel_format->dwFlags;
        lp_context = (DDPIXELFORMAT *)&lp_context->dwFlags;
      }
      return 0;
    }
    if ((lp_context->dwBitCount).dwRGBBitCount < (lp_pixel_format->dwBitCount).dwRGBBitCount) {
      for (iVar1 = 8; iVar1 != 0; iVar1 = iVar1 + -1) {
        lp_context->dwSize = lp_pixel_format->dwSize;
        lp_pixel_format = (DDPIXELFORMAT *)&lp_pixel_format->dwFlags;
        lp_context = (DDPIXELFORMAT *)&lp_context->dwFlags;
