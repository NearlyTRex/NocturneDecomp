#pragma once

// Function prototypes for core/weapon.cpp
// Generated from Ghidra function signatures

// Original: core_weapon.cpp_staticInit_FUN_005ede40
// Address: 005ede40
void staticInit(void);

// Original: core_weapon.cpp_CWeapon_ctor_FUN_005ede70
// Address: 005ede70
CWeapon * CWeapon::ctor(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_dtor_FUN_005edf80
// Address: 005edf80
CWeapon * CWeapon::dtor(CWeapon * this_ptr, uint d1, uint d2, uint d3);

// Original: core_weapon.cpp_CWeapon_FUN_005edff0
// Address: 005edff0
void CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_FUN_005ee030
// Address: 005ee030
int CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_FUN_005ee0c0
// Address: 005ee0c0
int * CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_process_FUN_005ee110
// Address: 005ee110
void CWeapon::process(CWeapon * this_ptr, float delta_time);

// Original: core_weapon.cpp_CWeapon_serialize_FUN_005ee250
// Address: 005ee250
void CWeapon::serialize(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_FUN_005ee330
// Address: 005ee330
int CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_FUN_005ee340
// Address: 005ee340
int CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_FUN_005ee3e0
// Address: 005ee3e0
void CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_FUN_005ee3f0
// Address: 005ee3f0
void CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_FUN_005ee490
// Address: 005ee490
int CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_FUN_005ee4a0
// Address: 005ee4a0
void CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_FUN_005ee640
// Address: 005ee640
void CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_FUN_005ee650
// Address: 005ee650
void CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_FUN_005ee670
// Address: 005ee670
int * CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_FUN_005ee6e0
// Address: 005ee6e0
undefined FUN_005ee6e0();

// Original: core_weapon.cpp_CWeapon_FUN_005ee710
// Address: 005ee710
int CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_FUN_005ee730
// Address: 005ee730
int CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_FUN_005ee740
// Address: 005ee740
void FUN_005ee740(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_FUN_005ee830
// Address: 005ee830
void CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_FUN_005ee860
// Address: 005ee860
void CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_FUN_005ee9f0
// Address: 005ee9f0
void CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_CWeapon_FUN_005eea60
// Address: 005eea60
void CWeapon(CWeapon * this_ptr);

// Original: core_weapon.cpp_CDemonGlobe_ctor_FUN_005eea80
// Address: 005eea80
CDemonGlobe * CDemonGlobe::ctor(CDemonGlobe * this_ptr);

// Original: core_weapon.cpp_CDemonGlobe_dtor_FUN_005eea90
// Address: 005eea90
CDemonGlobe * CDemonGlobe::dtor(CDemonGlobe * this_ptr);
