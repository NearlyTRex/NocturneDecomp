#pragma once

// Function prototypes for core/tentacle.cpp
// Generated from Ghidra function signatures

// Original: core_tentacle.cpp_staticInit_FUN_005daef0
// Address: 005daef0
CDemonActorType * staticInit(void);

// Original: core_tentacle.cpp_FUN_005daf20
// Address: 005daf20
undefined FUN_005daf20();

// Original: core_tentacle.cpp_CTentacle_getActorType_FUN_005daf50
// Address: 005daf50
CDemonActorType * CTentacle::getActorType(CTentacle * this_ptr);

// Original: core_tentacle.cpp_CTentacle_ctor_FUN_005daf60
// Address: 005daf60
CTentacle * CTentacle::ctor(CTentacle * this_ptr);

// Original: core_tentacle.cpp_CTentacle_FUN_005dafc0
// Address: 005dafc0
void CTentacle(CTentacle * this_ptr);

// Original: core_tentacle.cpp_CTentacle_process_FUN_005db050
// Address: 005db050
void CTentacle::process(CTentacle * this_ptr);

// Original: core_tentacle.cpp_CTentacle_FUN_005db840
// Address: 005db840
int CTentacle(CTentacle * this_ptr);

// Original: core_tentacle.cpp_CTentacle_serialize_FUN_005db880
// Address: 005db880
void CTentacle::serialize(CTentacle * this_ptr);

// Original: core_tentacle.cpp_FUN_005db900
// Address: 005db900
undefined FUN_005db900();

// Original: core_tentacle.cpp_FUN_005db9d0
// Address: 005db9d0
undefined FUN_005db9d0();

// Original: core_tentacle.cpp_CTentacle_FUN_005dbb30
// Address: 005dbb30
int CTentacle(CTentacle * this_ptr);

// Original: core_tentacle.cpp_FUN_005dbb70
// Address: 005dbb70
undefined FUN_005dbb70();

// Original: core_tentacle.cpp_CTentacle_FUN_005dbc60
// Address: 005dbc60
void CTentacle(CTentacle * this_ptr);

// Original: core_tentacle.cpp_CTentacle_FUN_005dbcc0
// Address: 005dbcc0
void CTentacle(CTentacle * this_ptr);

// Original: core_tentacle.cpp_CTentacle_dtor_FUN_005dbcf0
// Address: 005dbcf0
CTentacle * CTentacle::dtor(CTentacle * this_ptr, uint d1, uint d2, uint d3, uint d4, uint d5, uint d6, uint d7, uint d8);
