#pragma once

// Function prototypes for core/simbox.cpp
// Generated from Ghidra function signatures

// Original: core_simbox.cpp_staticInit_FUN_00588cd0
// Address: 00588cd0
void staticInit(void);

// Original: core_simbox.cpp_CSimBox_FUN_00588d00
// Address: 00588d00
void CSimBox(CSimBox * this_ptr);

// Original: core_simbox.cpp_CSimBox_getActorType_FUN_00588d30
// Address: 00588d30
CDemonActorType * CSimBox::getActorType(CSimBox * this_ptr);

// Original: core_simbox.cpp_CSimBox_ctor_FUN_00588d40
// Address: 00588d40
CSimBox * CSimBox::ctor(CSimBox * this_ptr);

// Original: core_simbox.cpp_CSimBox_FUN_00588e00
// Address: 00588e00
void CSimBox(CSimBox * this_ptr);

// Original: core_simbox.cpp_CSimBox_process_FUN_00588f20
// Address: 00588f20
void CSimBox::process(CSimBox * this_ptr);

// Original: core_simbox.cpp_CSimBox_FUN_005890f0
// Address: 005890f0
int CSimBox(CSimBox * this_ptr);

// Original: core_simbox.cpp_CSimBox_FUN_00589160
// Address: 00589160
float * CSimBox(CSimBox * this_ptr);

// Original: core_simbox.cpp_CSimBox_FUN_005891b0
// Address: 005891b0
void CSimBox(CSimBox * this_ptr);

// Original: core_simbox.cpp_CSimBox_FUN_00589260
// Address: 00589260
int CSimBox(CSimBox * this_ptr);

// Original: core_simbox.cpp_FUN_00589270
// Address: 00589270
void FUN_00589270(CSimBox * this_ptr);

// Original: core_simbox.cpp_FUN_005892d0
// Address: 005892d0
int FUN_005892d0(CSimBox * this_ptr);

// Original: core_simbox.cpp_CSimBox_FUN_00589310
// Address: 00589310
void CSimBox(CSimBox * this_ptr);

// Original: core_simbox.cpp_CSimBox_FUN_005893c0
// Address: 005893c0
void CSimBox(CSimBox * this_ptr);

// Original: core_simbox.cpp_CSimBox_dtor_FUN_005893e0
// Address: 005893e0
CSimBox * CSimBox::dtor(CSimBox * this_ptr, uint d1, uint d2, uint d3);
