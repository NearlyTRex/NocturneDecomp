#pragma once

// Function prototypes for core/waypoint.cpp
// Generated from Ghidra function signatures

// Original: core_waypoint.cpp_staticInit_FUN_005ebd10
// Address: 005ebd10
void staticInit(void);

// Original: core_waypoint.cpp_FUN_005ebd40
// Address: 005ebd40
undefined FUN_005ebd40();

// Original: core_waypoint.cpp_factoryFunc_FUN_005ebe30
// Address: 005ebe30
CWayPoint * factoryFunc(void);

// Original: core_waypoint.cpp_CWayPoint_getActorType_FUN_005ebe60
// Address: 005ebe60
CDemonActorType * CWayPoint::getActorType(CWayPoint * this_ptr);

// Original: core_waypoint.cpp_CWayPoint_ctor_FUN_005ebe70
// Address: 005ebe70
CWayPoint * CWayPoint::ctor(CWayPoint * this_ptr);

// Original: core_waypoint.cpp_CWaypoint_FUN_005ebeb0
// Address: 005ebeb0
void CWaypoint(CWayPoint * this_ptr);

// Original: core_waypoint.cpp_CWaypoint_FUN_005ebf70
// Address: 005ebf70
int CWaypoint(CWayPoint * this_ptr);

// Original: core_waypoint.cpp_CWayPoint_FUN_005ec280
// Address: 005ec280
void CWayPoint(CWayPoint * this_ptr);

// Original: core_waypoint.cpp_FUN_005ec320
// Address: 005ec320
undefined FUN_005ec320();

// Original: core_waypoint.cpp_CWayPoint_FUN_005ec4d0
// Address: 005ec4d0
void CWayPoint(CWayPoint * this_ptr);

// Original: core_waypoint.cpp_FUN_005ec4f0
// Address: 005ec4f0
undefined FUN_005ec4f0();

// Original: core_waypoint.cpp_CWayPoint_FUN_005ec5e0
// Address: 005ec5e0
void CWayPoint(CWayPoint * this_ptr);

// Original: core_waypoint.cpp_CWayPoint_FUN_005ec610
// Address: 005ec610
void CWayPoint(CWayPoint * this_ptr);

// Original: core_waypoint.cpp_CallWayPoint_removeAdj_FUN_005ec640
// Address: 005ec640
undefined CallWayPoint::removeAdj();

// Original: core_waypoint.cpp_CWayPoint_FUN_005ec690
// Address: 005ec690
void CWayPoint(CWayPoint * this_ptr);

// Original: core_waypoint.cpp_CWayPoint_addAdj_FUN_005ec700
// Address: 005ec700
undefined CWayPoint::addAdj();

// Original: core_waypoint.cpp_CWayPoint_removeAdj_FUN_005ec7b0
// Address: 005ec7b0
undefined CWayPoint::removeAdj();

// Original: core_waypoint.cpp_FUN_005ec830
// Address: 005ec830
undefined FUN_005ec830();

// Original: core_waypoint.cpp_CWayPoint_dtor_FUN_005ec8d0
// Address: 005ec8d0
CWayPoint * CWayPoint::dtor(CWayPoint * this_ptr, uint d1, uint d2);
