#pragma once

// Function prototypes for core/flamegun.cpp
// Generated from Ghidra function signatures

// Original: core_flamegun.cpp_staticInit_FUN_004cb810
// Address: 004cb810
void __cdecl staticInit(void);

// Original: core_flamegun.cpp_factoryFunc_FUN_004cb840
// Address: 004cb840
CFlameThrower * __cdecl factoryFunc(void);

// Original: core_flamegun.cpp_CFlameThrower_getActorType_FUN_004cb870
// Address: 004cb870
CDemonActorType * __cdecl CFlameThrower::getActorType(CFlameThrower *this_ptr);

// Original: core_flamegun.cpp_CFlameThrower_ctor_FUN_004cb880
// Address: 004cb880
CFlameThrower * __cdecl CFlameThrower::ctor(CFlameThrower *this_ptr);

// Original: core_flamegun.cpp_FUN_004cb920
// Address: 004cb920
undefined4 FUN_004cb920(void);

// Original: core_flamegun.cpp_CFlameThrower_process_FUN_004cb9b0
// Address: 004cb9b0
void __cdecl CFlameThrower::process(CFlameThrower *this_ptr,float delta_time);

// Original: core_flamegun.cpp_FUN_004cbab0
// Address: 004cbab0
float FUN_004cbab0(void);

// Original: core_flamegun.cpp_FUN_004cbad0
// Address: 004cbad0
void FUN_004cbad0(void);

// Original: core_flamegun.cpp_FUN_004cbad1
// Address: 004cbad1
float FUN_004cbad1(void);

// Original: core_flamegun.cpp_CFlameThrower_dtor_FUN_004cbb00
// Address: 004cbb00
CFlameThrower * __cdecl CFlameThrower::dtor(CFlameThrower *this_ptr,uint d1,uint d2);

// Original: core_flamegun.cpp_CFlashlight_getActorType_FUN_004cbbb0
// Address: 004cbbb0
CDemonActorType * __cdecl CFlashlight::getActorType(CFlashlight *this_ptr);

// Original: core_flamegun.cpp_CFlashlight_ctor_FUN_004cbbc0
// Address: 004cbbc0
CFlashlight * __cdecl CFlashlight::ctor(CFlashlight *this_ptr);

// Original: core_flamegun.cpp_FUN_004cbc40
// Address: 004cbc40
undefined4 FUN_004cbc40(void);

// Original: core_flamegun.cpp_FUN_004cbc50
// Address: 004cbc50
undefined4 FUN_004cbc50(void);

// Original: core_flamegun.cpp_CFlashlight_dtor_FUN_004cbc60
// Address: 004cbc60
CFlashlight * __cdecl CFlashlight::dtor(CFlashlight *this_ptr,uint d1,uint d2);

// Original: core_flamegun.cpp_staticInit_FUN_004cbcb0
// Address: 004cbcb0
void __cdecl staticInit(void);

// Original: core_flamegun.cpp_initializeFlamegun_FUN_004cbce0
// Address: 004cbce0
void __cdecl initializeFlamegun(void);
