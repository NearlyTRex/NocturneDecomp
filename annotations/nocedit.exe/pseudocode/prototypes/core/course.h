#pragma once

// Function prototypes for core/course.cpp
// Generated from Ghidra function signatures

// Original: core_course.cpp_CCourse_ctor_FUN_004424c0
// Address: 004424c0
CCourse * CCourse::ctor(CCourse * this_ptr);

// Original: core_course.cpp_CCourse_dtor_FUN_004424e0
// Address: 004424e0
CCourse * CCourse::dtor(CCourse * this_ptr);

// Original: core_course.cpp_CCourse_allocMemory_FUN_00442500
// Address: 00442500
void CCourse::allocMemory(CCourse * this_ptr);

// Original: core_course.cpp_CCourse_load_FUN_00442580
// Address: 00442580
void CCourse::load(CCourse * this_ptr, char * filename);

// Original: core_course.cpp_CCourse_free_FUN_004426c0
// Address: 004426c0
void CCourse::free(CCourse * this_ptr);

// Original: core_course.cpp_CCourse_FUN_00442710
// Address: 00442710
void CCourse(CCourse * this_ptr);

// Original: core_course.cpp_CCourse_FUN_004427a0
// Address: 004427a0
undefined CCourse();

// Original: core_course.cpp_CCourse_save_FUN_00442a90
// Address: 00442a90
void CCourse::save(CCourse * this_ptr);

// Original: core_course.cpp_OpeningCourseFileMaybe_FUN_00442bc0
// Address: 00442bc0
undefined OpeningCourseFileMaybe();

// Original: core_course.cpp_OpeningCourseFile2_FUN_00442d70
// Address: 00442d70
undefined OpeningCourseFile2();

// Original: core_course.cpp_CCourse_importMatrices_FUN_00442ea0
// Address: 00442ea0
void CCourse::importMatrices(CCourse * this_ptr);

// Original: core_course.cpp_CCourse_showEditorMenu_FUN_00443040
// Address: 00443040
void CCourse::showEditorMenu(CCourse * this_ptr);

// Original: core_course.cpp_FUN_004436d0
// Address: 004436d0
undefined FUN_004436d0();

// Original: core_course.cpp_FUN_00443760
// Address: 00443760
undefined FUN_00443760();

// Original: core_course.cpp_FUN_00443bc0
// Address: 00443bc0
undefined FUN_00443bc0();
