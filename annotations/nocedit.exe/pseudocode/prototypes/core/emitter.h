#pragma once

// Function prototypes for core/emitter.cpp
// Generated from Ghidra function signatures

// Original: core_emitter.cpp_staticInit_FUN_004a7c30
// Address: 004a7c30
void staticInit(void);

// Original: core_emitter.cpp_factoryFunc_FUN_004a7c60
// Address: 004a7c60
CEmitter * factoryFunc(void);

// Original: core_emitter.cpp_OtherClass_FUN_004a7c90
// Address: 004a7c90
undefined OtherClass();

// Original: core_emitter.cpp_CEmitter_ctor_FUN_004a7ca0
// Address: 004a7ca0
CEmitter * CEmitter::ctor(CEmitter * this_ptr);

// Original: core_emitter.cpp_FUN_004a7e00
// Address: 004a7e00
undefined FUN_004a7e00();

// Original: core_emitter.cpp_FUN_004a7e80
// Address: 004a7e80
undefined FUN_004a7e80();

// Original: core_emitter.cpp_FUN_004a8070
// Address: 004a8070
undefined FUN_004a8070();

// Original: core_emitter.cpp_FUN_004a8860
// Address: 004a8860
undefined FUN_004a8860();

// Original: core_emitter.cpp_FUN_004a8a80
// Address: 004a8a80
undefined FUN_004a8a80();

// Original: core_emitter.cpp_FUN_004a8ad0
// Address: 004a8ad0
undefined FUN_004a8ad0();

// Original: core_emitter.cpp_CEmitter_load_FUN_004a8b30
// Address: 004a8b30
undefined CEmitter::load();

// Original: core_emitter.cpp_FUN_004a8d10
// Address: 004a8d10
undefined FUN_004a8d10();

// Original: core_emitter.cpp_FUN_004a8d20
// Address: 004a8d20
undefined FUN_004a8d20();

// Original: core_emitter.cpp_FUN_004a8db0
// Address: 004a8db0
undefined FUN_004a8db0();

// Original: core_emitter.cpp_FUN_004a8fe0
// Address: 004a8fe0
undefined FUN_004a8fe0();

// Original: core_emitter.cpp_FUN_004a9350
// Address: 004a9350
undefined FUN_004a9350();

// Original: core_emitter.cpp_FUN_004a9390
// Address: 004a9390
undefined FUN_004a9390();

// Original: core_emitter.cpp_CEmitter_dtor_FUN_004a93c0
// Address: 004a93c0
CEmitter * CEmitter::dtor(CEmitter * this_ptr, uint d1, uint d2);

// Original: core_emitter.cpp_SLaserInfo_copy_FUN_004a9410
// Address: 004a9410
void SLaserInfo::copy(SLaserInfo * this_ptr, SLaserInfo * copy);

// Original: core_emitter.cpp_SLaserInfo_dtor_FUN_004a94c0
// Address: 004a94c0
SLaserInfo * SLaserInfo::dtor(SLaserInfo * this_ptr);
