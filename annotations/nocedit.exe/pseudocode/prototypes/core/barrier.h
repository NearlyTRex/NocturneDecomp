#pragma once

// Function prototypes for core/barrier.cpp
// Generated from Ghidra function signatures

// Original: core_barrier.cpp_staticInit_FUN_00414150
// Address: 00414150
void staticInit(void);

// Original: core_barrier.cpp_factoryFunc_FUN_00414180
// Address: 00414180
CBarrier * factoryFunc(void);

// Original: core_barrier.cpp_CBarrier_getActorType_FUN_004141b0
// Address: 004141b0
CDemonActorType * CBarrier::getActorType(CBarrier * this_ptr);

// Original: core_barrier.cpp_CBarrier_ctor_FUN_004141c0
// Address: 004141c0
CBarrier * CBarrier::ctor(CBarrier * this_ptr);

// Original: core_barrier.cpp_CBarrier_FUN_00414210
// Address: 00414210
void CBarrier(CBarrier * this_ptr);

// Original: core_barrier.cpp_CBarrier_FUN_00414240
// Address: 00414240
int CBarrier(CBarrier * this_ptr);

// Original: core_barrier.cpp_CBarrier_FUN_004142e0
// Address: 004142e0
void CBarrier(CBarrier * this_ptr);

// Original: core_barrier.cpp_CBarrier_serialize_FUN_00414340
// Address: 00414340
void CBarrier::serialize(CBarrier * this_ptr);

// Original: core_barrier.cpp_CBarrier_FUN_004143a0
// Address: 004143a0
int CBarrier(CBarrier * this_ptr);

// Original: core_barrier.cpp_CBarrier_FUN_004143e0
// Address: 004143e0
void CBarrier(CBarrier * this_ptr);

// Original: core_barrier.cpp_CBarrier_FUN_00414400
// Address: 00414400
void CBarrier(CBarrier * this_ptr);

// Original: core_barrier.cpp_CBarrier_FUN_00414460
// Address: 00414460
void CBarrier(CBarrier * this_ptr);

// Original: core_barrier.cpp_CBarrier_FUN_00414610
// Address: 00414610
void CBarrier(CBarrier * this_ptr);

// Original: core_barrier.cpp_CBarrier_dtor_FUN_00414670
// Address: 00414670
CBarrier * CBarrier::dtor(CBarrier * this_ptr, uint d1, uint d2);
