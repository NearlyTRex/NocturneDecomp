#pragma once

// Function prototypes for core/svetlana.cpp
// Generated from Ghidra function signatures

// Original: core_svetlana.cpp_staticInit_FUN_005d87e0
// Address: 005d87e0
CDemonActorType * staticInit(void);

// Original: core_svetlana.cpp_FUN_005d8810
// Address: 005d8810
undefined FUN_005d8810();

// Original: core_svetlana.cpp_CSvetlana_getActorType_FUN_005d8840
// Address: 005d8840
CDemonActorType * CSvetlana::getActorType(CSvetlana * this_ptr);

// Original: core_svetlana.cpp_CSvetlana_ctor_FUN_005d8850
// Address: 005d8850
CSvetlana * CSvetlana::ctor(CSvetlana * this_ptr);

// Original: core_svetlana.cpp_CSvetlana_FUN_005d88e0
// Address: 005d88e0
void CSvetlana(CSvetlana * this_ptr);

// Original: core_svetlana.cpp_CSvetlana_process_FUN_005d8ba0
// Address: 005d8ba0
void CSvetlana::process(CSvetlana * this_ptr, float delta_time);

// Original: core_svetlana.cpp_FUN_005d9260
// Address: 005d9260
undefined FUN_005d9260();

// Original: core_svetlana.cpp_MotionControllerAdvance_ParentBones_FUN_005d9970
// Address: 005d9970
undefined MotionControllerAdvance::ParentBones();

// Original: core_svetlana.cpp_AttackWithParentBone_FUN_005d9a10
// Address: 005d9a10
undefined AttackWithParentBone();

// Original: core_svetlana.cpp_CSVetlana_serialize_FUN_005d9bb0
// Address: 005d9bb0
void CSVetlana::serialize(CSvetlana * this_ptr);

// Original: core_svetlana.cpp_CSvetlana_FUN_005d9bc0
// Address: 005d9bc0
int CSvetlana(CSvetlana * this_ptr);

// Original: core_svetlana.cpp_FUN_005d9d30
// Address: 005d9d30
undefined FUN_005d9d30();

// Original: core_svetlana.cpp_FUN_005d9ec0
// Address: 005d9ec0
undefined FUN_005d9ec0();

// Original: core_svetlana.cpp_FUN_005d9ed0
// Address: 005d9ed0
undefined FUN_005d9ed0();

// Original: core_svetlana.cpp_FUN_005d9ee0
// Address: 005d9ee0
undefined FUN_005d9ee0();

// Original: core_svetlana.cpp_CSvetlana_FUN_005d9ef0
// Address: 005d9ef0
void CSvetlana(CSvetlana * this_ptr);

// Original: core_svetlana.cpp_CSvetlana_FUN_005d9f10
// Address: 005d9f10
void CSvetlana(CSvetlana * this_ptr);

// Original: core_svetlana.cpp_CSvetlana_FUN_005d9f30
// Address: 005d9f30
void CSvetlana(CSvetlana * this_ptr);

// Original: core_svetlana.cpp_CSvetlana_dtor_FUN_005d9f50
// Address: 005d9f50
CSvetlana * CSvetlana::dtor(CSvetlana * this_ptr, uint d1, uint d2, uint d3, uint d4);
