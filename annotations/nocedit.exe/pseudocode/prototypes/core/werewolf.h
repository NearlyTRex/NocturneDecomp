#pragma once

// Function prototypes for core/werewolf.cpp
// Generated from Ghidra function signatures

// Original: core_werewolf.cpp_staticInit_FUN_005efb70
// Address: 005efb70
void staticInit(void);

// Original: core_werewolf.cpp_factoryFunc_FUN_005efbd0
// Address: 005efbd0
CWerewolf * factoryFunc(void);

// Original: core_werewolf.cpp_CWerewolf_getActorType_FUN_005efc00
// Address: 005efc00
CDemonActorType * CWerewolf::getActorType(CWerewolf * this_ptr);

// Original: core_werewolf.cpp_CWerewolf_ctor_FUN_005efc10
// Address: 005efc10
CWerewolf * CWerewolf::ctor(CWerewolf * this_ptr);

// Original: core_werewolf.cpp_CWerewolf_FUN_005efcc0
// Address: 005efcc0
void CWerewolf(CWerewolf * this_ptr);

// Original: core_werewolf.cpp_CWerewolf_process_FUN_005efde0
// Address: 005efde0
void CWerewolf::process(CWerewolf * this_ptr, float delta_time);

// Original: core_werewolf.cpp_FUN_005f11a0
// Address: 005f11a0
undefined FUN_005f11a0();

// Original: core_werewolf.cpp_FUN_005f11e0
// Address: 005f11e0
undefined FUN_005f11e0();

// Original: core_werewolf.cpp_FUN_005f1230
// Address: 005f1230
undefined FUN_005f1230();

// Original: core_werewolf.cpp_CWerewolf_FUN_005f1820
// Address: 005f1820
int CWerewolf(CWerewolf * this_ptr);

// Original: core_werewolf.cpp_CWerewolf_serialize_FUN_005f1980
// Address: 005f1980
void CWerewolf::serialize(CWerewolf * this_ptr);

// Original: core_werewolf.cpp_FUN_005f1ac0
// Address: 005f1ac0
undefined FUN_005f1ac0();

// Original: core_werewolf.cpp_FUN_005f1db0
// Address: 005f1db0
undefined FUN_005f1db0();

// Original: core_werewolf.cpp_CWerewolf_FUN_005f1df0
// Address: 005f1df0
int CWerewolf(CWerewolf * this_ptr);

// Original: core_werewolf.cpp_FUN_005f1e40
// Address: 005f1e40
undefined FUN_005f1e40();

// Original: core_werewolf.cpp_FUN_005f2010
// Address: 005f2010
undefined FUN_005f2010();

// Original: core_werewolf.cpp_FUN_005f2070
// Address: 005f2070
undefined FUN_005f2070();

// Original: core_werewolf.cpp_CWerewolf_FUN_005f20d0
// Address: 005f20d0
void CWerewolf(CWerewolf * this_ptr);

// Original: core_werewolf.cpp_FUN_005f21e0
// Address: 005f21e0
undefined FUN_005f21e0();

// Original: core_werewolf.cpp_CWerewolf_FUN_005f21f0
// Address: 005f21f0
void CWerewolf(CWerewolf * this_ptr);

// Original: core_werewolf.cpp_CWerewolf_dtor_FUN_005f2220
// Address: 005f2220
CWerewolf * CWerewolf::dtor(CWerewolf * this_ptr, uint d1, uint d2, uint d3, uint d4, uint d5, uint d6, uint d7, uint d8);
