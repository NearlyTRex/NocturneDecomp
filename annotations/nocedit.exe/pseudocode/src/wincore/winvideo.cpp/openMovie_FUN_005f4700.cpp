...

{
  ...
  MVar2 = (*g_mciSendStringAFunc)(local_1b4,(LPSTR)0x0,0,0);
  if (MVar2 != 0) {
  ...
  g_MoviePlaying = 1;
  mcierr = (*g_mciSendStringAFunc)("status mov window handle",local_1b4,0x104,0);
  if (mcierr == 0) {
    g_MovieWindowHandle = atoi(local_1b4);
  }
  else {
  ...
  if ((g_FullscreenMovie == 0) && (g_MoviePlaying != 0)) {
    ...
    (*g_mciSendStringAFunc)("where mov source",(LPSTR)&bStack_b0,0x80,0);
