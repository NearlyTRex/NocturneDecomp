...

{
  ...
  if (header->front_guard != GAME_DEADBEEF) {
    ...
    core_main_c_displayErrorAndQuit_FUN_00506f10
              ("Head memory corruption detected in %d-byte block (" NOCTURNE_FMT_PTR "), allocated %s, line %d, detected at %s line %d.\n(Last ok check was at %s line %d)",iVar2,NOCTURNE_ARG_PTR(header + 1),header->source_file,iVar1,
  ...
  if (back_guard_val != GAME_BEEFDEAD) {
    ...
    core_main_c_displayErrorAndQuit_FUN_00506f10
              ("Tail memory corruption detected in %d-byte block (" NOCTURNE_FMT_PTR "), allocated %s, line %d, detected at %s line %d\n(Last ok check was at %s line %d)",header->num_bytes,NOCTURNE_ARG_PTR(header + 1),header->source_file,
  ...
  do {
    ...
    core_main_c_displayErrorAndQuit_FUN_00506f10
              ("List memory corruption detected in %d-byte block (" NOCTURNE_FMT_PTR "), allocated %s, line %d, detected at %s line %d\n(Last ok check was at %s line %d)",header->num_bytes,NOCTURNE_ARG_PTR(header + 1),header->source_file,
               header->source_line,filename,line_number,g_MemCheckLastFile,g_MemCheckLastLine);
LAB_0050f0dd:
