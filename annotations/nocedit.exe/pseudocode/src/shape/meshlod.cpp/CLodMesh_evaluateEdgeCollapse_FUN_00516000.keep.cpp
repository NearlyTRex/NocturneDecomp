...

{
  ...
  if (pCVar2->collapse_viability < 2) {
    if ((g_LodReplayMode == 0) && (local_48 = 1, 1 < pCVar2->adjacent_tri_count)) {
      do {
        iVar3 = 0;
        if (0 < this_ptr->active_attribute_count) {
          do {
            if ((this_ptr->attribute_enabled_flags[iVar3] != 0) &&
               (this_ptr->tri_data[pCVar2->adjacent_tri_indices[0]].attribute_indices[iVar3] !=
                this_ptr->tri_data[pCVar2->adjacent_tri_indices[local_48]].attribute_indices[iVar3])) {
    ...
    if ((full_check != 0) && (local_44 = 0, 0 < pCVar2->adjacent_tri_count)) {
      do {
        iVar3 = pCVar2->adjacent_tri_indices[local_44];
