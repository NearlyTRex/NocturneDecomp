...

{
  ...
  _sprintf(local_100,"reduce\\cube%04d.txt",INT_03f6bbe4);
  iVar5 = 0;
  ...
  if (0 < g_PolygonCount) {
    iVar2 = 0;
    do {
      iVar5 = iVar5 + g_ModelPolygonData[iVar2].vertex_indices_count - 2;
      iVar2 = iVar2 + 1;
    } while (iVar2 < g_PolygonCount);
  }
  if (file != (_FILE *)0x0) {
  ...
  if (0 < g_VertexCount) {
    do {
      local_58.vertex_data[iVar6].position.x = (double)g_LoadedVertices[iVar6].vertex.x;
      local_58.vertex_data[iVar6].position.y = (double)g_LoadedVertices[iVar6].vertex.y;
      local_58.vertex_data[iVar6].position.z = (double)g_LoadedVertices[iVar6].vertex.z;
      if (file != (_FILE *)0x0) {
        _fprintf(file,"%g %g %g\n",
                   (double)g_LoadedVertices[iVar6].vertex.x,
                   (double)g_LoadedVertices[iVar6].vertex.y,
                   (double)g_LoadedVertices[iVar6].vertex.z);
      }
      iVar6 = iVar6 + 1;
  ...
  if (0 < g_PolygonCount) {
    do {
      local_2c = g_ModelPolygonData[local_30].texture_name;
      shape_superopt_cpp_CPoly_ctor_FUN_005cc620(&local_168,(CObj *)0x0);
      local_168.parent_obj = &local_58;
      local_168.vertex_idx_0 = g_ModelPolygonData[local_30].vertex_indices[0];
      local_168.uv_coords[0].impl.x =
           (double)g_ModelPolygonData[local_30].uv_u[0] * 0.00390625;
      local_168.uv_coords[0].impl.y =
           (double)g_ModelPolygonData[local_30].uv_v[0] * 0.00390625;
      iVar6 = 0;
...
LAB_005d7a1a:
      if (iVar6 < 0) {
        ...
        strcpy(g_TriListTextureNames[g_TriListTextureCount],local_2c);
      ...
      if (2 < (int)g_ModelPolygonData[local_30].vertex_indices_count) {
        do {
          local_168.uv_coords[1].impl.x =
               (double)g_ModelPolygonData[local_30].uv_u[local_18 - 1] * 0.00390625;
          local_168.uv_coords[1].impl.y =
               (double)g_ModelPolygonData[local_30].uv_v[local_18 - 1] * 0.00390625;
          local_168.uv_coords[2].impl.x =
               (double)g_ModelPolygonData[local_30].uv_u[local_18] * 0.00390625;
          local_168.uv_coords[2].impl.y =
               (double)g_ModelPolygonData[local_30].uv_v[local_18] * 0.00390625;
          local_168.vertex_idx_1 = g_ModelPolygonData[local_30].vertex_indices[local_18 - 1];
          local_168.vertex_idx_2 = g_ModelPolygonData[local_30].vertex_indices[local_18];
          if (file != (_FILE *)0x0) {
