...

{
  ...
  do {
    uVar6 = 0;
    if ((this_ptr->base).count != 0) {
      pCVar4 = (this_ptr->base).poly_array;
      do {
        if ((pCVar4->field8_0x60 & 0x8000) == 0) break;
        uVar6 = uVar6 + 1;
        pCVar4 = pCVar4 + 1;
      } while (uVar6 < (uint)(this_ptr->base).count);
    }
    if (uVar6 == (this_ptr->base).count) {
      return 1;
    }
    iVar2 = (**(code **)((int)this_ptr->vtable + 0x38))();
    if (iVar2 == 0) {
      shape_superopt_cpp_logToFile_FUN_005c7910("Unable to add polygon for complex pass",0);
      return 0;
    }
    uVar6 = 0;
    (**(code **)((int)this_ptr->vtable + 0x30))();
    if ((this_ptr->base).count != 0) {
      iVar5 = 0;
      do {
        pCVar4 = (this_ptr->base).poly_array;
