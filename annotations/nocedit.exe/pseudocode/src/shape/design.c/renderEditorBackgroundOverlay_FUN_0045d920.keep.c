...
// Convention: __cdecl
// Signature: void __cdecl shape_design_c_renderEditorBackgroundOverlay_FUN_0045d920(void)
...
{
  SMRGLHeaderPrimitive local_prim;
  int local_18;
  int *local_14;

  if (((g_EditorTextureMode == 1) || (g_EditorTextureMode == 3)) || (g_EditorTextureMode == 4)) {
    if (g_AntiAliasingEnabled == 0) {
      local_prim.base.count = -g_EditorBackgroundColors[g_EditorCurrentTexture];
      engine_3d_c_processPolygonColor_FUN_00403e30(&local_prim);
    }
    else {
