...

{
  ...
  uVar1 = (int)ROUND(ROUND((float)scale_factor * (polygon1_ptr->normal).x)) -
          (int)ROUND(ROUND((float)scale_factor * (polygon2_ptr->normal).x));
  uVar2 = (int)uVar1 >> 0x1f;
  if ((((int)((uVar1 ^ uVar2) - uVar2) < tolerance) &&
      (uVar3 = (int)ROUND(ROUND((float)scale_factor * (polygon1_ptr->normal).y)) -
               (int)ROUND(ROUND((float)scale_factor * (polygon2_ptr->normal).y)),
      uVar4 = (int)uVar3 >> 0x1f, (int)((uVar3 ^ uVar4) - uVar4) < tolerance)) &&
     (uVar3 = (int)ROUND(ROUND((float)scale_factor * (polygon1_ptr->normal).z)) -
              (int)ROUND(ROUND((float)scale_factor * (polygon2_ptr->normal).z)),
     uVar4 = (int)uVar3 >> 0x1f, (int)((uVar3 ^ uVar4) - uVar4) < tolerance)) {
    local_2c = 1;
  }
  else {
