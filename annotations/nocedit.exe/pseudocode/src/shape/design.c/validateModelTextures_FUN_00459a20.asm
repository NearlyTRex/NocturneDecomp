; *****************************************************************************
;                               FUNCTION
; *****************************************************************************
; __cdecl int shape_design.c_validateModelTextures_FUN_00459a20(char * directory_path, int mode)
;
; Parameters:
; char *           Stack[0x4]:4   directory_path
; int              Stack[0x8]:4   mode
; Local Variables:
; undefined4       Stack[-0x3fd0]:4  local_3fd0
; undefined1       Stack[-0x3fcc]:1  local_3fcc
; undefined1       Stack[-0x14c]:1  local_14c
; undefined1       Stack[-0x84]:1  local_84
; undefined1       Stack[-0x83]:1  local_83
; undefined4       Stack[-0x34]:4  local_34
; undefined4       Stack[-0x30]:4  local_30
; undefined4       Stack[-0x2c]:4  local_2c
; undefined4       Stack[-0x28]:4  local_28
; undefined4       Stack[-0x24]:4  local_24
; undefined4       Stack[-0x20]:4  local_20
; undefined4       Stack[-0x1c]:4  local_1c
; undefined4       Stack[-0x18]:4  local_18
; undefined4       Stack[-0x14]:4  local_14
;
; XREF[2]:
;   shape_design.c_handleEditorHotkeys_FUN_0045b990 at 0045b9cd
;   shape_design.c_tileTextureMaps_FUN_0046a180 at 0046a1e9
;
; Referenced Globals:
;   TerminatedCString s_Checking_texture_files_0061acc5
;   TerminatedCString s_TGA_0061acdf
;   undefined4 DAT_0061ace0
;   undefined4 DAT_0061ace1
;   undefined4 s_A_0061ace2
;   TerminatedCString s_WARNING_The_following_te_0061ace4
;   TerminatedCString s_NO_TEXTURE_object_not_ma_0061ad12
;   TerminatedCString s_s_12s_texture_not_found_0061ad36
;   TerminatedCString s_s_12s_invalid_texture_si_0061ad53
;   TerminatedCString s_Hit_a_key_0061ad73
;   int g_PolygonCount
;   SShapeEditorPolygon[20000] g_ModelPolygonData
;   undefined4 DAT_016e9914
;   undefined4 DAT_016e9915
;   undefined4 DAT_016e9916
;   ... and 1 more
;
; Called Functions:
;   crt_stdio.c_sprintf_FUN_005fdbd0
;   crt_string.c_strcmp_FUN_005fef20
;   engine_2d.c_drawText_FUN_00401fd0
;   engine_dosio.c_getFileSize_FUN_00481880
;   wincore_wddvmem.cpp_swapBuffers_FUN_005eda20
;   wincore_windll.cpp_clearScreen_FUN_005b3e70
;   wincore_winrun.cpp_getNextKeypress_FUN_005f2e90
;
; *****************************************************************************

section .text

    PUSH EBX                            ; 00459a20
        ;   Label: shape_design.c_validateModelTextures_FUN_00459a20
    PUSH ESI                            ; 00459a21
    PUSH EDI                            ; 00459a22
    PUSH EBP                            ; 00459a23
    MOV EBP,ESP                         ; 00459a24
    SUB ESP,0x3fc0                      ; 00459a26
    CMP dword ptr [EBP + 0x18],0x0      ; 00459a2c
    JZ 0x00459a38                       ; 00459a30 | LAB_00459a38
        ;   XREF to: 00459a38 (CONDITIONAL_JUMP)
    CMP dword ptr [EBP + 0x18],0x1      ; 00459a32
    JNZ 0x00459a3a                      ; 00459a36 | LAB_00459a3a
        ;   XREF to: 00459a3a (CONDITIONAL_JUMP)
    JMP 0x00459a46                      ; 00459a38 | LAB_00459a46
        ;   Label: LAB_00459a38
        ;   XREF to: 00459a46 (UNCONDITIONAL_JUMP)
    MOV dword ptr [EBP + -0x10],0xffffffff ; 00459a3a
        ;   Label: LAB_00459a3a
    JMP 0x00459dd0                      ; 00459a41 | LAB_00459dd0
        ;   XREF to: 00459dd0 (UNCONDITIONAL_JUMP)
    CALL wincore_windll.cpp_clearScreen_FUN_005b3e70 ; 00459a46 | void wincore_windll.cpp_clearScreen_FUN_005b3e70()
        ;   Label: LAB_00459a46
        ;   XREF to: 005b3e70 (UNCONDITIONAL_CALL)
    PUSH 0x0                            ; 00459a4b
    PUSH 0x0                            ; 00459a4d
    MOV EAX,0x61acc5                    ; 00459a4f | = "Checking texture files..." | s_Checking_texture_files_0061acc5 = Checking texture files...
    PUSH EAX                            ; 00459a54 | = "Checking texture files..." | s_Checking_texture_files_0061acc5 = Checking texture files...
    CALL engine_2d.c_drawText_FUN_00401fd0 ; 00459a55 | void engine_2d.c_drawText_FUN_00401fd0(char * text, int x_pos, int y_pos)
        ;   XREF to: 00401fd0 (UNCONDITIONAL_CALL)
    ADD ESP,0xc                         ; 00459a5a
    CALL wincore_wddvmem.cpp_swapBuffers_FUN_005eda20 ; 00459a5d | void wincore_wddvmem.cpp_swapBuffers_FUN_005eda20()
        ;   XREF to: 005eda20 (UNCONDITIONAL_CALL)
    MOV dword ptr [EBP + -0x4],0x0      ; 00459a62
    MOV dword ptr [EBP + -0xc],0x0      ; 00459a69
    JMP 0x00459a78                      ; 00459a70 | LAB_00459a78
        ;   XREF to: 00459a78 (UNCONDITIONAL_JUMP)
    MOV EAX,dword ptr [EBP + -0xc]      ; 00459a72
        ;   Label: LAB_00459a72
    INC dword ptr [EBP + -0xc]          ; 00459a75
    CMP dword ptr [EBP + -0xc],0xc8     ; 00459a78
        ;   Label: LAB_00459a78
    JGE 0x00459a8f                      ; 00459a7f | LAB_00459a8f
        ;   XREF to: 00459a8f (CONDITIONAL_JUMP)
    IMUL EAX,dword ptr [EBP + -0xc],0x50 ; 00459a81
    MOV byte ptr [EAX + EBP*0x1 + 0xffffc044],0x0 ; 00459a85
    JMP 0x00459a72                      ; 00459a8d | LAB_00459a72
        ;   XREF to: 00459a72 (UNCONDITIONAL_JUMP)
    MOV dword ptr [EBP + -0xc],0x0      ; 00459a8f
        ;   Label: LAB_00459a8f
    JMP 0x00459a9e                      ; 00459a96 | LAB_00459a9e
        ;   XREF to: 00459a9e (UNCONDITIONAL_JUMP)
    MOV EAX,dword ptr [EBP + -0xc]      ; 00459a98
        ;   Label: LAB_00459a98
    INC dword ptr [EBP + -0xc]          ; 00459a9b
    MOV EAX,dword ptr [EBP + -0xc]      ; 00459a9e
        ;   Label: LAB_00459a9e
    CMP EAX,dword ptr [0x016e990c]      ; 00459aa1 | int g_PolygonCount
    JGE 0x00459da0                      ; 00459aa7 | LAB_00459da0
        ;   XREF to: 00459da0 (CONDITIONAL_JUMP)
    IMUL EDI,dword ptr [EBP + -0xc],0x184 ; 00459aad
    MOV ESI,0x16e9910                   ; 00459ab4 | SShapeEditorPolygon[20000] g_ModelPolygonData
    ADD ESI,EDI                         ; 00459ab9
    ADD ESI,0x4                         ; 00459abb
    LEA EDI,[EBP + -0x74]               ; 00459abe
    PUSH EDI                            ; 00459ac1
    MOV AL,byte ptr [ESI]               ; 00459ac2 | DAT_016e9914
        ;   Label: LAB_00459ac2
    MOV byte ptr [EDI],AL               ; 00459ac4
    CMP AL,0x0                          ; 00459ac6
    JZ 0x00459ada                       ; 00459ac8 | LAB_00459ada
        ;   XREF to: 00459ada (CONDITIONAL_JUMP)
    MOV AL,byte ptr [ESI + 0x1]         ; 00459aca | DAT_016e9915
    ADD ESI,0x2                         ; 00459acd
    MOV byte ptr [EDI + 0x1],AL         ; 00459ad0
    ADD EDI,0x2                         ; 00459ad3
    CMP AL,0x0                          ; 00459ad6
    JNZ 0x00459ac2                      ; 00459ad8 | LAB_00459ac2
        ;   XREF to: 00459ac2 (CONDITIONAL_JUMP)
    POP EDI                             ; 00459ada
        ;   Label: LAB_00459ada
    CMP byte ptr [EBP + -0x74],0x0      ; 00459adb
    JNZ 0x00459aea                      ; 00459adf | LAB_00459aea
        ;   XREF to: 00459aea (CONDITIONAL_JUMP)
    MOV dword ptr [EBP + -0x18],0x1     ; 00459ae1
    JMP 0x00459af1                      ; 00459ae8 | LAB_00459af1
        ;   XREF to: 00459af1 (UNCONDITIONAL_JUMP)
    MOV dword ptr [EBP + -0x18],0x0     ; 00459aea
        ;   Label: LAB_00459aea
    CMP dword ptr [EBP + 0x18],0x1      ; 00459af1
        ;   Label: LAB_00459af1
    JNZ 0x00459b42                      ; 00459af5 | LAB_00459b42
        ;   XREF to: 00459b42 (CONDITIONAL_JUMP)
    MOV EDX,0x2e                        ; 00459af7
    LEA ESI,[EBP + -0x74]               ; 00459afc
    MOV AL,byte ptr [ESI]               ; 00459aff
        ;   Label: LAB_00459aff
    CMP AL,DL                           ; 00459b01
    JZ 0x00459b17                       ; 00459b03 | LAB_00459b17
        ;   XREF to: 00459b17 (CONDITIONAL_JUMP)
    CMP AL,0x0                          ; 00459b05
    JZ 0x00459b15                       ; 00459b07 | LAB_00459b15
        ;   XREF to: 00459b15 (CONDITIONAL_JUMP)
    INC ESI                             ; 00459b09
    MOV AL,byte ptr [ESI]               ; 00459b0a
    CMP AL,DL                           ; 00459b0c
    JZ 0x00459b17                       ; 00459b0e | LAB_00459b17
        ;   XREF to: 00459b17 (CONDITIONAL_JUMP)
    INC ESI                             ; 00459b10
    CMP AL,0x0                          ; 00459b11
    JNZ 0x00459aff                      ; 00459b13 | LAB_00459aff
        ;   XREF to: 00459aff (CONDITIONAL_JUMP)
    SUB ESI,ESI                         ; 00459b15
        ;   Label: LAB_00459b15
    MOV dword ptr [EBP + -0x8],ESI      ; 00459b17
        ;   Label: LAB_00459b17
    CMP dword ptr [EBP + -0x8],0x0      ; 00459b1a
    JZ 0x00459b42                       ; 00459b1e | LAB_00459b42
        ;   XREF to: 00459b42 (CONDITIONAL_JUMP)
    MOV ESI,0x61acdf                    ; 00459b20 | = ".TGA" | s_TGA_0061acdf = .TGA
    MOV EDI,dword ptr [EBP + -0x8]      ; 00459b25
    PUSH EDI                            ; 00459b28
    MOV AL,byte ptr [ESI]               ; 00459b29 | = ".TGA" | s_TGA_0061acdf = .TGA
        ;   Label: LAB_00459b29
    MOV byte ptr [EDI],AL               ; 00459b2b
    CMP AL,0x0                          ; 00459b2d
    JZ 0x00459b41                       ; 00459b2f | LAB_00459b41
        ;   XREF to: 00459b41 (CONDITIONAL_JUMP)
    MOV AL,byte ptr [ESI + 0x1]         ; 00459b31 | DAT_0061ace0
    ADD ESI,0x2                         ; 00459b34
    MOV byte ptr [EDI + 0x1],AL         ; 00459b37
    ADD EDI,0x2                         ; 00459b3a
    CMP AL,0x0                          ; 00459b3d
    JNZ 0x00459b29                      ; 00459b3f | LAB_00459b29
        ;   XREF to: 00459b29 (CONDITIONAL_JUMP)
    POP EDI                             ; 00459b41
        ;   Label: LAB_00459b41
    LEA EAX,[EBP + -0x74]               ; 00459b42
        ;   Label: LAB_00459b42
    PUSH EAX                            ; 00459b45
    MOV EAX,dword ptr [EBP + 0x14]      ; 00459b46
    PUSH EAX                            ; 00459b49
    CALL engine_dosio.c_getFileSize_FUN_00481880 ; 00459b4a | int engine_dosio.c_getFileSize_FUN_00481880(char * directory, char * filename)
        ;   XREF to: 00481880 (UNCONDITIONAL_CALL)
    ADD ESP,0x8                         ; 00459b4f
    MOV dword ptr [EBP + -0x20],EAX     ; 00459b52
    MOV dword ptr [EBP + -0x1c],0x0     ; 00459b55
    CMP dword ptr [EBP + 0x18],0x0      ; 00459b5c
    JNZ 0x00459c28                      ; 00459b60 | LAB_00459c28
        ;   XREF to: 00459c28 (CONDITIONAL_JUMP)
    MOV EAX,dword ptr [EBP + -0x20]     ; 00459b66
    MOV dword ptr [EBP + 0xffffc040],EAX ; 00459b69
    JMP 0x00459ba0                      ; 00459b6f | LAB_00459ba0
        ;   XREF to: 00459ba0 (UNCONDITIONAL_JUMP)
    JMP 0x00459c28                      ; 00459b71 | LAB_00459c28
        ;   Label: LAB_00459b71
        ;   XREF to: 00459c28 (UNCONDITIONAL_JUMP)
    JMP 0x00459c28                      ; 00459b76 | LAB_00459c28
        ;   Label: LAB_00459b76
        ;   XREF to: 00459c28 (UNCONDITIONAL_JUMP)
    JMP 0x00459c28                      ; 00459b7b | LAB_00459c28
        ;   Label: LAB_00459b7b
        ;   XREF to: 00459c28 (UNCONDITIONAL_JUMP)
    JMP 0x00459c28                      ; 00459b80 | LAB_00459c28
        ;   Label: LAB_00459b80
        ;   XREF to: 00459c28 (UNCONDITIONAL_JUMP)
    JMP 0x00459c28                      ; 00459b85 | LAB_00459c28
        ;   Label: LAB_00459b85
        ;   XREF to: 00459c28 (UNCONDITIONAL_JUMP)
    JMP 0x00459c28                      ; 00459b8a | LAB_00459c28
        ;   Label: LAB_00459b8a
        ;   XREF to: 00459c28 (UNCONDITIONAL_JUMP)
    JMP 0x00459c28                      ; 00459b8f | LAB_00459c28
        ;   Label: LAB_00459b8f
        ;   XREF to: 00459c28 (UNCONDITIONAL_JUMP)
    MOV dword ptr [EBP + -0x1c],0x1     ; 00459b94
        ;   Label: LAB_00459b94
    JMP 0x00459c28                      ; 00459b9b | LAB_00459c28
        ;   XREF to: 00459c28 (UNCONDITIONAL_JUMP)
    CMP dword ptr [EBP + 0xffffc040],0x4000 ; 00459ba0
        ;   Label: LAB_00459ba0
    JC 0x00459be9                       ; 00459baa | LAB_00459be9
        ;   XREF to: 00459be9 (CONDITIONAL_JUMP)
    CMP dword ptr [EBP + 0xffffc040],0x4000 ; 00459bac
    JBE 0x00459b7b                      ; 00459bb6 | LAB_00459b7b
        ;   XREF to: 00459b7b (CONDITIONAL_JUMP)
    CMP dword ptr [EBP + 0xffffc040],0x40000 ; 00459bb8
    JC 0x00459bdb                       ; 00459bc2 | LAB_00459bdb
        ;   XREF to: 00459bdb (CONDITIONAL_JUMP)
    CMP dword ptr [EBP + 0xffffc040],0x40000 ; 00459bc4
    JBE 0x00459b71                      ; 00459bce | LAB_00459b71
        ;   XREF to: 00459b71 (CONDITIONAL_JUMP)
    CMP dword ptr [EBP + 0xffffc040],-0x1 ; 00459bd0
    JZ 0x00459b8f                       ; 00459bd7 | LAB_00459b8f
        ;   XREF to: 00459b8f (CONDITIONAL_JUMP)
    JMP 0x00459b94                      ; 00459bd9 | LAB_00459b94
        ;   XREF to: 00459b94 (UNCONDITIONAL_JUMP)
    CMP dword ptr [EBP + 0xffffc040],0x10000 ; 00459bdb
        ;   Label: LAB_00459bdb
    JZ 0x00459b76                       ; 00459be5 | LAB_00459b76
        ;   XREF to: 00459b76 (CONDITIONAL_JUMP)
    JMP 0x00459b94                      ; 00459be7 | LAB_00459b94
        ;   XREF to: 00459b94 (UNCONDITIONAL_JUMP)
    CMP dword ptr [EBP + 0xffffc040],0x400 ; 00459be9
        ;   Label: LAB_00459be9
    JC 0x00459c13                       ; 00459bf3 | LAB_00459c13
        ;   XREF to: 00459c13 (CONDITIONAL_JUMP)
    CMP dword ptr [EBP + 0xffffc040],0x400 ; 00459bf5
    JBE 0x00459b85                      ; 00459bff | LAB_00459b85
        ;   XREF to: 00459b85 (CONDITIONAL_JUMP)
    CMP dword ptr [EBP + 0xffffc040],0x1000 ; 00459c01
    JZ 0x00459b80                       ; 00459c0b | LAB_00459b80
        ;   XREF to: 00459b80 (CONDITIONAL_JUMP)
    JMP 0x00459b94                      ; 00459c11 | LAB_00459b94
        ;   XREF to: 00459b94 (UNCONDITIONAL_JUMP)
    CMP dword ptr [EBP + 0xffffc040],0x100 ; 00459c13
        ;   Label: LAB_00459c13
    JZ 0x00459b8a                       ; 00459c1d | LAB_00459b8a
        ;   XREF to: 00459b8a (CONDITIONAL_JUMP)
    JMP 0x00459b94                      ; 00459c23 | LAB_00459b94
        ;   XREF to: 00459b94 (UNCONDITIONAL_JUMP)
    CMP dword ptr [EBP + -0x20],-0x1    ; 00459c28
        ;   Label: LAB_00459c28
    JZ 0x00459c38                       ; 00459c2c | LAB_00459c38
        ;   XREF to: 00459c38 (CONDITIONAL_JUMP)
    CMP dword ptr [EBP + -0x1c],0x0     ; 00459c2e
    JZ 0x00459d9b                       ; 00459c32 | LAB_00459d9b
        ;   XREF to: 00459d9b (CONDITIONAL_JUMP)
    MOV dword ptr [EBP + -0x14],0x0     ; 00459c38
        ;   Label: LAB_00459c38
    MOV dword ptr [EBP + -0x24],0x0     ; 00459c3f
    JMP 0x00459c4e                      ; 00459c46 | LAB_00459c4e
        ;   XREF to: 00459c4e (UNCONDITIONAL_JUMP)
    MOV EAX,dword ptr [EBP + -0x24]     ; 00459c48
        ;   Label: LAB_00459c48
    INC dword ptr [EBP + -0x24]         ; 00459c4b
    MOV EAX,dword ptr [EBP + -0x24]     ; 00459c4e
        ;   Label: LAB_00459c4e
    CMP EAX,dword ptr [EBP + -0x4]      ; 00459c51
    JGE 0x00459c7e                      ; 00459c54 | LAB_00459c7e
        ;   XREF to: 00459c7e (CONDITIONAL_JUMP)
    IMUL EDX,dword ptr [EBP + -0x24],0x50 ; 00459c56
    LEA EAX,[EBP + 0xffffc044]          ; 00459c5a
    ADD EAX,EDX                         ; 00459c60
    PUSH EAX                            ; 00459c62
    LEA EAX,[EBP + -0x74]               ; 00459c63
    PUSH EAX                            ; 00459c66
    CALL crt_string.c_strcmp_FUN_005fef20 ; 00459c67 | int crt_string.c_strcmp_FUN_005fef20(char * str1, char * str2)
        ;   XREF to: 005fef20 (UNCONDITIONAL_CALL)
    ADD ESP,0x8                         ; 00459c6c
    TEST EAX,EAX                        ; 00459c6f
    JNZ 0x00459c7c                      ; 00459c71 | LAB_00459c7c
        ;   XREF to: 00459c7c (CONDITIONAL_JUMP)
    MOV dword ptr [EBP + -0x14],0x1     ; 00459c73
    JMP 0x00459c7e                      ; 00459c7a | LAB_00459c7e
        ;   XREF to: 00459c7e (UNCONDITIONAL_JUMP)
    JMP 0x00459c48                      ; 00459c7c | LAB_00459c48
        ;   Label: LAB_00459c7c
        ;   XREF to: 00459c48 (UNCONDITIONAL_JUMP)
    CMP dword ptr [EBP + -0x14],0x0     ; 00459c7e
        ;   Label: LAB_00459c7e
    JZ 0x00459c8e                       ; 00459c82 | LAB_00459c8e
        ;   XREF to: 00459c8e (CONDITIONAL_JUMP)
    CMP dword ptr [EBP + -0x18],0x0     ; 00459c84
    JZ 0x00459d8d                       ; 00459c88 | LAB_00459d8d
        ;   XREF to: 00459d8d (CONDITIONAL_JUMP)
    CMP dword ptr [EBP + -0x4],0xc8     ; 00459c8e
        ;   Label: LAB_00459c8e
    JGE 0x00459c9d                      ; 00459c95 | LAB_00459c9d
        ;   XREF to: 00459c9d (CONDITIONAL_JUMP)
    CMP dword ptr [EBP + -0x18],0x0     ; 00459c97
    JZ 0x00459c9f                       ; 00459c9b | LAB_00459c9f
        ;   XREF to: 00459c9f (CONDITIONAL_JUMP)
    JMP 0x00459cc8                      ; 00459c9d | LAB_00459cc8
        ;   Label: LAB_00459c9d
        ;   XREF to: 00459cc8 (UNCONDITIONAL_JUMP)
    LEA ESI,[EBP + -0x74]               ; 00459c9f
        ;   Label: LAB_00459c9f
    IMUL EDI,dword ptr [EBP + -0x4],0x50 ; 00459ca2
    LEA EAX,[EBP + 0xffffc044]          ; 00459ca6
    ADD EDI,EAX                         ; 00459cac
    PUSH EDI                            ; 00459cae
    MOV AL,byte ptr [ESI]               ; 00459caf
        ;   Label: LAB_00459caf
    MOV byte ptr [EDI],AL               ; 00459cb1
    CMP AL,0x0                          ; 00459cb3
    JZ 0x00459cc7                       ; 00459cb5 | LAB_00459cc7
        ;   XREF to: 00459cc7 (CONDITIONAL_JUMP)
    MOV AL,byte ptr [ESI + 0x1]         ; 00459cb7
    ADD ESI,0x2                         ; 00459cba
    MOV byte ptr [EDI + 0x1],AL         ; 00459cbd
    ADD EDI,0x2                         ; 00459cc0
    CMP AL,0x0                          ; 00459cc3
    JNZ 0x00459caf                      ; 00459cc5 | LAB_00459caf
        ;   XREF to: 00459caf (CONDITIONAL_JUMP)
    POP EDI                             ; 00459cc7
        ;   Label: LAB_00459cc7
    MOV EAX,dword ptr [EBP + -0x4]      ; 00459cc8
        ;   Label: LAB_00459cc8
    INC dword ptr [EBP + -0x4]          ; 00459ccb
    CMP dword ptr [EBP + -0x4],0x1      ; 00459cce
    JNZ 0x00459ceb                      ; 00459cd2 | LAB_00459ceb
        ;   XREF to: 00459ceb (CONDITIONAL_JUMP)
    CALL wincore_windll.cpp_clearScreen_FUN_005b3e70 ; 00459cd4 | void wincore_windll.cpp_clearScreen_FUN_005b3e70()
        ;   XREF to: 005b3e70 (UNCONDITIONAL_CALL)
    PUSH 0x0                            ; 00459cd9
    PUSH 0x0                            ; 00459cdb
    MOV EAX,0x61ace4                    ; 00459cdd | = "WARNING: The following textures were ..." | s_WARNING_The_following_te_0061ace4 = WARNING: The following textures were invalid:
    PUSH EAX                            ; 00459ce2 | = "WARNING: The following textures were ..." | s_WARNING_The_following_te_0061ace4 = WARNING: The following textures were invalid:
    CALL engine_2d.c_drawText_FUN_00401fd0 ; 00459ce3 | void engine_2d.c_drawText_FUN_00401fd0(char * text, int x_pos, int y_pos)
        ;   XREF to: 00401fd0 (UNCONDITIONAL_CALL)
    ADD ESP,0xc                         ; 00459ce8
    CMP dword ptr [EBP + -0x18],0x0     ; 00459ceb
        ;   Label: LAB_00459ceb
    JZ 0x00459d21                       ; 00459cef | LAB_00459d21
        ;   XREF to: 00459d21 (CONDITIONAL_JUMP)
    MOV EAX,0x61ad12                    ; 00459cf1 | = "  NO TEXTURE    (object not mapped)" | s_NO_TEXTURE_object_not_ma_0061ad12 =   NO TEXTURE    (object not mapped)
    PUSH EAX                            ; 00459cf6 | = "  NO TEXTURE    (object not mapped)" | s_NO_TEXTURE_object_not_ma_0061ad12 =   NO TEXTURE    (object not mapped)
    LEA EAX,[EBP + 0xfffffec4]          ; 00459cf7
    PUSH EAX                            ; 00459cfd
    CALL crt_stdio.c_sprintf_FUN_005fdbd0 ; 00459cfe | int crt_stdio.c_sprintf_FUN_005fdbd0(char * buffer, char * format)
        ;   XREF to: 005fdbd0 (UNCONDITIONAL_CALL)
    ADD ESP,0x8                         ; 00459d03
    MOV EAX,dword ptr [EBP + -0x4]      ; 00459d06
    INC EAX                             ; 00459d09
    IMUL EAX,EAX,0xb                    ; 00459d0a
    PUSH EAX                            ; 00459d0d
    PUSH 0x0                            ; 00459d0e
    LEA EAX,[EBP + 0xfffffec4]          ; 00459d10
    PUSH EAX                            ; 00459d16
    CALL engine_2d.c_drawText_FUN_00401fd0 ; 00459d17 | void engine_2d.c_drawText_FUN_00401fd0(char * text, int x_pos, int y_pos)
        ;   XREF to: 00401fd0 (UNCONDITIONAL_CALL)
    ADD ESP,0xc                         ; 00459d1c
    JMP 0x00459d8d                      ; 00459d1f | LAB_00459d8d
        ;   XREF to: 00459d8d (UNCONDITIONAL_JUMP)
    CMP dword ptr [EBP + -0x20],-0x1    ; 00459d21
        ;   Label: LAB_00459d21
    JNZ 0x00459d5b                      ; 00459d25 | LAB_00459d5b
        ;   XREF to: 00459d5b (CONDITIONAL_JUMP)
    LEA EAX,[EBP + -0x74]               ; 00459d27
    PUSH EAX                            ; 00459d2a
    MOV EAX,0x61ad36                    ; 00459d2b | = "  %-12s  (texture not found)" | s_s_12s_texture_not_found_0061ad36 =   %-12s  (texture not found)
    PUSH EAX                            ; 00459d30 | = "  %-12s  (texture not found)" | s_s_12s_texture_not_found_0061ad36 =   %-12s  (texture not found)
    LEA EAX,[EBP + 0xfffffec4]          ; 00459d31
    PUSH EAX                            ; 00459d37
    CALL crt_stdio.c_sprintf_FUN_005fdbd0 ; 00459d38 | int crt_stdio.c_sprintf_FUN_005fdbd0(char * buffer, char * format)
        ;   XREF to: 005fdbd0 (UNCONDITIONAL_CALL)
    ADD ESP,0xc                         ; 00459d3d
    MOV EAX,dword ptr [EBP + -0x4]      ; 00459d40
    INC EAX                             ; 00459d43
    IMUL EAX,EAX,0xb                    ; 00459d44
    PUSH EAX                            ; 00459d47
    PUSH 0x0                            ; 00459d48
    LEA EAX,[EBP + 0xfffffec4]          ; 00459d4a
    PUSH EAX                            ; 00459d50
    CALL engine_2d.c_drawText_FUN_00401fd0 ; 00459d51 | void engine_2d.c_drawText_FUN_00401fd0(char * text, int x_pos, int y_pos)
        ;   XREF to: 00401fd0 (UNCONDITIONAL_CALL)
    ADD ESP,0xc                         ; 00459d56
    JMP 0x00459d8d                      ; 00459d59 | LAB_00459d8d
        ;   XREF to: 00459d8d (UNCONDITIONAL_JUMP)
    LEA EAX,[EBP + -0x74]               ; 00459d5b
        ;   Label: LAB_00459d5b
    PUSH EAX                            ; 00459d5e
    MOV EAX,0x61ad53                    ; 00459d5f | = "  %-12s  (invalid texture size)" | s_s_12s_invalid_texture_si_0061ad53 =   %-12s  (invalid texture size)
    PUSH EAX                            ; 00459d64 | = "  %-12s  (invalid texture size)" | s_s_12s_invalid_texture_si_0061ad53 =   %-12s  (invalid texture size)
    LEA EAX,[EBP + 0xfffffec4]          ; 00459d65
    PUSH EAX                            ; 00459d6b
    CALL crt_stdio.c_sprintf_FUN_005fdbd0 ; 00459d6c | int crt_stdio.c_sprintf_FUN_005fdbd0(char * buffer, char * format)
        ;   XREF to: 005fdbd0 (UNCONDITIONAL_CALL)
    ADD ESP,0xc                         ; 00459d71
    MOV EAX,dword ptr [EBP + -0x4]      ; 00459d74
    INC EAX                             ; 00459d77
    IMUL EAX,EAX,0xb                    ; 00459d78
    PUSH EAX                            ; 00459d7b
    PUSH 0x0                            ; 00459d7c
    LEA EAX,[EBP + 0xfffffec4]          ; 00459d7e
    PUSH EAX                            ; 00459d84
    CALL engine_2d.c_drawText_FUN_00401fd0 ; 00459d85 | void engine_2d.c_drawText_FUN_00401fd0(char * text, int x_pos, int y_pos)
        ;   XREF to: 00401fd0 (UNCONDITIONAL_CALL)
    ADD ESP,0xc                         ; 00459d8a
    IMUL EAX,dword ptr [EBP + -0xc],0x184 ; 00459d8d
        ;   Label: LAB_00459d8d
    MOV byte ptr [EAX + 0x16e9914],0x0  ; 00459d94 | DAT_016e9914
    JMP 0x00459a98                      ; 00459d9b | LAB_00459a98
        ;   Label: LAB_00459d9b
        ;   XREF to: 00459a98 (UNCONDITIONAL_JUMP)
    CMP dword ptr [EBP + -0x4],0x0      ; 00459da0
        ;   Label: LAB_00459da0
    JZ 0x00459dca                       ; 00459da4 | LAB_00459dca
        ;   XREF to: 00459dca (CONDITIONAL_JUMP)
    MOV EAX,dword ptr [EBP + -0x4]      ; 00459da6
    ADD EAX,0x3                         ; 00459da9
    IMUL EAX,EAX,0xb                    ; 00459dac
    PUSH EAX                            ; 00459daf
    PUSH 0x0                            ; 00459db0
    MOV EAX,0x61ad73                    ; 00459db2 | = "Hit a key..." | s_Hit_a_key_0061ad73 = Hit a key...
    PUSH EAX                            ; 00459db7 | = "Hit a key..." | s_Hit_a_key_0061ad73 = Hit a key...
    CALL engine_2d.c_drawText_FUN_00401fd0 ; 00459db8 | void engine_2d.c_drawText_FUN_00401fd0(char * text, int x_pos, int y_pos)
        ;   XREF to: 00401fd0 (UNCONDITIONAL_CALL)
    ADD ESP,0xc                         ; 00459dbd
    CALL wincore_wddvmem.cpp_swapBuffers_FUN_005eda20 ; 00459dc0 | void wincore_wddvmem.cpp_swapBuffers_FUN_005eda20()
        ;   XREF to: 005eda20 (UNCONDITIONAL_CALL)
    CALL wincore_winrun.cpp_getNextKeypress_FUN_005f2e90 ; 00459dc5 | int wincore_winrun.cpp_getNextKeypress_FUN_005f2e90()
        ;   XREF to: 005f2e90 (UNCONDITIONAL_CALL)
    MOV EAX,dword ptr [EBP + -0x4]      ; 00459dca
        ;   Label: LAB_00459dca
    MOV dword ptr [EBP + -0x10],EAX     ; 00459dcd
    MOV EAX,dword ptr [EBP + -0x10]     ; 00459dd0
        ;   Label: LAB_00459dd0
    MOV ESP,EBP                         ; 00459dd3
    POP EBP                             ; 00459dd5
    POP EDI                             ; 00459dd6
    POP ESI                             ; 00459dd7
    POP EBX                             ; 00459dd8
    RET                                 ; 00459dd9

