...
// Convention: __cdecl
// Signature: istrstream * __cdecl crt_strstream_cpp_istrstream_dtor_FUN_005ff5ca(istrstream *this_ptr,uint flags)
  ...
  if ((flags & 4) == 0) {
    ...
    *(WatcomThunkedDestructorDirect **)(this_ptr->padding + pWVar2->offset_to_vbase + -0x2c) =
    ...
    piVar3 = crt_iostream_cpp_istream_dtor_FUN_006063e1
                       ((istream *)&(this_ptr->_istream_core).__last_read_length,1);
    this_ptr = (istrstream *)
               crt_strstream_cpp_strstreambase_dtor_FUN_006062a6
                         ((strstreambase *)&piVar3[-2]._ios.__fill_character,1);
    if ((flags & 1) == 0) {
