...
// Convention: __cdecl
// Signature: char * __cdecl crt_stdio_c_parse_format_spec_FUN_00604c1c(char *format_ptr,scanf_state_t *state)
...

{
  ...
  if (bVar1 < 0x68) {
    if (0x48 < bVar1) {
      if (bVar1 < 0x4a) {
        if ((format_ptr[1] == 0x36) && (format_ptr[2] == 0x34)) {
          format_ptr = format_ptr + 3;
