...
// Convention: __cdecl
// Signature: int __cdecl crt_stdio_c_fgetc_outptr_FUN_005ff245(_FILE *stream,uchar *output_byte)

#include "nocturne.h"

int __cdecl fgetc_outptr(_FILE *stream,uchar *output_byte)

{
  ...
  if (((*(int *)((int)&stream->_handle + *(int *)(stream->_ptr + 4)) != 0) ||
     ...
     (iVar3 = stream_setup(stream,1), iVar3 == 0)) {
    stream->_cnt = 0;
    return (int)stream;
  }
  ...
  fgetcInternal(stream,output_byte);
  return (int)stream;
}
