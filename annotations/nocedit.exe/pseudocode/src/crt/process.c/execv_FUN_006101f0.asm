...
section .text
    ...
    PUSH ECX                            ; 006101ff
    PUSH 0x1                            ; 00610200
    CALL crt_process.c_spawnvp_FUN_0060f39c ; 00610202 | int crt_process.c_spawnvp_FUN_0060f39c(int mode, char * cmdname, char * * argv)
        ;   XREF to: 0060f39c (UNCONDITIONAL_CALL)
    ADD ESP,0x10                        ; 00610207
    MOV EBX,EAX                         ; 0061020a
    CALL dword ptr [0x00684ee4]         ; 0061020c | GET_TLS_FUNC * PTR_crt_thread.c_GetTLS_FUN_0060242c_00684ee4
    CMP dword ptr [EAX + 0x4],0x0       ; 00610212
    JZ 0x0061021f                       ; 00610216 | LAB_0061021f
        ;   XREF to: 0061021f (CONDITIONAL_JUMP)
    MOV EAX,0xffffffff                  ; 00610218
    POP EBX                             ; 0061021d
    RET                                 ; 0061021e
    CALL dword ptr [0x00685478]         ; 0061021f | void * PTR_crt_exit.c_ExitHookStub_FUN_0060b530_00685478
        ;   Label: LAB_0061021f
    PUSH EBX                            ; 00610225
    CALL dword ptr CS:[0x611534]        ; 00610226 | ExitProcess * PTR_ExitProcess_00611534
