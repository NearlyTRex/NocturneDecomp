  ...
  if (pWVar2 == (LPWSTR)0x0) {
    ...
    *(ushort *)((int)dest + SVar1) = 0;
    ...
    if (uVar4 == 0) {
      ...
      count = uVar4 * 2;
      dest_00 = malloc(count + 2);
      if (dest_00 == (void *)0x0) {
        free(dest);
        return -1;
      }
      memcpy(dest_00,pWVar2 + 1,count);
      *(ushort *)((int)dest_00 + count) = 0;
    }
    iVar1 = FUN_006107f0(dest,dest_00);
    free(dest);
    free(dest_00);
    if (iVar1 == 0) {
      return -1;
    ...
    iVar1 = FUN_0060fcf4(param_1);
    if (iVar1 != 0) {
      return -1;
    }
    uVar2 = wcslen(param_1);
    dest_01 = malloc((uVar2 + 1) * 2);
    if (dest_01 == (char *)0x0) {
      convertWindowsErrorToErrno(5);
      return -1;
    }
    iVar1 = wcstombs(dest_01,param_1,(uVar2 + 1) * 2);
    if (iVar1 == -1) {
      free(dest_01);
      return -1;
    }
