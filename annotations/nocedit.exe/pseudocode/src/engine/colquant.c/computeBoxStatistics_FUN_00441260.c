...

{
  ...
  local_30 = (ulonglong)(uint)workspace->boxes[box_index].count;
  workspace->boxes[box_index].avg_red = (short)(int)ROUND(ROUND(local_78 / (double)local_30));
  local_30 = (ulonglong)(uint)workspace->boxes[box_index].count;
  workspace->boxes[box_index].avg_green = (short)(int)ROUND(ROUND(local_70 / (double)local_30));
  local_30 = (ulonglong)(uint)workspace->boxes[box_index].count;
  workspace->boxes[box_index].avg_blue = (short)(int)ROUND(ROUND(local_50 / (double)local_30));
  local_30 = (ulonglong)(uint)workspace->boxes[box_index].count;
  ...
  *(uint *)((int)&workspace->boxes[box_index].total_weighted_error + 4) = 0;
  ...
  dVar13 = engine_colquant_c_doubleMax_FUN_00441790
                     ((double)CONCAT26((short)((uint)uVar2 >> 0x10),
