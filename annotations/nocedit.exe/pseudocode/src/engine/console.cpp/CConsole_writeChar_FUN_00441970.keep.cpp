...

{
  ...
  row_limit = this_ptr->console_height + -1;
#else
  row_limit = nocturne_console_capacity() + -1;
#endif
  if (character == '\n') {
...
#if NOCTURNE_AUTHENTIC_CONSOLE
    cell = this_ptr->console_buffer +
           (this_ptr->current_column + this_ptr->current_row * GAME_CONSOLE_ROW_STRIDE);
#else
    cell = nocturne_console_row(this_ptr->current_row) + this_ptr->current_column;
#endif
    if (character == '\0') {
      *cell = '\0';
      return;
