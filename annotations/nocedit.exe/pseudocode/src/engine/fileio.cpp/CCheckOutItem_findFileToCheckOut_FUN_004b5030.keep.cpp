// Name: engine_fileio.cpp_CCheckOutItem_findFileToCheckOut_FUN_004b5030
// Address: 004b5030
// MANUAL RECONSTRUCTION
// Address Range: [[004b5030, 004b5347]]
// Convention: __cdecl
// Signature: int __cdecl engine_fileio_cpp_CCheckOutItem_findFileToCheckOut_FUN_004b5030(CCheckOutItem *this_ptr,char *file_pattern,char *output_filename_buffer)

#include "nocturne.h"

int __cdecl engine_fileio_cpp_CCheckOutItem_findFileToCheckOut_FUN_004b5030(CCheckOutItem *this_ptr,char *file_pattern,char *output_filename_buffer)

{
  int iVar2;
  int iVar4;
  int iVar3;
  int iVar5;
  CCheckOutItem *pCVar4;
  int selected_index;
  CPickList local_86c;
  char local_4c4 [400];
  CFileFinder local_334;
  char local_220 [260];
  char local_11c [256];
  CCheckOutList local_1c;
  char local_14 [4];

  iVar2 = engine_fileio_cpp_establishUserIdentity_FUN_004b1c00();
  if (iVar2 == 0) {
    return 0;
  }
  local_1c.count = 0;
  local_1c.items = (CCheckOutItem *)0x0;
  iVar4 = engine_fileio_cpp_CCheckOutList_load_FUN_004b2890(&local_1c);
  if (iVar4 == 0) {
    engine_fileio_cpp_CCheckOutList_reset_FUN_004b2860(&local_1c);
    return 0;
  }
  engine_dosio_cpp_ensureTrailingSlash_FUN_00481f80(this_ptr->name,local_14,local_11c);
  engine_dosio_cpp_makePath_FUN_00481f50(local_220,local_14,local_11c,(char *)0x0,(char *)0x0);
  strcat(local_220,file_pattern);
  shape_edittool_cpp_CPickList_ctor_FUN_004a3b90(&local_86c);
  shape_edittool_cpp_CPickList_setSelectedResult_FUN_004a3e10(&local_86c,1);
  engine_dosio_cpp_CFileFinder_ctor_FUN_00481c30(&local_334);
  engine_dosio_cpp_CFileFinder_openSearch_FUN_00481c70(&local_334,local_220);
  while (local_334.filename[0] != '\0') {
    iVar4 = _sprintf(local_4c4,"%s\t",local_334.filename);
    iVar3 = engine_fileio_cpp_CCheckOutList_findEntry_FUN_004b2e60(&local_1c,local_334.filename);
    if (-1 < iVar3) {
      _sprintf
                (local_4c4 + iVar4,"Checked out by %s",local_1c.items[iVar3].value);
    }
    shape_edittool_cpp_CStrList_add_FUN_004a2b80(&local_86c.base,local_4c4);
    engine_dosio_cpp_CFileFinder_findNext_FUN_00481cf0(&local_334);
  }
  selected_index = 0;
  engine_dosio_cpp_CFileFinder_closeSearch_FUN_00481d70(&local_334);
  while( true ) {
    selected_index = shape_edittool_cpp_CPickList_displayChoicesAndWaitForInput_FUN_004a3e20
                               (&local_86c,"Select file to check out",selected_index,0);
    if (selected_index < 0) {
      engine_dosio_cpp_CFileFinder_dtor_FUN_00481c50(&local_334,0);
      shape_edittool_cpp_CPickList_dtor_FUN_004a3c80(&local_86c,0);
      engine_fileio_cpp_CCheckOutList_reset_FUN_004b2860(&local_1c);
      return 0;
    }
    shape_edittool_cpp_CStrList_getFieldAt_FUN_004a2f80
              (&local_86c.base,output_filename_buffer,selected_index,0);
    iVar4 = engine_fileio_cpp_CCheckOutList_findEntry_FUN_004b2e60(&local_1c,output_filename_buffer)
    ;
    if (iVar4 < 0) break;
    iVar5 = _stricmp
                      (local_1c.items[iVar4].value,g_VersionControlSession.primary_username);
    pCVar4 = local_1c.items + iVar4;
    if (iVar5 == 0) {
      shape_edittool_cpp_CEditorTools_showError_FUN_0049e740
                (g_CEditorToolsPtr,"You already have %s checked out",pCVar4->name);
    }
    else {
      shape_edittool_cpp_CEditorTools_showError_FUN_0049e740
                (g_CEditorToolsPtr,"%s is currently checked out by %s",pCVar4->name,pCVar4->value);
    }
  }
  engine_dosio_cpp_CFileFinder_dtor_FUN_00481c50(&local_334,0);
  shape_edittool_cpp_CPickList_dtor_FUN_004a3c80(&local_86c,0);
  engine_fileio_cpp_CCheckOutList_reset_FUN_004b2860(&local_1c);
  return 1;
}
