...

{
  ...
  if (-1 < (int)uVar1) {
    ...
    {
      _sprintf(local_3f4,"You currently have %s checked out.  Replace local copy with network copy?",target_filename);
    ...
    if ((uVar1 & 8) == 0) {
      _sprintf(local_200,"A writable copy of %s exists.  Replace it?",target_filename);
      iVar1 = engine_fileio_cpp_showOverwriteConfirmationDialog_FUN_004b2f90
