...
// Convention: __cdecl
// Signature: int __cdecl engine_2d_c_drawCharacterHighlighted_FUN_00401b70(int char_code,int x_pos,int y_pos)
...

{
  ...
  uVar6 = (uint)g_FontTable[char_code + -0x20].width;
  if ((((x_pos < g_ClipLeft) || (y_pos < g_ClipTop)) || ((int)((g_ClipRight + 1) - uVar6) < x_pos))
    ...
    if (g_BitsPerPixel == 8) {
      do {
        iVar3 = 0;
        puVar3 = (byte *)g_ScreenBufferArray[iVar7 / 4] + x_pos;
        if (uVar6 != 0) {
          do {
            if (*puVar1 != '\0') {
              *puVar3 = 0xff;
      ...
      do {
        iVar2 = 0;
        puVar4 = (ushort *)g_ScreenBufferArray[iVar7 / 4] + x_pos;
        if (uVar6 != 0) {
          do {
            if (*puVar1 != '\0') {
