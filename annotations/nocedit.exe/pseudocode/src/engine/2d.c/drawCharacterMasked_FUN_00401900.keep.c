...
// Convention: __cdecl
// Signature: int __cdecl engine_2d_c_drawCharacterMasked_FUN_00401900(int char_code,int x_pos,int y_pos)
...

{
  ...
  if ((((x_pos < g_ClipLeft) || (y_pos < g_ClipTop)) || ((int)((g_ClipRight + 1) - uVar7) < x_pos))
    ...
    if (g_BitsPerPixel == 8) {
      do {
        iVar4 = 0;
        puVar5 = (byte *)g_ScreenBufferArray[iVar8 / 4] + x_pos;
        if (uVar7 != 0) {
          do {
            if (*puVar1 == '\0') {
              *puVar5 = 0;
      ...
      do {
        puVar4 = (ushort *)g_ScreenBufferArray[iVar8 / 4] + x_pos;
        iVar2 = 0;
        if (uVar7 != 0) {
          do {
