  ...
  if (this_ptr->face_count == 0) {
      ...
      do {
        while ((g_CullingMode != 0 &&
               ...
               iVar3 = engine_prim_c_calculateTriangleWindingOrder_FUN_00552150
                                 (this_ptr->vertex_buffer_ptr + *(ushort *)vertex_indices,pSVar4,
                                  pSVar4), iVar3 == 0))) {
          face_count = face_count + -1;
          vertex_indices = (int *)((int)vertex_indices + local_14);
          if (face_count < 1) {
            return;
          }
        }
        ...
        if ((this_ptr->face_capture_enabled == 0) &&
          ...
          engine_clipper_c_clipAndRasterize_FUN_004371b0(iVar3,&local_68[0].x);
        }
        else {
          engine_clipper_c_clipPolygonToViewport_FUN_00438420(iVar3,&local_68[0].x);
        }
        face_count = face_count + -1;
        vertex_indices = (int *)((int)vertex_indices + local_14);
        if (face_count < 1) {
    ...
    if (0 < face_count) {
      ...
      do {
        ...
        face_count = face_count + -1;
        vertex_indices = (int *)((int)vertex_indices + local_18);
      } while (0 < face_count);
    }
  }
  return;
