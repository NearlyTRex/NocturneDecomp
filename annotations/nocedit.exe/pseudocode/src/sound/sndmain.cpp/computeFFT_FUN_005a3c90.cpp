// Name: sound_sndmain.cpp_computeFFT_FUN_005a3c90
// Address: 005a3c90
// Address Range: [[005a3c90, 005a3fd0]]
// Convention: __cdecl
// Signature: void sound_sndmain.cpp_computeFFT_FUN_005a3c90(float * input, int size, float * output_real, float * output_imag)

#include "nocturne.h"

/* WARNING: Removing unreachable block (ram,0x005a3fc6) */

void __cdecl
sound_sndmain_cpp_computeFFT_FUN_005a3c90
          (float *input,int size,float *output_real,float *output_imag)

{
  double dVar1;
  double dVar2;
  float10 fVar3;
  double dVar4;
  double dVar5;
  double dVar6;
  double dVar7;
  uint uVar8;
  float *pfVar9;
  int iVar10;
  float *pfVar11;
  uint uVar12;
  float *pfVar13;
  float *pfVar14;
  uint uVar15;
  int iVar16;
  float10 fVar17;
  float10 fVar18;
  float10 fVar19;
  float10 fVar20;
  float10 fVar21;
  float10 fVar22;
  float10 fVar23;
  int local_2c;
  int local_28;
  int local_18;
  
  for (local_28 = 1; 1 << ((byte)local_28 & 0x1f) < size; local_28 = local_28 + 1) {
  }
  uVar15 = 0;
  if (0 < size) {
    do {
      iVar10 = 0;
      uVar8 = 0;
      uVar12 = uVar15;
      if (0 < local_28) {
        do {
          iVar10 = iVar10 + 1;
          uVar8 = uVar8 * 2 | uVar12 & 1;
          uVar12 = (int)uVar12 >> 1;
        } while (iVar10 < local_28);
      }
      output_real[uVar8] = *input;
      output_imag[uVar8] = 0.0;
      uVar15 = uVar15 + 1;
      input = input + 1;
    } while ((int)uVar15 < size);
  }
  local_2c = 2;
  if (1 < size) {
    do {
      iVar10 = local_2c / 2;
      fVar17 = (float10)3.1415926535000001 / (float10)iVar10;
      fVar3 = (float10)fcos(fVar17);
      fVar18 = (float10)fsin(fVar17);
      fVar19 = (float10)2;
      fVar17 = fVar17 * fVar19;
      fVar20 = (float10)fsin(fVar17);
      fVar17 = (float10)fcos(fVar17);
      local_18 = 0;
      if (0 < size) {
        do {
          iVar16 = 0;
          if (0 < iVar10) {
            fVar21 = (float10)(double)((float10)(double)fVar3 * fVar19);
            pfVar9 = output_imag + local_18;
            pfVar11 = output_real + iVar10 + local_18;
            pfVar13 = output_real + local_18;
            pfVar14 = output_imag + iVar10 + local_18;
            dVar1 = (double)fVar3;
            dVar5 = (double)fVar17;
            dVar2 = (double)fVar18;
            dVar7 = (double)fVar20;
            do {
              dVar6 = dVar2;
              dVar4 = dVar1;
              fVar22 = fVar21 * (float10)dVar4 - (float10)dVar5;
              dVar1 = (double)fVar22;
              fVar23 = fVar21 * (float10)dVar6 - (float10)dVar7;
              dVar2 = (double)fVar23;
              iVar16 = iVar16 + 1;
              fVar23 = fVar22 * (float10)*pfVar11 - fVar23 * (float10)*pfVar14;
              fVar22 = (float10)dVar2 * (float10)*pfVar11 + (float10)dVar1 * (float10)*pfVar14;
              *pfVar11 = (float)((float10)*pfVar13 - fVar23);
              *pfVar14 = (float)((float10)*pfVar9 - fVar22);
              *pfVar13 = (float)((float10)*pfVar13 + fVar23);
              *pfVar9 = (float)(fVar22 + (float10)*pfVar9);
              pfVar9 = pfVar9 + 1;
              pfVar11 = pfVar11 + 1;
              pfVar13 = pfVar13 + 1;
              pfVar14 = pfVar14 + 1;
              dVar5 = dVar4;
              dVar7 = dVar6;
            } while (iVar16 < iVar10);
          }
          local_18 = local_18 + local_2c;
        } while (local_18 < size);
      }
      local_2c = local_2c * 2;
    } while (local_2c <= size);
  }
  return;
}
