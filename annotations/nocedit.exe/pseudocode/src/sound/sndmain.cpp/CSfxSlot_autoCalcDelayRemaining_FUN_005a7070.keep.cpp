...
// Convention: __cdecl
// Signature: void __cdecl sound_sndmain_cpp_CSfxSlot_autoCalcDelayRemaining_FUN_005a7070(CSfxSlot *this_ptr)
...

{
  ...
  if (((this_ptr->options).flags & 1) != 0) {
    (this_ptr->options).delay_remaining = 0.0;
    return;
  }
  ...
  (this_ptr->options).delay_remaining =
       (SQRT(dVar2 * dVar2 + dVar3 * dVar3 + dVar1 * dVar1) * g_Cached3DDistanceFactor) /
       1116.4000000000001;
  return;
}
