...

{
  ...
  if (((this_ptr->options).flags & 1) == 0) {
    ...
    if (0 < g_AudioChannelCount) {
      do {
        local_30 = this_ptr->distance_to_speakers[iVar3];
        if (this_ptr->distance_to_speakers[iVar3] < this_ptr->min_distance) {
        ...
        this_ptr->channel_volumes[iVar3] = fVar2 * local_20;
        iVar3 = iVar3 + 1;
    ...
    if (0 < g_AudioChannelCount) {
      do {
        this_ptr->channel_volumes[iVar2] = fVar2;
        iVar2 = iVar2 + 1;
