...
// Convention: __cdecl
// Signature: void sound_sndmain.cpp_CSfxSample_releaseSoundBuffer_FUN_005a6540(CSfxSample * sample)
// Cross-references:
  ...
  if (sample->sound_buffer != (void *)0x0) {
    if ((sample->buffer_id != (void *)0x0) && (g_CSoundInstance != (CSound *)0x0)) {
      (*g_CSoundInstance->vtable->releaseBuffer)
                (g_CSoundInstance,sample->buffer_id,sample->field12_0x178,sample->sound_buffer);
    }
    sample->sound_buffer = (void *)0x0;
    sample->field12_0x178 = (void *)0x0;
  }
  return;
}
