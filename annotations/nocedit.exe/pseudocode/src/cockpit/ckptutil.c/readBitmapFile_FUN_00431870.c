...

{
  ...
  if (buffer == (void *)0x0) {
    ...
    if (buffer == (void *)0x0) {
      sprintf(local_60,"Unable to allocate %u bytes for bitmap file (%s).",size,filename);
  ...
  if (file == (_FILE *)0x0) {
    sprintf(local_60,"Unable to open bitmap file (%s).",filename);
    g_CurrentFilename = "..\\cockpit\\ckptutil.c";
  ...
  if (SVar1 != 1) {
    sprintf(local_60,"Unable to read bitmap file (%s).",filename);
