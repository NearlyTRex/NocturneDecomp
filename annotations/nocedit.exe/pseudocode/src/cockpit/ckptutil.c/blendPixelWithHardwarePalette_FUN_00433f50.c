...

{
  ...
  if ((((g_ClipLeft <= x) && (x <= g_ClipRight)) && (g_ClipTop <= y)) &&
    ...
    if (g_BitsPerPixel != 0x10) {
      ...
      local_34 = (uint)(longlong)ROUND(fVar13);
      uStack_2c = (uint)(longlong)ROUND(dVar14);
      local_3c = (uint)(longlong)ROUND(fVar11);
      *(uint *)((int)g_ScreenBufferArray[y] + extraout_EAX_00 * 4) =
           local_3c << ((byte)g_GreenBitPosition & 0x1f) | local_34 << (extraout_CL_00 & 0x1f) |
    ...
    fVar11 = (float10)dVar14;
    dVar14 = crt_math_c_round_FUN_005fe6b0((double)fVar12);
    uStack_2c = (uint)(longlong)ROUND(fVar11);
    local_34 = (uint)(longlong)ROUND(dVar14);
    ...
    *(ushort *)((int)g_ScreenBufferArray[y] + extraout_EAX * 2) =
         (ushort)((uStack_2c >> ((byte)g_GreenBitCount & 0x1f)) << ((byte)g_GreenBitPosition & 0x1f)
                 ) | (ushort)((local_3c >> (extraout_CL & 0x1f)) << ((byte)g_RedBitPosition & 0x1f))
