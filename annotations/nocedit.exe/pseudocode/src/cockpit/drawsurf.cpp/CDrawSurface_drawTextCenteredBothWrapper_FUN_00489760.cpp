// Name: cockpit_drawsurf.cpp_CDrawSurface_drawTextCenteredBothWrapper_FUN_00489760
// Address: 00489760
// Address Range: [[00489760, 00489780]]
// Convention: __cdecl
// Signature: void __cdecl cockpit_drawsurf_cpp_CDrawSurface_drawTextCenteredBothWrapper_FUN_00489760 (CDrawSurface *this_ptr,int x,int y,char *text)

#include "nocturne.h"

void __cdecl
cockpit_drawsurf_cpp_CDrawSurface_drawTextCenteredBothWrapper_FUN_00489760
          (CDrawSurface *this_ptr,int x,int y,char *text)

{
  cockpit_drawsurf_cpp_CDrawSurface_drawTextCenteredBoth_FUN_00489730(this_ptr,text,x,y);
  return;
}
