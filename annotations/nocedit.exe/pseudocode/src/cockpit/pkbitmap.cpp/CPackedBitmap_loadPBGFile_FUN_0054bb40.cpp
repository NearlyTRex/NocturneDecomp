// Name: cockpit_pkbitmap.cpp_CPackedBitmap_loadPBGFile_FUN_0054bb40
// Address: 0054bb40
// Address Range: [[0054bb40, 0054bc98]]
// Convention: __cdecl
// Signature: int __cdecl cockpit_pkbitmap_cpp_CPackedBitmap_loadPBGFile_FUN_0054bb40 (CPackedBitmap *this_ptr,char *pbg_filename,char *palette_filename,int param3,int param4, int selected_bitmap_index)

#include "nocturne.h"

int __cdecl
cockpit_pkbitmap_cpp_CPackedBitmap_loadPBGFile_FUN_0054bb40
          (CPackedBitmap *this_ptr,char *pbg_filename,char *palette_filename,int param3,int param4,
...

{
  ...
  pCVar1 = cockpit_pkbitmap_cpp_CPackedBitmap_parsePBGFile_FUN_0054b9b0
                     (this_ptr,file_handle,param3,param4,selected_bitmap_index);
  local_14 = pCVar1;
  shape_memdbg_cpp_closeFile_FUN_0050f9b0(file_handle,"..\\cockpit\\pkbitmap.cpp",0x59a);
  iVar2 = 0;
  if (0 < *(int *)this_ptr->filename) {
    do {
      iVar2 = iVar2 + 1;
      cockpit_pkbitmap_cpp_CPackedBitmap_setFilename_FUN_0054a990(pCVar1,pbg_filename);
      pCVar1 = pCVar1 + 1;
    } while (iVar2 < *(int *)this_ptr->filename);
  }
    ...
    if (0 < *(int *)this_ptr->filename) {
      do {
        while ((-1 < selected_bitmap_index && (iVar2 != selected_bitmap_index))) {
          iVar2 = iVar2 + 1;
          pCVar1 = pCVar1 + 1;
          if (*(int *)this_ptr->filename <= iVar2) {
            return (int)local_14;
          }
        }
