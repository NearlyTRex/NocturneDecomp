...

{
  ...
  if (size * iVar2 - iVar1 != 0) {
    _sprintf
              (local_144,"Size of %s is %lu, must be a multiple of frame size %ux%u in CPackedBitmapSet::loadJoinedRAW",filename,(unsigned long)iVar1,bitmap_width,
               bitmap_height);
  ...
  bitmap_data = (uchar *)shape_memdbg_cpp_debugMalloc_FUN_0050f250(size,"..\\cockpit\\pkbmpset.cpp",211)
  ;
  if (bitmap_data == (uchar *)0x0) {
    _sprintf(local_e0,"Can't allocate %u bytes for RAW buffer in CPackedBitmapSet::loadJoinedRAW",(uint)size);
    g_CurrentFilename = "..\\cockpit\\pkbmpset.cpp";
  ...
  shape_memdbg_cpp_closeFile_FUN_0050f9b0(file,"..\\cockpit\\pkbmpset.cpp",237);
