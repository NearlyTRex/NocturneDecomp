...

{
  ...
  array_memory = shape_memdbg_cpp_debugAllocTracked2_FUN_0050f1f0
                           (bitmap_count * (int)sizeof(CPackedBitmap) + 4,"..\\cockpit\\pkbmpset.cpp",100);
  ...
  if (pCVar1 == (CPackedBitmap *)0x0) {
    _sprintf(local_d0,"Unable to allocate %u bytes for %u CPackedBitmaps in CPackedBitmapSet::alloc",bitmap_count * (int)sizeof(CPackedBitmap),bitmap_count);
