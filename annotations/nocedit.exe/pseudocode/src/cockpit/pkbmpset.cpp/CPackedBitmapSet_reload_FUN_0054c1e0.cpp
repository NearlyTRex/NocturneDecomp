...

{
  ...
  if (local_18 == (uchar *)0x0) {
    sprintf(local_148,"Can't allocate %u bytes for RAW buffer in CPackedBitmapSet::loadJoinedRAW",size);
  ...
  if (0 < this_ptr->bitmap_count) {
    ...
    do {
      if ((in_stack_0000000c < 0) || (iVar2 == in_stack_0000000c)) {
        ...
        if (SVar1 != 1) {
          sprintf(local_80,"Error reading from %s in CPackedBitmapSet::loadJoinedRAW",local_1c);
