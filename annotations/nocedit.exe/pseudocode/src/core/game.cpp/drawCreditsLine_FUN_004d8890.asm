; *****************************************************************************
;                               FUNCTION
; *****************************************************************************
; void __cdecl core_game_cpp_drawCreditsLine_FUN_004d8890(char *text_string)
;
; Parameters:
; char *           Stack[0x4]:4   text_string
;
; Referenced Globals:
;   int g_WindowWidth = 0x140
;   int g_WindowHeight = 0xc8
;   CBitFont* g_ThemeFont
;   int g_HelpCursorX
;   int g_HelpCursorY
;   int g_HelpLineHeight
;
; Called Functions:
;   engine_font.cpp_CBitFont_drawText_FUN_004cda80
;
; *****************************************************************************

section .text

    PUSH EBX                            ; 004d8890
        ;   Label: core_game.cpp_drawCreditsLine_FUN_004d8890
    PUSH ESI                            ; 004d8891
    PUSH EDI                            ; 004d8892
    PUSH -0x1                           ; 004d8893
    PUSH 0x7                            ; 004d8895
    MOV EDX,dword ptr [0x02d831c8]      ; 004d8897 | g_HelpCursorY
    PUSH EDX                            ; 004d889d
    MOV ECX,dword ptr [0x02d831c4]      ; 004d889e | g_HelpCursorX
    PUSH ECX                            ; 004d88a4
    MOV EBX,dword ptr [ESP + 0x20]      ; 004d88a5
    PUSH EBX                            ; 004d88a9
    MOV ESI,dword ptr [0x020a5720]      ; 004d88aa | g_ThemeFont
    PUSH ESI                            ; 004d88b0
    CALL engine_font.cpp_CBitFont_drawText_FUN_004cda80 ; 004d88b1
        ;   XREF to: 004cda80 (UNCONDITIONAL_CALL)  ; int engine_font.cpp_CBitFont_drawText_FUN_004cda80(CBitFont * this_ptr, char * text, int x, int y, ...)
    MOV EAX,[0x02d831cc]                ; 004d88b6 | g_HelpLineHeight
    MOV EDI,dword ptr [0x02d831c8]      ; 004d88bb | g_HelpCursorY
    MOV EDX,dword ptr [0x00679398]      ; 004d88c1 | g_WindowHeight
    ADD EDI,EAX                         ; 004d88c7
    SHL EAX,0x2                         ; 004d88c9
    ADD ESP,0x18                        ; 004d88cc
    SUB EDX,EAX                         ; 004d88cf
    MOV dword ptr [0x02d831c8],EDI      ; 004d88d1 | g_HelpCursorY
    CMP EDX,EDI                         ; 004d88d7
    JL 0x004d88df                       ; 004d88d9
        ;   XREF to: 004d88df (CONDITIONAL_JUMP)  ; LAB_004d88df
    POP EDI                             ; 004d88db
    POP ESI                             ; 004d88dc
    POP EBX                             ; 004d88dd
    RET                                 ; 004d88de
    MOV EDX,dword ptr [0x02d831cc]      ; 004d88df | g_HelpLineHeight
        ;   Label: LAB_004d88df
    LEA EAX,[EDX*0x4 + 0x0]             ; 004d88e5
    SUB EAX,EDX                         ; 004d88ec
    ADD EAX,EAX                         ; 004d88ee
    MOV [0x02d831c8],EAX                ; 004d88f0 | g_HelpCursorY
    MOV EAX,[0x00679394]                ; 004d88f5 | g_WindowWidth
    MOV EDX,EAX                         ; 004d88fa
    MOV ECX,0x3                         ; 004d88fc
    SAR EDX,0x1f                        ; 004d8901
    IDIV ECX                            ; 004d8904
    ADD dword ptr [0x02d831c4],EAX      ; 004d8906 | g_HelpCursorX
    POP EDI                             ; 004d890c
    POP ESI                             ; 004d890d
    POP EBX                             ; 004d890e
    RET                                 ; 004d890f

