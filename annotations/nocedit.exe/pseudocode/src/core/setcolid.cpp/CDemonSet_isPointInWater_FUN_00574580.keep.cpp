...

{
  ...
  if ((ABS(g_CWaterPtr->water_level_y) == 0.0) || (g_CWaterPtr->water_level_y <= point->y)) {
    ...
    if (0 < this_ptr->actor_count) {
      do {
        this_ptr_00 = (CWaterActor *)
                      core_actor_cpp_castToClassHash_FUN_0040c790
                                (this_ptr->actors[iVar1],g_CWaterActorClassInfo.name_hash);
        if (this_ptr_00 != (CWaterActor *)0x0) {
          core_actor_cpp_CDemonActor_worldToLocalPoint_FUN_00408f10
                    ((CDemonActor *)this_ptr_00,&local_1c,point);
          (*((this_ptr_00->base).vtable._ub)->getBoundingBox)((CDemonActor *)this_ptr_00,&CStack_34)
          ;
