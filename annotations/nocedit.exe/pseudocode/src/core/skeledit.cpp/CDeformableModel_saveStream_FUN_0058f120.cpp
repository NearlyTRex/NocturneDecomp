...

{
  ...
  if (0 < this_ptr->num_parts) {
    do {
      ...
      for (iVar22 = 0; iVar22 < this_ptr->parts[local_24].adj_part_count; iVar22 = iVar22 + 1) {
        _fprintf(file_handle,"\t%d\n");
      }
      iVar22 = 0;
      pCVar18 = pCVar20;
  ...
  if (0 < this_ptr->num_lods) {
    local_34 = 0;
    do {
      for (iVar21 = 0; iVar21 < *(int *)((int)this_ptr->cap_tri_count + local_34);
          iVar21 = iVar21 + 1) {
        _fprintf(file_handle,"%d\n");
      }
      iVar22 = iVar22 + 1;
    ...
    do {
      ...
      if (0 < this_ptr->num_textures) {
        do {
          _fprintf(file_handle,"%s\n");
          iVar21 = iVar21 + 1;
        } while (iVar21 < this_ptr->num_textures);
      }
      iVar22 = iVar22 + 1;
    } while (iVar22 < this_ptr->num_texture_sets);
  }
