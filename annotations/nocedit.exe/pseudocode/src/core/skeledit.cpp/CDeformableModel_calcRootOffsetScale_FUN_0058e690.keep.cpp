...

{
  ...
  this_ptr_00 = (CDeformableModelInstance *)shape_memdbg_cpp_debugAllocTracked1_FUN_0050f1b0
                          (sizeof(CDeformableModelInstance),"..\\core\\skeledit.cpp",2557);
  ...
  if (&local_5c != (CBoundingBox3D *)&local_38) {
    local_5c.min = local_38;
  }
  if (&local_5c.max != &local_38) {
    local_5c.max = local_38;
  }
  iVar10 = 1;
  if (1 < this_ptr->vertex_count[0]) {
    do {
      local_38.x = (float)this_ptr_01->skinned_vertices_buffer[iVar10].x * 0.00390625f;
      local_38.y = (float)this_ptr_01->skinned_vertices_buffer[iVar10].y * 0.00390625f;
      local_38.z = (float)this_ptr_01->skinned_vertices_buffer[iVar10].z * 0.00390625f;
