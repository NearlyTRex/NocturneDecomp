; *****************************************************************************
;                               FUNCTION
; *****************************************************************************
; unknown undefined core_msnedit.cpp_FUN_0053ca80()
;
;
; XREF[1]:
;   core_msnedit.cpp_PrepareMissionMaybe_FUN_005390f0 at 00539cf3
;
; *****************************************************************************

section .text

    PUSH ESI                            ; 0053ca80
        ;   Label: core_msnedit.cpp_FUN_0053ca80
    MOV ECX,dword ptr [ESP + 0x8]       ; 0053ca81
    MOV ESI,dword ptr [ESP + 0xc]       ; 0053ca85
    MOV EAX,ESI                         ; 0053ca89
    TEST ESI,ESI                        ; 0053ca8b
    JNZ 0x0053ca95                      ; 0053ca8d | LAB_0053ca95
        ;   XREF to: 0053ca95 (CONDITIONAL_JUMP)
    MOV EAX,dword ptr [ECX + 0x54c]     ; 0053ca8f
    TEST EAX,EAX                        ; 0053ca95
        ;   Label: LAB_0053ca95
    JZ 0x0053cac1                       ; 0053ca97 | LAB_0053cac1
        ;   XREF to: 0053cac1 (CONDITIONAL_JUMP)
    PUSH EDI                            ; 0053ca99
    XOR EDX,EDX                         ; 0053ca9a
    MOV EAX,dword ptr [EAX + 0x150]     ; 0053ca9c
        ;   Label: LAB_0053ca9c
    TEST EAX,EAX                        ; 0053caa2
    JNZ 0x0053caac                      ; 0053caa4 | LAB_0053caac
        ;   XREF to: 0053caac (CONDITIONAL_JUMP)
    MOV EAX,dword ptr [ECX + 0x54c]     ; 0053caa6
    CMP EAX,ESI                         ; 0053caac
        ;   Label: LAB_0053caac
    JZ 0x0053cac0                       ; 0053caae | LAB_0053cac0
        ;   XREF to: 0053cac0 (CONDITIONAL_JUMP)
    MOV EDI,dword ptr [ECX]             ; 0053cab0
    CMP EDI,dword ptr [EAX + 0x2c]      ; 0053cab2
    JZ 0x0053cac3                       ; 0053cab5 | LAB_0053cac3
        ;   XREF to: 0053cac3 (CONDITIONAL_JUMP)
    INC EDX                             ; 0053cab7
        ;   Label: LAB_0053cab7
    CMP EDX,0x3e8                       ; 0053cab8
    JL 0x0053ca9c                       ; 0053cabe | LAB_0053ca9c
        ;   XREF to: 0053ca9c (CONDITIONAL_JUMP)
    POP EDI                             ; 0053cac0
        ;   Label: LAB_0053cac0
    POP ESI                             ; 0053cac1
        ;   Label: LAB_0053cac1
    RET                                 ; 0053cac2
    CMP dword ptr [EAX + 0x148],0x0     ; 0053cac3
        ;   Label: LAB_0053cac3
    JNZ 0x0053cab7                      ; 0053caca | LAB_0053cab7
        ;   XREF to: 0053cab7 (CONDITIONAL_JUMP)
    POP EDI                             ; 0053cacc
    POP ESI                             ; 0053cacd
    RET                                 ; 0053cace

