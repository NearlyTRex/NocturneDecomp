...

{
  ...
  if ((this_ptr->needs_actor_search != 0) && (this_ptr->dest_actor == (CDemonActor *)0x0)) {
    for (iVar4 = 0; iVar4 < g_CDemonSetPtr->actor_count; iVar4 = iVar4 + 1) {
      pCVar1 = g_CDemonSetPtr->actors[iVar4];
      local_2c = (this_ptr->base).location.position.x - (pCVar1->location).position.x;
      local_28 = (this_ptr->base).location.position.y - (pCVar1->location).position.y;
      local_24 = (this_ptr->base).location.position.z - (pCVar1->location).position.z;
