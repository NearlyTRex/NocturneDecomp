// Name: core_setedit.cpp_CDemonSet_writeIni_FUN_00584920
// Address: 00584920
// Address Range: [[00584920, 00584937]]
// Convention: __cdecl
// Signature: void __cdecl core_setedit_cpp_CDemonSet_writeIni_FUN_00584920(CDemonSet *this_ptr,CIniFile *ini_file)

#include "nocturne.h"

void __cdecl core_setedit_cpp_CDemonSet_writeIni_FUN_00584920(CDemonSet *this_ptr,CIniFile *ini_file)

{
  engine_ini_cpp_CIniFile_setString_FUN_004fbbb0
            (ini_file,"groundTypeDirectory",g_GroundTypesBuffer);
  return;
}
