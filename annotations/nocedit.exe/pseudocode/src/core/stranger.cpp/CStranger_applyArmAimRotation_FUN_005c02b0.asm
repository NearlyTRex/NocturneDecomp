; *****************************************************************************
;                               FUNCTION
; *****************************************************************************
; void __cdecl core_stranger_cpp_CStranger_applyArmAimRotation_FUN_005c02b0(CStranger *this_ptr,int arm_side)
;
; Parameters:
; CStranger *      Stack[0x4]:4   this_ptr
; int              Stack[0x8]:4   arm_side
; Local Variables:
; float[1520]      Stack[-0x19b8]:6080  afStackY_19b8
; float            Stack[-0x1e0]:4  local_1e0
; CQuaternion4f    Stack[-0x1dc]:16  local_1dc
; CQuaternion4f    Stack[-0x1cc]:16  local_1cc
; CQuaternion4f    Stack[-0x1bc]:16  local_1bc
; CQuaternion4f    Stack[-0x1ac]:16  local_1ac
; CQuaternion4f    Stack[-0x19c]:16  local_19c
; CQuaternion4f    Stack[-0x18c]:16  local_18c
; CQuaternion4f    Stack[-0x17c]:16  local_17c
; CQuaternion4f    Stack[-0x16c]:16  local_16c
; CQuaternion4f    Stack[-0x15c]:16  local_15c
; CQuaternion4f    Stack[-0x14c]:16  local_14c
; CQuaternion4f    Stack[-0x13c]:16  local_13c
; CQuaternion4f    Stack[-0x12c]:16  local_12c
; CQuaternion4f    Stack[-0x11c]:16  local_11c
; CQuaternion4f    Stack[-0x10c]:16  local_10c
; CQuaternion4f    Stack[-0xfc]:16  local_fc
; CQuaternion4f    Stack[-0xec]:16  local_ec
; CQuaternion4f    Stack[-0xdc]:16  local_dc
; CQuaternion4f    Stack[-0xcc]:16  local_cc
; CQuaternion4f    Stack[-0xbc]:16  local_bc
; CQuaternion4f    Stack[-0xac]:16  local_ac
; CQuaternion4f    Stack[-0x9c]:16  local_9c
; CQuaternion4f    Stack[-0x8c]:16  local_8c
; CQuaternion4f    Stack[-0x7c]:16  local_7c
; CQuaternion4f    Stack[-0x6c]:16  local_6c
; CQuaternion4f    Stack[-0x5c]:16  local_5c
; CQuaternion4f    Stack[-0x4c]:16  local_4c
; float            Stack[-0x34]:4  local_34
; char *           Stack[-0x30]:4  local_30
; CQuaternion4f *  Stack[-0x2c]:4  local_2c
; int              Stack[-0x28]:4  local_28
; int              Stack[-0x24]:4  local_24
; SArmAimData *    Stack[-0x20]:4  local_20
; CDeformableModelInstance * Stack[-0x1c]:4  local_1c
; float            Stack[-0x18]:4  local_18
;
; XREF[1]:
;   core_stranger.cpp_CStranger_makeDrawDecision_FUN_005bfb60 at 005c003b
;
; Referenced Globals:
;   double DOUBLE_00653944 = 0.000100000000000000
;   undefined4 g_StrangerIndices[3]
;   undefined4 g_StrangerIndices[4]
;   undefined4 g_StrangerIndices[5]
;   undefined4 g_StrangerIndices[6]
;
; Called Functions:
;   core_motion.cpp_CMotionController_markerPositionToFrame_FUN_0052e3a0
;   core_motion.cpp_CMotionList_findMotionIndex_FUN_0052d460
;   core_skeleton.cpp_CDeformableModelInstance_blendBoneRotations_FUN_0059f750
;   core_skeleton.cpp_CDeformableModelInstance_blendMotion_FUN_0059eb50
;   core_skeleton.cpp_CDeformableModelInstance_getSkeletonPtr_FUN_005a0820
;   core_xform.cpp_multiplyQuaternion_FUN_005f7640
;   core_xform.cpp_quaternionFromAngleX_FUN_005f79b0
;   core_xform.cpp_quaternionFromAngleY_FUN_005f79f0
;   core_xform.cpp_quaternionFromAngleZ_FUN_005f7a30
;   core_xform.cpp_slerpQuaternion_FUN_005f77e0
;
; *****************************************************************************

section .text

    PUSH EBX                            ; 005c02b0
        ;   Label: core_stranger.cpp_CStranger_applyArmAimRotation_FUN_005c02b0
    PUSH ESI                            ; 005c02b1
    PUSH EDI                            ; 005c02b2
    PUSH EBP                            ; 005c02b3
    MOV EBP,ESP                         ; 005c02b4
    SUB ESP,0x1cc                       ; 005c02b6
    AND ESP,0xfffffff8                  ; 005c02bc
    MOV EBX,dword ptr [EBP + 0x14]      ; 005c02bf
    CMP dword ptr [EBP + 0x18],0x0      ; 005c02c2
    JZ 0x005c05ad                       ; 005c02c6
        ;   XREF to: 005c05ad (CONDITIONAL_JUMP)  ; LAB_005c05ad
    PUSH 0xbfc90fdb                     ; 005c02cc
    MOV EAX,[0x03f6badc]                ; 005c02d1 | g_StrangerIndices[4]
    LEA ESI,[ESP + 0x8]                 ; 005c02d6
    MOV dword ptr [ESP + 0x1c0],EAX     ; 005c02da
    MOV EAX,[0x03f6bae4]                ; 005c02e1 | g_StrangerIndices[6]
    LEA EDI,[ESP + 0x18]                ; 005c02e6
    MOV dword ptr [ESP + 0x1bc],EAX     ; 005c02ea
    CALL core_xform.cpp_quaternionFromAngleY_FUN_005f79f0 ; 005c02f1
        ;   XREF to: 005f79f0 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_quaternionFromAngleY_FUN_005f79f0(float angle_radians, CQuaternion4f * quat_out)
    LEA ESI,[ESP + 0x8]                 ; 005c02f6
    ADD ESP,0x4                         ; 005c02fa
    JMP 0x0060f8d6                      ; 005c02fd
        ;   XREF to: 0060f8d6 (UNCONDITIONAL_JUMP)  ; LAB_0060f8d6
    LEA ESI,[ESP + 0xf8]                ; 005c0306
        ;   Label: LAB_005c0306
    LEA EDI,[ESP + 0x28]                ; 005c030d
    CALL core_xform.cpp_quaternionFromAngleZ_FUN_005f7a30 ; 005c0311
        ;   XREF to: 005f7a30 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_quaternionFromAngleZ_FUN_005f7a30(float angle_radians, CQuaternion4f * quat_out)
    ADD ESP,0x4                         ; 005c0316
    LEA EAX,[EBX + 0x1fbe8]             ; 005c0319
    LEA ESI,[ESP + 0xf4]                ; 005c031f
    MOV dword ptr [ESP + 0x1c0],EAX     ; 005c0326
        ;   Label: LAB_005c0326
    JMP 0x0060d067                      ; 005c032d
        ;   XREF to: 0060d067 (UNCONDITIONAL_JUMP)  ; CAVE_cave_005fde92
    MOV dword ptr [ESP],EAX             ; 005c0337
        ;   Label: LAB_005c0337
    MOV EAX,dword ptr [ESP + 0x1c0]     ; 005c033a
    FLD float ptr [EAX + 0x14]          ; 005c0341
    FLD1                                ; 005c0344
    FSUBRP                              ; 005c0346
    FLD float ptr [ESP]                 ; 005c0348
    FXCH                                ; 005c034b
    FMUL ST1                            ; 005c034d
    FLD1                                ; 005c034f
    FSUBRP                              ; 005c0351
    FMULP                               ; 005c0353
    FST float ptr [ESP]                 ; 005c0355
    FLDZ                                ; 005c0358
    FCOMPP                              ; 005c035a
    FNSTSW AX                           ; 005c035c
    SAHF                                ; 005c035e
    JNC 0x005c05a6                      ; 005c035f
        ;   XREF to: 005c05a6 (CONDITIONAL_JUMP)  ; LAB_005c05a6
    MOV EAX,dword ptr [EBX + 0x2a8c]    ; 005c0365
    SHL EAX,0x3                         ; 005c036b
    MOV ESI,EAX                         ; 005c036e
    SHL EAX,0x3                         ; 005c0370
    SUB EAX,ESI                         ; 005c0373
    LEA ESI,[EBX + 0x262c]              ; 005c0375
    ADD EAX,ESI                         ; 005c037b
    PUSH 0x1                            ; 005c037d
    ADD EAX,0x8                         ; 005c037f
    PUSH EAX                            ; 005c0382
    LEA EAX,[EBX + 0x158]               ; 005c0383
    PUSH EAX                            ; 005c0389
    MOV dword ptr [ESP + 0x1d0],EAX     ; 005c038a
    CALL core_skeleton.cpp_CDeformableModelInstance_getSkeletonPtr_FUN_005a0820 ; 005c0391
        ;   XREF to: 005a0820 (UNCONDITIONAL_CALL)  ; CSkeleton * core_skeleton.cpp_CDeformableModelInstance_getSkeletonPtr_FUN_005a0820(CDeformableModelInstance * this_ptr)
    ADD ESP,0x4                         ; 005c0396
    PUSH EAX                            ; 005c0399
    CALL core_motion.cpp_CMotionList_findMotionIndex_FUN_0052d460 ; 005c039a
        ;   XREF to: 0052d460 (UNCONDITIONAL_CALL)  ; int core_motion.cpp_CMotionList_findMotionIndex_FUN_0052d460(CMotionList * this_ptr, char * motion_name, int error_on_not_found)
    ADD ESP,0xc                         ; 005c039f
    PUSH dword ptr [EBX + 0x2a90]       ; 005c03a2
    PUSH EAX                            ; 005c03a8
    MOV EDI,dword ptr [ESP + 0x1cc]     ; 005c03a9
    PUSH EDI                            ; 005c03b0
    MOV ESI,EAX                         ; 005c03b1
    CALL core_motion.cpp_CMotionController_markerPositionToFrame_FUN_0052e3a0 ; 005c03b3
        ;   XREF to: 0052e3a0 (UNCONDITIONAL_CALL)  ; float core_motion.cpp_CMotionController_markerPositionToFrame_FUN_0052e3a0(CMotionController * this_ptr, int motion_index, float marker_position)
    MOV dword ptr [ESP + 0x1d4],EAX     ; 005c03b8
    FLD float ptr [ESP + 0x1d4]         ; 005c03bf
    ADD ESP,0xc                         ; 005c03c6
    PUSH 0x59ddb0                       ; 005c03c9
    MOV EAX,dword ptr [ESP + 0x1c0]     ; 005c03ce
    PUSH EAX                            ; 005c03d5
    PUSH dword ptr [ESP + 0x8]          ; 005c03d6
    FSTP float ptr [ESP + 0x1b8]        ; 005c03da
    PUSH dword ptr [ESP + 0x1b8]        ; 005c03e1
    PUSH ESI                            ; 005c03e8
    PUSH EDI                            ; 005c03e9
    CALL core_skeleton.cpp_CDeformableModelInstance_blendMotion_FUN_0059eb50 ; 005c03ea
        ;   XREF to: 0059eb50 (UNCONDITIONAL_CALL)  ; void core_skeleton.cpp_CDeformableModelInstance_blendMotion_FUN_0059eb50(CDeformableModelInstance * this_ptr, int target_motion_index, float target_frame_number, float blend_weight, ...)
    ADD ESP,0x18                        ; 005c03ef
    MOV EAX,dword ptr [ESP + 0x1c0]     ; 005c03f2
    LEA ESI,[ESP + 0x194]               ; 005c03f9
    PUSH dword ptr [EAX + 0xc]          ; 005c0400
    LEA EDI,[ESP + 0x78]                ; 005c0403
    CALL core_xform.cpp_quaternionFromAngleY_FUN_005f79f0 ; 005c0407
        ;   XREF to: 005f79f0 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_quaternionFromAngleY_FUN_005f79f0(float angle_radians, CQuaternion4f * quat_out)
    ADD ESP,0x4                         ; 005c040c
    LEA EAX,[ESP + 0x74]                ; 005c040f
    LEA ESI,[ESP + 0x194]               ; 005c0413
    PUSH EAX                            ; 005c041a
    JMP 0x0060d08e                      ; 005c041b
        ;   XREF to: 0060d08e (UNCONDITIONAL_JUMP)  ; LAB_0060d08e
    LEA ESI,[ESP + 0x48]                ; 005c0426
        ;   Label: LAB_005c0426
    PUSH dword ptr [EAX + 0x8]          ; 005c042a
    LEA EDI,[ESP + 0x6c]                ; 005c042d
    CALL core_xform.cpp_quaternionFromAngleX_FUN_005f79b0 ; 005c0431
        ;   XREF to: 005f79b0 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_quaternionFromAngleX_FUN_005f79b0(float angle_radians, CQuaternion4f * quat_out)
    LEA ESI,[ESP + 0x4c]                ; 005c0436
    ADD ESP,0x4                         ; 005c043a
    JMP 0x0060d0b6                      ; 005c043d
        ;   XREF to: 0060d0b6 (UNCONDITIONAL_JUMP)  ; LAB_0060d0b6
    PUSH EAX                            ; 005c0445
        ;   Label: LAB_005c0445
    LEA ESI,[ESP + 0x13c]               ; 005c0446
    LEA EDI,[ESP + 0x5c]                ; 005c044d
    CALL core_xform.cpp_multiplyQuaternion_FUN_005f7640 ; 005c0451
        ;   XREF to: 005f7640 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_multiplyQuaternion_FUN_005f7640(CQuaternion4f * quat1_in, CQuaternion4f * quat2_in, CQuaternion4f * quat_out)
    ADD ESP,0x8                         ; 005c0456
    LEA ESI,[ESP + 0x134]               ; 005c0459
    LEA EAX,[ESP + 0x14]                ; 005c0460
    PUSH 0x3f733333                     ; 005c0464
    PUSH EAX                            ; 005c0469
    JMP 0x0060f8fc                      ; 005c046a
        ;   XREF to: 0060f8fc (UNCONDITIONAL_JUMP)  ; LAB_0060f8fc
    LEA ESI,[ESP + 0x150]               ; 005c0473
        ;   Label: LAB_005c0473
    LEA EDI,[ESP + 0x90]                ; 005c047a
    CALL core_xform.cpp_slerpQuaternion_FUN_005f77e0 ; 005c0481
        ;   XREF to: 005f77e0 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_slerpQuaternion_FUN_005f77e0(CQuaternion4f * quat1_in, CQuaternion4f * quat2_in, float t, CQuaternion4f * quat_out)
    ADD ESP,0xc                         ; 005c0486
    LEA EAX,[ESP + 0x54]                ; 005c0489
    PUSH dword ptr [ESP]                ; 005c048d
    LEA ESI,[ESP + 0x148]               ; 005c0490
    PUSH EAX                            ; 005c0497
    LEA EAX,[ESP + 0x8c]                ; 005c0498
    JMP 0x0060f922                      ; 005c049f
        ;   XREF to: 0060f922 (UNCONDITIONAL_JUMP)  ; LAB_0060f922
    LEA ESI,[ESP + 0x130]               ; 005c04a4
        ;   Label: LAB_005c04a4
    LEA EDI,[ESP + 0xa0]                ; 005c04ab
    CALL core_xform.cpp_multiplyQuaternion_FUN_005f7640 ; 005c04b2
        ;   XREF to: 005f7640 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_multiplyQuaternion_FUN_005f7640(CQuaternion4f * quat1_in, CQuaternion4f * quat2_in, CQuaternion4f * quat_out)
    LEA ESI,[ESP + 0x130]               ; 005c04b7
    ADD ESP,0x8                         ; 005c04be
    LEA EAX,[ESP + 0x98]                ; 005c04c1
    JMP 0x0060f944                      ; 005c04c8
        ;   XREF to: 0060f944 (UNCONDITIONAL_JUMP)  ; LAB_0060f944
    MOV EDI,dword ptr [ESP + 0x1c4]     ; 005c04cd
        ;   Label: LAB_005c04cd
    LEA EAX,[EBX + 0x808]               ; 005c04d4
    SHL EDI,0x4                         ; 005c04da
    ADD EAX,EDI                         ; 005c04dd
    PUSH EAX                            ; 005c04df
    LEA ESI,[ESP + 0x180]               ; 005c04e0
    MOV dword ptr [ESP + 0x1c0],EAX     ; 005c04e7
    CALL core_xform.cpp_slerpQuaternion_FUN_005f77e0 ; 005c04ee
        ;   XREF to: 005f77e0 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_slerpQuaternion_FUN_005f77e0(CQuaternion4f * quat1_in, CQuaternion4f * quat2_in, float t, CQuaternion4f * quat_out)
    LEA EAX,[EBX + EDI*0x1]             ; 005c04f3
    MOV EDI,EAX                         ; 005c04f6
    LEA ESI,[ESP + 0x180]               ; 005c04f8
    ADD ESP,0xc                         ; 005c04ff
    LEA EDI,[EDI + 0x808]               ; 005c0502
    MOV dword ptr [ESP + 0x1b0],EAX     ; 005c0508
    JMP 0x0060f966                      ; 005c050f
        ;   XREF to: 0060f966 (UNCONDITIONAL_JUMP)  ; LAB_0060f966
    PUSH EAX                            ; 005c0517
        ;   Label: LAB_005c0517
    LEA EAX,[ESP + 0x28]                ; 005c0518
    PUSH EAX                            ; 005c051c
    LEA EAX,[ESP + 0x1c]                ; 005c051d
    PUSH EAX                            ; 005c0521
    LEA ESI,[ESP + 0x160]               ; 005c0522
    LEA EDI,[ESP + 0xc0]                ; 005c0529
    CALL core_xform.cpp_multiplyQuaternion_FUN_005f7640 ; 005c0530
        ;   XREF to: 005f7640 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_multiplyQuaternion_FUN_005f7640(CQuaternion4f * quat1_in, CQuaternion4f * quat2_in, CQuaternion4f * quat_out)
    LEA ESI,[ESP + 0x160]               ; 005c0535
    ADD ESP,0x8                         ; 005c053c
    LEA EAX,[ESP + 0xb8]                ; 005c053f
    JMP 0x0060f98b                      ; 005c0546
        ;   XREF to: 0060f98b (UNCONDITIONAL_JUMP)  ; LAB_0060f98b
    LEA ESI,[ESP + 0x16c]               ; 005c054b
        ;   Label: LAB_005c054b
    LEA EDI,[ESP + 0xac]                ; 005c0552
    CALL core_xform.cpp_multiplyQuaternion_FUN_005f7640 ; 005c0559
        ;   XREF to: 005f7640 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_multiplyQuaternion_FUN_005f7640(CQuaternion4f * quat1_in, CQuaternion4f * quat2_in, CQuaternion4f * quat_out)
    LEA ESI,[ESP + 0x16c]               ; 005c055e
    ADD ESP,0x8                         ; 005c0565
    JMP 0x0060f9ad                      ; 005c0568
        ;   XREF to: 0060f9ad (UNCONDITIONAL_JUMP)  ; LAB_0060f9ad
    MOV ESI,dword ptr [ESP + 0x1bc]     ; 005c0571
        ;   Label: LAB_005c0571
    PUSH ESI                            ; 005c0578
    LEA EAX,[ESP + 0xac]                ; 005c0579
    PUSH dword ptr [ESP + 0x8]          ; 005c0580
    PUSH EAX                            ; 005c0584
    MOV EDI,dword ptr [ESP + 0x1d4]     ; 005c0585
    PUSH EDI                            ; 005c058c
    CALL core_skeleton.cpp_CDeformableModelInstance_blendBoneRotations_FUN_0059f750 ; 005c058d
        ;   XREF to: 0059f750 (UNCONDITIONAL_CALL)  ; void core_skeleton.cpp_CDeformableModelInstance_blendBoneRotations_FUN_0059f750(CDeformableModelInstance * this_ptr, CQuaternion4f * source_quaternions, float blend_weight, int bone_index, ...)
    FLD float ptr [EBX + 0x1fc8c]       ; 005c0592
    ADD ESP,0x14                        ; 005c0598
    FCOMP double ptr [0x00653944]       ; 005c059b | DOUBLE_00653944
    FNSTSW AX                           ; 005c05a1
    SAHF                                ; 005c05a3
    JA 0x005c060c                       ; 005c05a4
        ;   XREF to: 005c060c (CONDITIONAL_JUMP)  ; LAB_005c060c
    MOV ESP,EBP                         ; 005c05a6
        ;   Label: LAB_005c05a6
    POP EBP                             ; 005c05a8
    POP EDI                             ; 005c05a9
    POP ESI                             ; 005c05aa
    POP EBX                             ; 005c05ab
    RET                                 ; 005c05ac
    PUSH 0x3fc90fdb                     ; 005c05ad
        ;   Label: LAB_005c05ad
    MOV EAX,[0x03f6bad8]                ; 005c05b2 | g_StrangerIndices[3]
    LEA ESI,[ESP + 0xc8]                ; 005c05b7
    MOV dword ptr [ESP + 0x1c0],EAX     ; 005c05be
    MOV EAX,[0x03f6bae0]                ; 005c05c5 | g_StrangerIndices[5]
    LEA EDI,[ESP + 0x18]                ; 005c05ca
    MOV dword ptr [ESP + 0x1bc],EAX     ; 005c05ce
    CALL core_xform.cpp_quaternionFromAngleY_FUN_005f79f0 ; 005c05d5
        ;   XREF to: 005f79f0 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_quaternionFromAngleY_FUN_005f79f0(float angle_radians, CQuaternion4f * quat_out)
    LEA ESI,[ESP + 0xc8]                ; 005c05da
    ADD ESP,0x4                         ; 005c05e1
    JMP 0x0060f9d3                      ; 005c05e4
        ;   XREF to: 0060f9d3 (UNCONDITIONAL_JUMP)  ; LAB_0060f9d3
    LEA ESI,[ESP + 0x38]                ; 005c05ed
        ;   Label: LAB_005c05ed
    LEA EDI,[ESP + 0x28]                ; 005c05f1
    CALL core_xform.cpp_quaternionFromAngleZ_FUN_005f7a30 ; 005c05f5
        ;   XREF to: 005f7a30 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_quaternionFromAngleZ_FUN_005f7a30(float angle_radians, CQuaternion4f * quat_out)
    ADD ESP,0x4                         ; 005c05fa
    LEA EAX,[EBX + 0x1fc08]             ; 005c05fd
    LEA ESI,[ESP + 0x34]                ; 005c0603
    JMP 0x005c0326                      ; 005c0607
        ;   XREF to: 005c0326 (UNCONDITIONAL_JUMP)  ; LAB_005c0326
    PUSH 0x3fc90fdb                     ; 005c060c
        ;   Label: LAB_005c060c
    LEA ESI,[ESP + 0x188]               ; 005c0611
    LEA EDI,[ESP + 0xe8]                ; 005c0618
    CALL core_xform.cpp_quaternionFromAngleX_FUN_005f79b0 ; 005c061f
        ;   XREF to: 005f79b0 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_quaternionFromAngleX_FUN_005f79b0(float angle_radians, CQuaternion4f * quat_out)
    ADD ESP,0x4                         ; 005c0624
    LEA EAX,[ESP + 0xe4]                ; 005c0627
    LEA ESI,[ESP + 0x184]               ; 005c062e
    PUSH EAX                            ; 005c0635
    LEA EAX,[ESP + 0x88]                ; 005c0636
    JMP 0x0060f9f9                      ; 005c063d
        ;   XREF to: 0060f9f9 (UNCONDITIONAL_JUMP)  ; LAB_0060f9f9
    LEA ESI,[ESP + 0x10c]               ; 005c0642
        ;   Label: LAB_005c0642
    LEA EDI,[ESP + 0xdc]                ; 005c0649
    CALL core_xform.cpp_multiplyQuaternion_FUN_005f7640 ; 005c0650
        ;   XREF to: 005f7640 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_multiplyQuaternion_FUN_005f7640(CQuaternion4f * quat1_in, CQuaternion4f * quat2_in, CQuaternion4f * quat_out)
    LEA ESI,[ESP + 0x10c]               ; 005c0655
    ADD ESP,0x8                         ; 005c065c
    JMP 0x0060fa1b                      ; 005c065f
        ;   XREF to: 0060fa1b (UNCONDITIONAL_JUMP)  ; LAB_0060fa1b
    PUSH dword ptr [EBX + 0x1fc8c]      ; 005c066a
        ;   Label: LAB_005c066a
    PUSH EAX                            ; 005c0670
    MOV EAX,dword ptr [ESP + 0x1bc]     ; 005c0671
    PUSH EAX                            ; 005c0678
    LEA ESI,[ESP + 0x120]               ; 005c0679
    CALL core_xform.cpp_slerpQuaternion_FUN_005f77e0 ; 005c0680
        ;   XREF to: 005f77e0 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_slerpQuaternion_FUN_005f77e0(CQuaternion4f * quat1_in, CQuaternion4f * quat2_in, float t, CQuaternion4f * quat_out)
    ADD ESP,0xc                         ; 005c0685
    MOV EDI,dword ptr [ESP + 0x1b0]     ; 005c0688
    LEA ESI,[ESP + 0x114]               ; 005c068f
    LEA EDI,[EDI + 0x808]               ; 005c0696
    JMP 0x0060fa43                      ; 005c069c
        ;   XREF to: 0060fa43 (UNCONDITIONAL_JUMP)  ; LAB_0060fa43
    POP EBP                             ; 005c06a2
        ;   Label: LAB_005c06a2
    POP EDI                             ; 005c06a3
    POP ESI                             ; 005c06a4
    POP EBX                             ; 005c06a5
    RET                                 ; 005c06a6
    MOV ECX,dword ptr [ESI]             ; 0060d067
        ;   Label: CAVE_cave_005fde92
    MOV dword ptr [EDI],ECX             ; 0060d069
    MOV ECX,dword ptr [ESI + 0x4]       ; 0060d06b
    MOV dword ptr [EDI + 0x4],ECX       ; 0060d06e
    MOV ECX,dword ptr [ESI + 0x8]       ; 0060d071
    MOV dword ptr [EDI + 0x8],ECX       ; 0060d074
    MOV ECX,dword ptr [ESI + 0xc]       ; 0060d077
    MOV dword ptr [EDI + 0xc],ECX       ; 0060d07a
    ADD ESI,0x10                        ; 0060d07d
    ADD EDI,0x10                        ; 0060d080
    MOV EAX,dword ptr [EBX + 0x2a90]    ; 0060d083
    JMP 0x005c0337                      ; 0060d089
        ;   XREF to: 005c0337 (UNCONDITIONAL_JUMP)  ; LAB_005c0337
    MOV ECX,dword ptr [ESI]             ; 0060d08e
        ;   Label: LAB_0060d08e
    MOV dword ptr [EDI],ECX             ; 0060d090
    MOV ECX,dword ptr [ESI + 0x4]       ; 0060d092
    MOV dword ptr [EDI + 0x4],ECX       ; 0060d095
    MOV ECX,dword ptr [ESI + 0x8]       ; 0060d098
    MOV dword ptr [EDI + 0x8],ECX       ; 0060d09b
    MOV ECX,dword ptr [ESI + 0xc]       ; 0060d09e
    MOV dword ptr [EDI + 0xc],ECX       ; 0060d0a1
    ADD ESI,0x10                        ; 0060d0a4
    ADD EDI,0x10                        ; 0060d0a7
    MOV EAX,dword ptr [ESP + 0x1c4]     ; 0060d0aa
    JMP 0x005c0426                      ; 0060d0b1
        ;   XREF to: 005c0426 (UNCONDITIONAL_JUMP)  ; LAB_005c0426
    LEA EAX,[ESP + 0x68]                ; 0060d0b6
        ;   Label: LAB_0060d0b6
    MOV ECX,dword ptr [ESI]             ; 0060d0ba
    MOV dword ptr [EDI],ECX             ; 0060d0bc
    MOV ECX,dword ptr [ESI + 0x4]       ; 0060d0be
    MOV dword ptr [EDI + 0x4],ECX       ; 0060d0c1
    MOV ECX,dword ptr [ESI + 0x8]       ; 0060d0c4
    MOV dword ptr [EDI + 0x8],ECX       ; 0060d0c7
    MOV ECX,dword ptr [ESI + 0xc]       ; 0060d0ca
    MOV dword ptr [EDI + 0xc],ECX       ; 0060d0cd
    ADD ESI,0x10                        ; 0060d0d0
    ADD EDI,0x10                        ; 0060d0d3
    JMP 0x005c0445                      ; 0060d0d6
        ;   XREF to: 005c0445 (UNCONDITIONAL_JUMP)  ; LAB_005c0445
    MOV ECX,dword ptr [ESI]             ; 0060f8d6
        ;   Label: LAB_0060f8d6
    MOV dword ptr [EDI],ECX             ; 0060f8d8
    MOV ECX,dword ptr [ESI + 0x4]       ; 0060f8da
    MOV dword ptr [EDI + 0x4],ECX       ; 0060f8dd
    MOV ECX,dword ptr [ESI + 0x8]       ; 0060f8e0
    MOV dword ptr [EDI + 0x8],ECX       ; 0060f8e3
    MOV ECX,dword ptr [ESI + 0xc]       ; 0060f8e6
    MOV dword ptr [EDI + 0xc],ECX       ; 0060f8e9
    ADD ESI,0x10                        ; 0060f8ec
    ADD EDI,0x10                        ; 0060f8ef
    PUSH 0xbfc90fdb                     ; 0060f8f2
    JMP 0x005c0306                      ; 0060f8f7
        ;   XREF to: 005c0306 (UNCONDITIONAL_JUMP)  ; LAB_005c0306
    MOV ECX,dword ptr [ESI]             ; 0060f8fc
        ;   Label: LAB_0060f8fc
    MOV dword ptr [EDI],ECX             ; 0060f8fe
    MOV ECX,dword ptr [ESI + 0x4]       ; 0060f900
    MOV dword ptr [EDI + 0x4],ECX       ; 0060f903
    MOV ECX,dword ptr [ESI + 0x8]       ; 0060f906
    MOV dword ptr [EDI + 0x8],ECX       ; 0060f909
    MOV ECX,dword ptr [ESI + 0xc]       ; 0060f90c
    MOV dword ptr [EDI + 0xc],ECX       ; 0060f90f
    ADD ESI,0x10                        ; 0060f912
    ADD EDI,0x10                        ; 0060f915
    PUSH 0x665998                       ; 0060f918
    JMP 0x005c0473                      ; 0060f91d
        ;   XREF to: 005c0473 (UNCONDITIONAL_JUMP)  ; LAB_005c0473
    MOV ECX,dword ptr [ESI]             ; 0060f922
        ;   Label: LAB_0060f922
    MOV dword ptr [EDI],ECX             ; 0060f924
    MOV ECX,dword ptr [ESI + 0x4]       ; 0060f926
    MOV dword ptr [EDI + 0x4],ECX       ; 0060f929
    MOV ECX,dword ptr [ESI + 0x8]       ; 0060f92c
    MOV dword ptr [EDI + 0x8],ECX       ; 0060f92f
    MOV ECX,dword ptr [ESI + 0xc]       ; 0060f932
    MOV dword ptr [EDI + 0xc],ECX       ; 0060f935
    ADD ESI,0x10                        ; 0060f938
    ADD EDI,0x10                        ; 0060f93b
    PUSH EAX                            ; 0060f93e
    JMP 0x005c04a4                      ; 0060f93f
        ;   XREF to: 005c04a4 (UNCONDITIONAL_JUMP)  ; LAB_005c04a4
    MOV ECX,dword ptr [ESI]             ; 0060f944
        ;   Label: LAB_0060f944
    MOV dword ptr [EDI],ECX             ; 0060f946
    MOV ECX,dword ptr [ESI + 0x4]       ; 0060f948
    MOV dword ptr [EDI + 0x4],ECX       ; 0060f94b
    MOV ECX,dword ptr [ESI + 0x8]       ; 0060f94e
    MOV dword ptr [EDI + 0x8],ECX       ; 0060f951
    MOV ECX,dword ptr [ESI + 0xc]       ; 0060f954
    MOV dword ptr [EDI + 0xc],ECX       ; 0060f957
    ADD ESI,0x10                        ; 0060f95a
    ADD EDI,0x10                        ; 0060f95d
    PUSH EAX                            ; 0060f960
        ;   Label: LAB_0060f960
    JMP 0x005c04cd                      ; 0060f961
        ;   XREF to: 005c04cd (UNCONDITIONAL_JUMP)  ; LAB_005c04cd
    MOV ECX,dword ptr [ESI]             ; 0060f966
        ;   Label: LAB_0060f966
    MOV dword ptr [EDI],ECX             ; 0060f968
    MOV ECX,dword ptr [ESI + 0x4]       ; 0060f96a
    MOV dword ptr [EDI + 0x4],ECX       ; 0060f96d
    MOV ECX,dword ptr [ESI + 0x8]       ; 0060f970
    MOV dword ptr [EDI + 0x8],ECX       ; 0060f973
    MOV ECX,dword ptr [ESI + 0xc]       ; 0060f976
    MOV dword ptr [EDI + 0xc],ECX       ; 0060f979
    ADD ESI,0x10                        ; 0060f97c
    ADD EDI,0x10                        ; 0060f97f
    LEA EAX,[ESP + 0x54]                ; 0060f982
    JMP 0x005c0517                      ; 0060f986
        ;   XREF to: 005c0517 (UNCONDITIONAL_JUMP)  ; LAB_005c0517
    MOV ECX,dword ptr [ESI]             ; 0060f98b
        ;   Label: LAB_0060f98b
    MOV dword ptr [EDI],ECX             ; 0060f98d
    MOV ECX,dword ptr [ESI + 0x4]       ; 0060f98f
    MOV dword ptr [EDI + 0x4],ECX       ; 0060f992
    MOV ECX,dword ptr [ESI + 0x8]       ; 0060f995
    MOV dword ptr [EDI + 0x8],ECX       ; 0060f998
    MOV ECX,dword ptr [ESI + 0xc]       ; 0060f99b
    MOV dword ptr [EDI + 0xc],ECX       ; 0060f99e
    ADD ESI,0x10                        ; 0060f9a1
    ADD EDI,0x10                        ; 0060f9a4
    PUSH EAX                            ; 0060f9a7
    JMP 0x005c054b                      ; 0060f9a8
        ;   XREF to: 005c054b (UNCONDITIONAL_JUMP)  ; LAB_005c054b
    MOV ECX,dword ptr [ESI]             ; 0060f9ad
        ;   Label: LAB_0060f9ad
    MOV dword ptr [EDI],ECX             ; 0060f9af
    MOV ECX,dword ptr [ESI + 0x4]       ; 0060f9b1
    MOV dword ptr [EDI + 0x4],ECX       ; 0060f9b4
    MOV ECX,dword ptr [ESI + 0x8]       ; 0060f9b7
    MOV dword ptr [EDI + 0x8],ECX       ; 0060f9ba
    MOV ECX,dword ptr [ESI + 0xc]       ; 0060f9bd
    MOV dword ptr [EDI + 0xc],ECX       ; 0060f9c0
    ADD ESI,0x10                        ; 0060f9c3
    ADD EDI,0x10                        ; 0060f9c6
    PUSH 0x59ddb0                       ; 0060f9c9
    JMP 0x005c0571                      ; 0060f9ce
        ;   XREF to: 005c0571 (UNCONDITIONAL_JUMP)  ; LAB_005c0571
    MOV ECX,dword ptr [ESI]             ; 0060f9d3
        ;   Label: LAB_0060f9d3
    MOV dword ptr [EDI],ECX             ; 0060f9d5
    MOV ECX,dword ptr [ESI + 0x4]       ; 0060f9d7
    MOV dword ptr [EDI + 0x4],ECX       ; 0060f9da
    MOV ECX,dword ptr [ESI + 0x8]       ; 0060f9dd
    MOV dword ptr [EDI + 0x8],ECX       ; 0060f9e0
    MOV ECX,dword ptr [ESI + 0xc]       ; 0060f9e3
    MOV dword ptr [EDI + 0xc],ECX       ; 0060f9e6
    ADD ESI,0x10                        ; 0060f9e9
    ADD EDI,0x10                        ; 0060f9ec
    PUSH 0x3fc90fdb                     ; 0060f9ef
    JMP 0x005c05ed                      ; 0060f9f4
        ;   XREF to: 005c05ed (UNCONDITIONAL_JUMP)  ; LAB_005c05ed
    MOV ECX,dword ptr [ESI]             ; 0060f9f9
        ;   Label: LAB_0060f9f9
    MOV dword ptr [EDI],ECX             ; 0060f9fb
    MOV ECX,dword ptr [ESI + 0x4]       ; 0060f9fd
    MOV dword ptr [EDI + 0x4],ECX       ; 0060fa00
    MOV ECX,dword ptr [ESI + 0x8]       ; 0060fa03
    MOV dword ptr [EDI + 0x8],ECX       ; 0060fa06
    MOV ECX,dword ptr [ESI + 0xc]       ; 0060fa09
    MOV dword ptr [EDI + 0xc],ECX       ; 0060fa0c
    ADD ESI,0x10                        ; 0060fa0f
    ADD EDI,0x10                        ; 0060fa12
    PUSH EAX                            ; 0060fa15
    JMP 0x005c0642                      ; 0060fa16
        ;   XREF to: 005c0642 (UNCONDITIONAL_JUMP)  ; LAB_005c0642
    MOV ECX,dword ptr [ESI]             ; 0060fa1b
        ;   Label: LAB_0060fa1b
    MOV dword ptr [EDI],ECX             ; 0060fa1d
    MOV ECX,dword ptr [ESI + 0x4]       ; 0060fa1f
    MOV dword ptr [EDI + 0x4],ECX       ; 0060fa22
    MOV ECX,dword ptr [ESI + 0x8]       ; 0060fa25
    MOV dword ptr [EDI + 0x8],ECX       ; 0060fa28
    MOV ECX,dword ptr [ESI + 0xc]       ; 0060fa2b
    MOV dword ptr [EDI + 0xc],ECX       ; 0060fa2e
    ADD ESI,0x10                        ; 0060fa31
    ADD EDI,0x10                        ; 0060fa34
    LEA EAX,[ESP + 0xd4]                ; 0060fa37
    JMP 0x005c066a                      ; 0060fa3e
        ;   XREF to: 005c066a (UNCONDITIONAL_JUMP)  ; LAB_005c066a
    MOV ECX,dword ptr [ESI]             ; 0060fa43
        ;   Label: LAB_0060fa43
    MOV dword ptr [EDI],ECX             ; 0060fa45
    MOV ECX,dword ptr [ESI + 0x4]       ; 0060fa47
    MOV dword ptr [EDI + 0x4],ECX       ; 0060fa4a
    MOV ECX,dword ptr [ESI + 0x8]       ; 0060fa4d
    MOV dword ptr [EDI + 0x8],ECX       ; 0060fa50
    MOV ECX,dword ptr [ESI + 0xc]       ; 0060fa53
    MOV dword ptr [EDI + 0xc],ECX       ; 0060fa56
    ADD ESI,0x10                        ; 0060fa59
    ADD EDI,0x10                        ; 0060fa5c
    MOV ESP,EBP                         ; 0060fa5f
    JMP 0x005c06a2                      ; 0060fa61
        ;   XREF to: 005c06a2 (UNCONDITIONAL_JUMP)  ; LAB_005c06a2

