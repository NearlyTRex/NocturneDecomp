    ...
    for (local_18 = 0; local_18 < (int)g_CDemonSetPtr->actor_list_ptr; local_18 = local_18 + 1) {
      ...
      if ((this_ptr != (CDemonActor *)0x0) &&
         (ABS((in_stack_00000004->location).position.y -
          ...
          (float)2)) {
        core_actor_cpp_CDemonActor_worldToLocalPoint_FUN_00408f10(this_ptr,&CStack_cc,pCStack_24);
        if ((ABS(CStack_cc.z) <= (float)4) && (CStack_cc.z <= 0.0)) {
          (*this_ptr->vtable->getBoundingBox)(this_ptr,&CStack_e4);
          if (CStack_e4.max.x < CStack_cc.x) {
            if (CStack_e4.max.x + 1.0 < CStack_cc.x) goto LAB_005c2488;
            CStack_cc.x = CStack_e4.max.x;
          }
          if (CStack_cc.x < CStack_e4.min.x) {
            if (CStack_cc.x < CStack_e4.min.x + (float)-1) goto LAB_005c2488;
            CStack_cc.x = CStack_e4.min.x;
          }
          ...
          CStack_cc.z = fStack_70 * 0.5f;
          fStack_8c = fStack_74 * 0.5f;
          fStack_88 = CStack_cc.z;
          pCVar4 = core_actor_cpp_CDemonActor_localToWorldPoint_FUN_00408ec0
                             (this_ptr,&CStack_84,&CStack_cc);
          if (&CStack_cc != pCVar4) {
            CStack_cc.x = pCVar4->x;
            CStack_cc.y = pCVar4->y;
            CStack_cc.z = pCVar4->z;
          }
          CStack_cc.y = CStack_cc.y + this_ptr[2].location.position.z;
          ...
          if (fVar5 <= (float10)fVar3) {
            ...
            if (0.0 < pCVar4->z) {
              ...
              CStack_48.x = CStack_cc.x - (in_stack_00000004->location).position.x;
              CStack_48.y = CStack_cc.y - (in_stack_00000004->location).position.y;
              CStack_48.z = CStack_cc.z - (in_stack_00000004->location).position.z;
