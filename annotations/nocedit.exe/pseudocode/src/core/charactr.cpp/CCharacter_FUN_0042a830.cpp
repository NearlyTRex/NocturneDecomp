...
// Convention: __cdecl
// Signature: void __cdecl core_charactr_cpp_CCharacter_FUN_0042a830(CCharacter *this_ptr)
...

{
  ...
  if (this_ptr->is_fully_burned == 0) {
    ...
    fVar1 = this_ptr->burn_alpha - in_stack_00000008;
  ...
  if (0 < this_ptr->fire_count) {
    ...
    do {
      if ((this_ptr->model).part_visibility_flags[*(int *)pSVar10->unk] != 0) {
        ...
        (*(local_40[local_20].base.vtable._ub)->process)(&local_40[local_20].base,in_stack_00000008)
        ;
      }
      pSVar10 = pSVar10 + 1;
  ...
  if ((this_ptr->is_ethereal == 0) && (0 < this_ptr->fire_count)) {
    ...
    do {
      ...
      if (((this_ptr->model).part_visibility_flags[(local_18->motion_list).state_count] != 0) &&
         (fVar1 = (float)local_34 * this_ptr->field64_0x2f18 +
