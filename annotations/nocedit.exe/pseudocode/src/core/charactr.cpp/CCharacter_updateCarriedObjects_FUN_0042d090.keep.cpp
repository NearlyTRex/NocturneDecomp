  ...
  do {
    if (hand->carry_actor != (CDemonActor *)0x0) {
      (*(((this_ptr->base).vtable._uc)->_uc).getCarryObjToBodyXForm)
                (this_ptr,hand_index,&vtable_output);
      CStack_214 = vtable_output;
      if (hand->blend_factor < 1.0) {
        ...
        if (fVar2 <= 1.0) {
          ...
          core_xform_cpp_lerpMatrix3x4_FUN_005f7140
                    (&hand->initial_carry_transform,&CStack_94,hand->blend_factor,&CStack_154);
          core_xform_cpp_multiplyMatrix3x4_FUN_005f4f10
                    (&CStack_184,&bone_matrices[hand->bone_index],&CStack_124);
