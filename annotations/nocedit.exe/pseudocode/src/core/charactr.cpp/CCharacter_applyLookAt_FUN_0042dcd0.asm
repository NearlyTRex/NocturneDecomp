; *****************************************************************************
;                               FUNCTION
; *****************************************************************************
; void __cdecl core_charactr_cpp_CCharacter_applyLookAt_FUN_0042dcd0(CCharacter *this_ptr)
;
; Parameters:
; CCharacter *     Stack[0x4]:4   this_ptr
; Local Variables:
; float[1522]      Stack[-0x1850]:6088  afStackY_1850
; CQuaternion4f    Stack[-0x74]:16  local_74
; CQuaternion4f    Stack[-0x64]:16  local_64
; CQuaternion4f    Stack[-0x54]:16  local_54
; CQuaternion4f    Stack[-0x44]:16  local_44
; CQuaternion4f    Stack[-0x34]:16  local_34
; CQuaternion4f    Stack[-0x24]:16  local_24
; CQuaternion4f    Stack[-0x14]:16  local_14
;
; XREF[1]:
;   core_charactr.cpp_CCharacter_applyGestureLookAt_FUN_0042dfc0 at 0042dfcf
;
; Referenced Globals:
;   TerminatedCString s_core_charactr_cpp_0061746e
;   TerminatedCString s_CCharacter_applyLookAt_n_00617483
;   char* g_CurrentFilename
;   int g_CurrentLineNumber
;
; Called Functions:
;   core_main.c_displayErrorAndQuit_FUN_00506f10
;   core_xform.cpp_multiplyQuaternion_FUN_005f7640
;   core_xform.cpp_quaternionFromAngleX_FUN_005f79b0
;   core_xform.cpp_quaternionFromAngleY_FUN_005f79f0
;   core_xform.cpp_slerpQuaternion_FUN_005f77e0
;
; *****************************************************************************

section .text

    PUSH EBX                            ; 0042dcd0
        ;   Label: core_charactr.cpp_CCharacter_applyLookAt_FUN_0042dcd0
    SUB ESP,0x70                        ; 0042dcd1
    MOV EBX,dword ptr [ESP + 0x78]      ; 0042dcd4
    FLD float ptr [EBX + 0x25e0]        ; 0042dcd8
    FLDZ                                ; 0042dcde
    FCOMPP                              ; 0042dce0
    FNSTSW AX                           ; 0042dce2
    SAHF                                ; 0042dce4
    JNC 0x0042dd9b                      ; 0042dce5
        ;   XREF to: 0042dd9b (CONDITIONAL_JUMP)  ; LAB_0042dd9b
    PUSH EDI                            ; 0042dceb
    PUSH ESI                            ; 0042dcec
    CMP dword ptr [EBX + 0x25e4],0x0    ; 0042dced
    JL 0x0042dda0                       ; 0042dcf4
        ;   XREF to: 0042dda0 (CONDITIONAL_JUMP)  ; LAB_0042dda0
    PUSH dword ptr [EBX + 0x25dc]       ; 0042dcfa
        ;   Label: LAB_0042dcfa
    LEA ESI,[ESP + 0x5c]                ; 0042dd00
    LEA EDI,[ESP + 0x3c]                ; 0042dd04
    CALL core_xform.cpp_quaternionFromAngleX_FUN_005f79b0 ; 0042dd08
        ;   XREF to: 005f79b0 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_quaternionFromAngleX_FUN_005f79b0(float angle_radians, CQuaternion4f * quat_out)
    LEA ESI,[ESP + 0x5c]                ; 0042dd0d
    ADD ESP,0x4                         ; 0042dd11
    JMP 0x0060554f                      ; 0042dd14
        ;   XREF to: 0060554f (UNCONDITIONAL_JUMP)  ; LAB_0060554f
    PUSH ESI                            ; 0042dd1c
        ;   Label: LAB_0042dd1c
    PUSH dword ptr [EBX + 0x25d8]       ; 0042dd1d
    LEA ESI,[ESP + 0x70]                ; 0042dd23
    LEA EDI,[ESP + 0x30]                ; 0042dd27
    CALL core_xform.cpp_quaternionFromAngleY_FUN_005f79f0 ; 0042dd2b
        ;   XREF to: 005f79f0 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_quaternionFromAngleY_FUN_005f79f0(float angle_radians, CQuaternion4f * quat_out)
    LEA ESI,[ESP + 0x70]                ; 0042dd30
    ADD ESP,0x4                         ; 0042dd34
    JMP 0x00605574                      ; 0042dd37
        ;   XREF to: 00605574 (UNCONDITIONAL_JUMP)  ; LAB_00605574
    PUSH ESI                            ; 0042dd3f
        ;   Label: LAB_0042dd3f
    LEA ESI,[ESP + 0x10]                ; 0042dd40
    LEA EDI,[ESP + 0x20]                ; 0042dd44
    CALL core_xform.cpp_multiplyQuaternion_FUN_005f7640 ; 0042dd48
        ;   XREF to: 005f7640 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_multiplyQuaternion_FUN_005f7640(CQuaternion4f * quat1_in, CQuaternion4f * quat2_in, CQuaternion4f * quat_out)
    LEA ESI,[ESP + 0x10]                ; 0042dd4d
    ADD ESP,0x8                         ; 0042dd51
    JMP 0x00605599                      ; 0042dd54
        ;   XREF to: 00605599 (UNCONDITIONAL_JUMP)  ; LAB_00605599
    PUSH dword ptr [EBX + 0x25e0]       ; 0042dd5c
        ;   Label: LAB_0042dd5c
    PUSH ESI                            ; 0042dd62
    MOV ESI,dword ptr [EBX + 0x25e4]    ; 0042dd63
    LEA EAX,[EBX + 0x808]               ; 0042dd69
    SHL ESI,0x4                         ; 0042dd6f
    ADD ESI,EAX                         ; 0042dd72
    PUSH ESI                            ; 0042dd74
    LEA ESI,[ESP + 0x54]                ; 0042dd75
    CALL core_xform.cpp_slerpQuaternion_FUN_005f77e0 ; 0042dd79
        ;   XREF to: 005f77e0 (UNCONDITIONAL_CALL)  ; CQuaternion4f * core_xform.cpp_slerpQuaternion_FUN_005f77e0(CQuaternion4f * quat1_in, CQuaternion4f * quat2_in, float t, CQuaternion4f * quat_out)
    MOV ESI,dword ptr [EBX + 0x25e4]    ; 0042dd7e
    SHL ESI,0x4                         ; 0042dd84
    LEA EDI,[ESI + EBX*0x1 + 0x808]     ; 0042dd87
    LEA ESI,[ESP + 0x54]                ; 0042dd8e
    ADD ESP,0xc                         ; 0042dd92
    JMP 0x0060e54c                      ; 0042dd95
        ;   XREF to: 0060e54c (UNCONDITIONAL_JUMP)  ; LAB_0060e54c
    POP EDI                             ; 0042dd9a
        ;   Label: LAB_0042dd9a
    ADD ESP,0x70                        ; 0042dd9b
        ;   Label: LAB_0042dd9b
    POP EBX                             ; 0042dd9e
    RET                                 ; 0042dd9f
    PUSH EBX                            ; 0042dda0
        ;   Label: LAB_0042dda0
    MOV ECX,0x61746e                    ; 0042dda1 | = "..\\core\\charactr.cpp"
    MOV ESI,0xde5                       ; 0042dda6
    PUSH 0x617483                       ; 0042ddab | = "CCharacter::applyLookAt - never set l..."
    MOV dword ptr [0x02f0ca48],ECX      ; 0042ddb0 | g_CurrentFilename
    MOV dword ptr [0x02f0ca4c],ESI      ; 0042ddb6 | g_CurrentLineNumber
    CALL core_main.c_displayErrorAndQuit_FUN_00506f10 ; 0042ddbc
        ;   XREF to: 00506f10 (UNCONDITIONAL_CALL)  ; void core_main.c_displayErrorAndQuit_FUN_00506f10(char * format)
    ADD ESP,0x8                         ; 0042ddc1
    JMP 0x0042dcfa                      ; 0042ddc4
        ;   XREF to: 0042dcfa (UNCONDITIONAL_JUMP)  ; LAB_0042dcfa
    MOV ECX,dword ptr [ESI]             ; 0060554f
        ;   Label: LAB_0060554f
    MOV dword ptr [EDI],ECX             ; 00605551
    MOV ECX,dword ptr [ESI + 0x4]       ; 00605553
    MOV dword ptr [EDI + 0x4],ECX       ; 00605556
    MOV ECX,dword ptr [ESI + 0x8]       ; 00605559
    MOV dword ptr [EDI + 0x8],ECX       ; 0060555c
    MOV ECX,dword ptr [ESI + 0xc]       ; 0060555f
    MOV dword ptr [EDI + 0xc],ECX       ; 00605562
    ADD ESI,0x10                        ; 00605565
    ADD EDI,0x10                        ; 00605568
    LEA ESI,[ESP + 0x38]                ; 0060556b
    JMP 0x0042dd1c                      ; 0060556f
        ;   XREF to: 0042dd1c (UNCONDITIONAL_JUMP)  ; LAB_0042dd1c
    MOV ECX,dword ptr [ESI]             ; 00605574
        ;   Label: LAB_00605574
    MOV dword ptr [EDI],ECX             ; 00605576
    MOV ECX,dword ptr [ESI + 0x4]       ; 00605578
    MOV dword ptr [EDI + 0x4],ECX       ; 0060557b
    MOV ECX,dword ptr [ESI + 0x8]       ; 0060557e
    MOV dword ptr [EDI + 0x8],ECX       ; 00605581
    MOV ECX,dword ptr [ESI + 0xc]       ; 00605584
    MOV dword ptr [EDI + 0xc],ECX       ; 00605587
    ADD ESI,0x10                        ; 0060558a
    ADD EDI,0x10                        ; 0060558d
    LEA ESI,[ESP + 0x2c]                ; 00605590
    JMP 0x0042dd3f                      ; 00605594
        ;   XREF to: 0042dd3f (UNCONDITIONAL_JUMP)  ; LAB_0042dd3f
    MOV ECX,dword ptr [ESI]             ; 00605599
        ;   Label: LAB_00605599
    MOV dword ptr [EDI],ECX             ; 0060559b
    MOV ECX,dword ptr [ESI + 0x4]       ; 0060559d
    MOV dword ptr [EDI + 0x4],ECX       ; 006055a0
    MOV ECX,dword ptr [ESI + 0x8]       ; 006055a3
    MOV dword ptr [EDI + 0x8],ECX       ; 006055a6
    MOV ECX,dword ptr [ESI + 0xc]       ; 006055a9
    MOV dword ptr [EDI + 0xc],ECX       ; 006055ac
    ADD ESI,0x10                        ; 006055af
    ADD EDI,0x10                        ; 006055b2
    LEA ESI,[ESP + 0x18]                ; 006055b5
    JMP 0x0042dd5c                      ; 006055b9
        ;   XREF to: 0042dd5c (UNCONDITIONAL_JUMP)  ; LAB_0042dd5c
    MOV ECX,dword ptr [ESI]             ; 0060e54c
        ;   Label: LAB_0060e54c
    MOV dword ptr [EDI],ECX             ; 0060e54e
    MOV ECX,dword ptr [ESI + 0x4]       ; 0060e550
    MOV dword ptr [EDI + 0x4],ECX       ; 0060e553
    MOV ECX,dword ptr [ESI + 0x8]       ; 0060e556
    MOV dword ptr [EDI + 0x8],ECX       ; 0060e559
    MOV ECX,dword ptr [ESI + 0xc]       ; 0060e55c
    MOV dword ptr [EDI + 0xc],ECX       ; 0060e55f
    ADD ESI,0x10                        ; 0060e562
    ADD EDI,0x10                        ; 0060e565
    POP ESI                             ; 0060e568
    JMP 0x0042dd9a                      ; 0060e569
        ;   XREF to: 0042dd9a (UNCONDITIONAL_JUMP)  ; LAB_0042dd9a

