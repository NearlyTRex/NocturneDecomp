// Name: core_dmodel.cpp_CKeyFramedModel_exportToS3D_FUN_00479f30
// Address: 00479f30
// Address Range: [[00479f30, 0047a3a3]]
// Convention: __cdecl
// Signature: void __cdecl core_dmodel_cpp_CKeyFramedModel_exportToS3D_FUN_00479f30(CKeyFramedModel *this_ptr,char *filename)

#include "nocturne.h"

void __cdecl core_dmodel_cpp_CKeyFramedModel_exportToS3D_FUN_00479f30(CKeyFramedModel *this_ptr,char *filename)

{
  uint uVar1;
  _FILE *p_Var2;
  _FILE *p_Var5;
  int iVar6;
  SMRGLPrimitiveQuad *pSVar7;
  int iVar3;
  int iVar8;
  char *pcVar9;
  int iVar4;
  int *piVar10;
  int *piVar5;
  byte bVar11;
  int aiStackY_1034 [935];
  char local_14c [256];
  int local_48 [4];
  int local_38;
  uint local_34;
  int local_30;
  int local_2c;
  int local_28;
  int local_24;
  int local_20;
  int local_1c;
  _FILE *local_18;
  int local_14;
  int local_10;
  CVector3i *pCVar1;
  double dVar2;
  double dVar3;
  double dVar4;
  
  bVar11 = 0;
  p_Var5 = shape_memdbg_cpp_openFile_FUN_0050f7a0
                     (filename,(char *)0x0,"wt","..\\core\\dmodel.cpp",0x75b);
  local_18 = p_Var5;
  if (p_Var5 != (_FILE *)0x0) {
    _fprintf(p_Var5,"// .S3D version\n");
    _fprintf(p_Var5,"103\n");
    iVar8 = 0;
    iVar6 = 0;
    if (0 < this_ptr->poly_count) {
      pSVar7 = this_ptr->poly_vert_list;
      do {
        iVar8 = iVar8 + (pSVar7->base).base.count + -2;
        iVar6 = iVar6 + 1;
        pSVar7 = pSVar7 + 1;
      } while (iVar6 < this_ptr->poly_count);
    }
    _fprintf(local_18,"// numTextures,numTris,numVerts,numParts,numFrames,numLights,numCameras\n");
    p_Var2 = local_18;
    _fprintf(local_18,"%d,%d,%d,1,%d,0,0\n",this_ptr->texture_count,iVar8,
               this_ptr->vertex_count,this_ptr->frame_count);
    splitpath(filename,(char *)0x0,(char *)0x0,local_14c,(char *)0x0);
    _fprintf(p_Var2,"// partList: firstVert,numVerts,firstTri,numTris,\"name\"\n");
    _fprintf(p_Var2,"0,%d,0,%d,\"%s\"\n",this_ptr->vertex_count,iVar8,local_14c);
    _fprintf(p_Var2,"// texture list: name\n");
    p_Var2 = local_18;
    iVar3 = 0;
    if (0 < this_ptr->texture_count) {
      pcVar9 = this_ptr->texture_list[0].textures[0].texture_name;
      do {
        _fprintf(p_Var2,"%s\n",pcVar9);
        iVar3 = iVar3 + 1;
        pcVar9 = pcVar9 + 0x48;
      } while (iVar3 < this_ptr->texture_count);
    }
    _fprintf(local_18,"// triList: materialIndex,vertices(index, texX, texY)\n");
    local_20 = 0;
    if (0 < this_ptr->poly_count) {
      local_24 = 0;
      local_28 = 0;
      do {
        local_14 = (int)&(((SMRGLPrimitiveQuad *)(this_ptr->poly_vert_list->vertices + -2))->base).
                         base.type + local_28;
        iVar3 = 2;
        if (2 < *(int *)(local_14 + 4)) {
          local_1c = local_24;
          iVar4 = local_14;
          do {
            piVar10 = (int *)(local_14 + 0x1c + (uint)bVar11 * -8);
            uVar1 = *(uint *)(local_14 + 0x18);
            local_48[(uint)bVar11 * -2] = *piVar10;
            local_48[(uint)bVar11 * -2 + (uint)bVar11 * -2 + 1] = piVar10[(uint)bVar11 * -2 + 1];
            piVar5 = (int *)(iVar4 + 0x28 + (uint)bVar11 * -8);
            local_48[2] = *(int *)(iVar4 + 0x24);
            local_48[(uint)bVar11 * -2 + 3] = *piVar5;
            (&local_38)[(uint)bVar11 * -2 + (uint)bVar11 * -2] = piVar5[(uint)bVar11 * -2 + 1];
            piVar5 = (int *)(iVar4 + 0x34 + (uint)bVar11 * -8);
            local_34 = *(uint *)(iVar4 + 0x30);
            (&local_30)[(uint)bVar11 * -2] = *piVar5;
            (&local_2c)[(uint)bVar11 * -2 + (uint)bVar11 * -2] = piVar5[(uint)bVar11 * -2 + 1];
            local_10 = local_48[0];
            _fprintf(local_18,"%d, %d,%g,%g, %d,%g,%g, %d,%g,%g\n",
                       *(uint *)((int)this_ptr->poly_texture_index_list + local_1c),uVar1,
                       (double)local_48[0] * 1.52587890625e-05,(double)local_48[1] * 1.52587890625e-05,
                       local_48[2],(double)local_48[3] * 1.52587890625e-05,
                       (double)local_38 * 1.52587890625e-05,local_34,
                       SUB84(__BITCAST_UINT64((double)local_30 * 1.52587890625e-05),0),
                       (int)((ulonglong)((double)local_30 * 1.52587890625e-05) >> 0x20),
                       SUB84(__BITCAST_UINT64((double)local_2c * 1.52587890625e-05),0),
                       (int)((ulonglong)((double)local_2c * 1.52587890625e-05) >> 0x20));
            iVar3 = iVar3 + 1;
            iVar4 = iVar4 + 0xc;
          } while (iVar3 < *(int *)(local_14 + 4));
        }
        local_28 = local_28 + 0x48;
        local_24 = local_24 + 4;
        local_20 = local_20 + 1;
      } while (local_20 < this_ptr->poly_count);
    }
    _fprintf(local_18,"// vertList: x,y,z\n");
    iVar3 = 0;
    for (iVar4 = 0; p_Var2 = local_18, iVar4 < this_ptr->vertex_count * this_ptr->frame_count;
        iVar4 = iVar4 + 1) {
      pCVar1 = this_ptr->vertex_list;
      dVar2 = (double)*(int *)((int)&pCVar1->z + iVar3) * 0.00390625;
      dVar3 = (double)*(int *)((int)&pCVar1->y + iVar3) * 0.00390625;
      dVar4 = (double)*(int *)((int)&pCVar1->x + iVar3) * 0.00390625;
      iVar3 = iVar3 + 0xc;
      _fprintf(local_18,"%g,%g,%g\n",SUB84(__BITCAST_UINT64(dVar4),0),(int)((ulonglong)dVar4 >> 0x20),
                 SUB84(__BITCAST_UINT64(dVar3),0),(int)((ulonglong)dVar3 >> 0x20),SUB84(__BITCAST_UINT64(dVar2),0),
                 (int)((ulonglong)dVar2 >> 0x20));
    }
    _fprintf(local_18,"// lightList: \"name\", type, x,y,z, r,g,b, (type-specific info)\n");
    _fprintf(p_Var2,"// cameraList: \"name\", x,y,z, p,b,h, fov(rad)\n");
    shape_memdbg_cpp_closeFile_FUN_0050f9b0(p_Var2,"..\\core\\dmodel.cpp",0x7a0);
    return;
  }
  shape_edittool_cpp_CEditorTools_showError_FUN_0049e740
            (g_CEditorToolsPtr,"Can't create %s",filename);
  return;
}
