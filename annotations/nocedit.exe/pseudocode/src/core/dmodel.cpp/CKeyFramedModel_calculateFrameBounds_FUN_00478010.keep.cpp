...

{
  ...
  if (0 < this_ptr->frame_count) {
    frame_bounds_array = this_ptr->frame_bounds;
    bounds_min_ptr = &this_ptr->bounds;
    frame_index = 0;
    do {
      this_ptr_00 = &frame_bounds_array[frame_index];
      pCVar1 = this_ptr->vertex_list + frame_index * this_ptr->vertex_count;
    ...
    } while (frame_index < this_ptr->frame_count);
  }
  return;
}
