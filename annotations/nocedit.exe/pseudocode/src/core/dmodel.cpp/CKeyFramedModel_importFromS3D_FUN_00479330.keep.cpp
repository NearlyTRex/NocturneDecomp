...
LAB_004797c3:
    ...
    for (iVar8 = 0; iVar8 < this_ptr->poly_count; iVar8 = iVar8 + 1) {
      iVar9 = _fscanf(local_20,"%d, %d,%f,%f, %d,%f,%f, %d,%f,%f\n",&local_44,
                         &vertex_indices[0],&vertex_u[0],&vertex_v[0],
                         &vertex_indices[1],&vertex_u[1],&vertex_v[1],
                         &vertex_indices[2],&vertex_u[2],&vertex_v[2]);
      if (iVar9 != 10) {
        shape_edittool_cpp_CPickList_dtor_FUN_004a3c80(&local_c84,0);
        goto LAB_00479417;
      }
      ...
      for (iVar9 = 0; iVar9 < pSVar2[iVar8].base.base.count; iVar9 = iVar9 + 1) {
        ...
        dVar5 = 65536;
        local_1c = (int)ROUND(ROUND((double)vertex_u[iVar9] * 65536));
        pSVar2[iVar8].vertices[iVar9].texture_u =
             (int)ROUND(ROUND((double)vertex_u[iVar9] * 65536));
        local_1c = (int)ROUND(ROUND(dVar5 * (double)vertex_v[iVar9]));
        pSVar2[iVar8].vertices[iVar9].texture_v =
             (int)ROUND(ROUND(dVar5 * (double)vertex_v[iVar9]));
      }
    }
    wincore_windll_cpp_clearScreen_FUN_005b3e70();
...
LAB_00479c73:
  do {
    ...
    p_Var5 = local_20;
    if (iVar8 != 0) {
      for (uVar11 = (uint)local_14; 0 < (int)uVar11; uVar11 = uVar11 - 1) {
        do {
          iVar8 = _fgetc(p_Var5);
