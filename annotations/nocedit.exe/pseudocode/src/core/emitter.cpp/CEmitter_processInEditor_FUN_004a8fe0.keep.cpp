...
LAB_004a9009:
    ...
    if (g_SlewTargetMode != 0) {
      if (this_ptr != g_EditorSelectedEmitter) {
        ...
        if (pCVar4 != &g_EmitterTarget) {
          g_EmitterTarget = *pCVar4;
        }
        pUVar2 = &(this_ptr->base).orient;
        if (pUVar2 != &g_EmitterEditorOrientation) {
          g_EmitterEditorOrientation.vec = pUVar2->vec;
        }
      }
      ...
      pCVar1->position = g_EmitterTarget;
      if (pUVar2 != &g_EmitterEditorOrientation) {
        pUVar2->vec = g_EmitterEditorOrientation.vec;
      }
      core_actor_cpp_CDemonActor_updateOrientationMatrix_FUN_00408c10(&this_ptr->base);
      core_actor_cpp_CDemonActor_processInEditor_FUN_0040d040(&this_ptr->base);
      pCVar1 = &(this_ptr->base).location;
      if (pCVar1 != (CLocation *)&g_EmitterTarget) {
        g_EmitterTarget = pCVar1->position;
      }
      pUVar2 = &(this_ptr->base).orient;
      if (pUVar2 != &g_EmitterEditorOrientation) {
        g_EmitterEditorOrientation.vec = pUVar2->vec;
  ...
  if ((this_ptr->emitter_size).x < 0.5f) {
