; *****************************************************************************
;                               FUNCTION
; *****************************************************************************
; int __cdecl core_teleport_cpp_CTeleport_renderOpaque_FUN_005dae10(CTeleport *this_ptr)
;
; Parameters:
; CTeleport *      Stack[0x4]:4   this_ptr
;
; Referenced Globals:
;   CDemonRenderer* g_CDemonRendererPtr2 = 02c6d578
;   CDemonMission* g_CDemonMissionPtr = 02f33740
;   CDemonRenderer g_CDemonRendererInstance
;   CDemonMission g_CDemonMissionInstance
;   undefined4 g_CDemonMissionInstance.is_in_editor
;
; Called Functions:
;   core_actor.cpp_CDemonActor_renderBoundingBox_FUN_0040d940
;   engine_drender.cpp_CDemonRenderer_isShadowPass_FUN_0048cae0
;
; *****************************************************************************

section .text

    MOV EAX,[0x0067d550]                ; 005dae10 | g_CDemonMissionInstance | g_CDemonMissionPtr
        ;   Label: core_teleport.cpp_CTeleport_renderOpaque_FUN_005dae10
    CMP dword ptr [EAX + 0x4],0x0       ; 005dae15 | g_CDemonMissionInstance.is_in_editor
    JNZ 0x005dae1e                      ; 005dae19
        ;   XREF to: 005dae1e (CONDITIONAL_JUMP)  ; LAB_005dae1e
    XOR EAX,EAX                         ; 005dae1b
        ;   Label: LAB_005dae1b
    RET                                 ; 005dae1d
    MOV ECX,dword ptr [0x006703ec]      ; 005dae1e | g_CDemonRendererInstance | g_CDemonRendererPtr2
        ;   Label: LAB_005dae1e
    PUSH ECX                            ; 005dae24 | g_CDemonRendererInstance
    CALL engine_drender.cpp_CDemonRenderer_isShadowPass_FUN_0048cae0 ; 005dae25
        ;   XREF to: 0048cae0 (UNCONDITIONAL_CALL)  ; int engine_drender.cpp_CDemonRenderer_isShadowPass_FUN_0048cae0(CDemonRenderer * this_ptr)
    ADD ESP,0x4                         ; 005dae2a
    TEST EAX,EAX                        ; 005dae2d
    JNZ 0x005dae1b                      ; 005dae2f
        ;   XREF to: 005dae1b (CONDITIONAL_JUMP)  ; LAB_005dae1b
    PUSH EBX                            ; 005dae31
    PUSH 0xfb                           ; 005dae32
    MOV EBX,dword ptr [ESP + 0xc]       ; 005dae37
    PUSH EBX                            ; 005dae3b
    CALL core_actor.cpp_CDemonActor_renderBoundingBox_FUN_0040d940 ; 005dae3c
        ;   XREF to: 0040d940 (UNCONDITIONAL_CALL)  ; void core_actor.cpp_CDemonActor_renderBoundingBox_FUN_0040d940(CDemonActor * this_ptr, uint32_t color)
    ADD ESP,0x8                         ; 005dae41
    XOR EAX,EAX                         ; 005dae44
    POP EBX                             ; 005dae46
    RET                                 ; 005dae47

