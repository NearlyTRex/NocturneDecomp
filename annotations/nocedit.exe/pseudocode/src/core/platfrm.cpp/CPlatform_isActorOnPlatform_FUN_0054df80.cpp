...

{
  ...
  if (actor != (CDemonActor *)0x0) {
    ...
    if (EVar1 == COLLISION_TYPE_CYLINDER) {
      ...
      fVar1 = (this_ptr->base).orient.vec.z;
      (this_ptr->base).orient.vec.x = fVar2;
      fVar2 = core_actor_cpp_normalizeAngleToPi_FUN_0040cd70(fVar1);
      fVar1 = (this_ptr->base).orient.vec.x;
      (this_ptr->base).orient.vec.z = fVar2;
      if ((ABS(fVar1) <= (float)0.01) &&
        ...
        if (((CStack_60.min.x <= CStack_24.x + local_88.cylinder_radius) &&
          ...
          if (pCVar2->collision_triangle_list != (CDemonTriangle *)0x0) {
            ...
            fVar3 = core_dmodel_cpp_CKeyFramedModel_intersectRay_FUN_004781d0
                              (this_ptr_00,frame_index,ray_origin,ray_direction,output_normal);
            if (fVar3 < 0.0) {
              return 0;
            }
            if (1.0 < fVar3) {
