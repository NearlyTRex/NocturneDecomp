...
// Address Range: [[004507a0, 004509a2] [0060c80c, 0060c82b]]
// Convention: __cdecl
...

{
  ...
  pcVar1 = g_CoronaBlurOutputBuffer[scanline_y] + x_start;
  piVar3 = g_CoronaDepthBuffer[scanline_y] + x_start;
  puVar2 = g_ZBufferScanlineArray[scanline_y << (g_CameraDownscaleIterations.bytes[0] & 0x1f)] +
  ...
  for (; x_start < x_end; x_start = x_start + 1) {
    if ((((*puVar2 < (uint)*piVar3) &&
        ...
        (core_dcamera_cpp_CDemonCamera_worldToScreenWithFrustumCull_FUN_0044d7d0
                   (&g_CurrentLightForCorona->base,input_ptr,&local_34), 0 < local_34.z)) &&
       ((uVar1 = (uint)g_CurrentLightForCorona->shadow_depth_buffer
                       [(local_34.x >> 0x10) +
                        (local_34.y >> 0x10) * g_CurrentLightForCorona->shadow_map_width],
        (int)uVar1 < g_CoronaMaxDepth && (local_34.z < (int)(uVar1 + 0x80))))) {
      *pcVar1 = *pcVar1 + (char)((int)((g_CoronaMaxDepth - uVar1) *
                                      (uint)g_CurrentLightForCorona->precomputed_lighting_textures
                                            [((local_34.y >>
                                             ...
                                             0x1f)) + (g_CurrentLightForCorona->texture_coord_mask &
                                                      local_34.x >>
                                                      ((byte)g_CurrentLightForCorona->shadow_x_shift
                                                      & 0x1f))]) >>
