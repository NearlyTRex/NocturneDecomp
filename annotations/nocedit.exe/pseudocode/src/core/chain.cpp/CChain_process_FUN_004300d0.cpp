...

{
  ...
  if (this_ptr->is_visible != 0) {
    ...
    local_ec = 0.0;
    for (iVar6 = 0; pSVar18 = pSVar18 + 1, iVar6 < this_ptr->vertex_count + -1; iVar6 = iVar6 + 1) {
      fVar13 = pSVar18[-1].position.x - (pSVar18->position).x;
      fVar8 = pSVar18[-1].position.y - (pSVar18->position).y;
      fVar9 = pSVar18[-1].position.z - (pSVar18->position).z;
  ...
  this_ptr->vertices[iVar14 + -1].velocity.y = this_ptr->vertices[iVar14 + -1].velocity.z;
  this_ptr->vertices[iVar14 + -1].velocity.x = this_ptr->vertices[iVar14 + -1].velocity.y;
  ...
  do {
    ...
    if (pSVar17 != pSVar4) {
      (pSVar17->position).x = (pSVar4->position).x;
      (pSVar17->position).y = (pSVar4->position).y;
      (pSVar17->position).z = (pSVar4->position).z;
    }
    iVar14 = this_ptr->vertex_count + -1;
    pSVar18[iVar14].velocity.z = 0.0;
    pSVar18[iVar14].velocity.y = pSVar18[iVar14].velocity.z;
    pSVar18[iVar14].velocity.x = pSVar18[iVar14].velocity.y;
    if (pSVar18 != (SChainVertex *)pCVar4) {
      (pSVar18->position).x = (pCVar4->position).x;
      this_ptr->vertices[0].position.y = (this_ptr->base).location.position.y;
      this_ptr->vertices[0].position.z = (this_ptr->base).location.position.z;
    }
    this_ptr->vertices[0].velocity.z = 0.0;
    local_1c = 1;
    this_ptr->vertices[0].velocity.y = this_ptr->vertices[0].velocity.z;
    pCVar20->x = this_ptr->vertices[0].velocity.y;
  ...
  if (0 < this_ptr->vertex_count) {
    ...
    do {
      ...
      if ((SChainVertex *)pCVar20 != pSVar18) {
        pCVar20->x = (pSVar18->position).x;
        pCVar20->y = (pSVar18->position).y;
        pCVar20->z = (pSVar18->position).z;
      }
      iVar14 = iVar14 + 1;
      pCVar20 = pCVar20 + 3;
  ...
  if (this_ptr->is_visible != 0) {
    if (local_ec < this_ptr->chain_length * (float)0.90000000000000002) {
      pSVar18 = this_ptr->vertices;
      local_f0 = 0.0;
      for (iVar14 = 0; pSVar18 = pSVar18 + 1, iVar14 < this_ptr->vertex_count + -1;
          iVar14 = iVar14 + 1) {
        local_c4 = pSVar18[-1].position.x - (pSVar18->position).x;
        local_c0 = pSVar18[-1].position.y - (pSVar18->position).y;
        local_bc = pSVar18[-1].position.z - (pSVar18->position).z;
