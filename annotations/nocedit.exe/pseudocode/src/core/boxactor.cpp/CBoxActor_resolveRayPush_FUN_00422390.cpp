...

{
  ...
  if (local_24.z * local_48.z + local_24.x * local_48.x + local_24.y * local_48.y < 0.0) {
    ...
    if ((0.0 <= fVar5) && (fVar5 < 1.0)) {
      ...
      (pCVar1->position).x = (pCVar1->position).x + ray_direction->x * fVar5;
      fVar3 = (this_ptr->base).location.position.z;
      (this_ptr->base).location.position.y = fVar2 + fVar1 * fVar5;
      (this_ptr->base).location.position.z = fVar3 + fVar4 * fVar5;
