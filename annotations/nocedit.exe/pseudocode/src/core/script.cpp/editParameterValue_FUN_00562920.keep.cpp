...

{
  ...
  case 0:
    ...
    if (iVar5 != 0) {
      _sprintf(value_buffer,"%d",local_30);
      return 1;
    }
    break;
  case 1:
    iVar5 = sscanf(value_buffer,"%f",&local_3758);
  ...
  case 0x17:
    ...
    if (pCVar6 != (CMotionList *)0x0) {
      ...
      if (0 < pCVar6->motion_count) {
        ...
        str1 = pCVar6->motions;
        do {
          _sprintf(local_6a0,"%s\t%s",str1->motion_name,(char *)local_24);
  ...
  case 0x19:
    ...
    for (iVar8 = 0; iVar8 < g_CScriptPtr->dialog_entry_count; iVar8 = iVar8 + 1) {
      ...
      _sprintf(g_ScriptFormatBuffer,"%s\t%s\t%s",pcVar9,pcVar9 + 0x78,pcVar9 + 0x96);
      iVar5 = iVar5 + 0x226;
