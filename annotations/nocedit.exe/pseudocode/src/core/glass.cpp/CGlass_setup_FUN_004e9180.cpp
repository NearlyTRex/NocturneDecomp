...

{
  ...
  local_14 = (int)ROUND(ROUND((this_ptr->glass_size).y * (float)0.5));
  this_ptr->grid_rows = local_14 + 1;
  if (4 < local_14 + 1) {
    ...
    for (iVar7 = 0; iVar7 < this_ptr->grid_cols + 1; iVar7 = iVar7 + 1) {
      ...
      *(int *)(pcVar9 + 0x5e4) =
           (int)ROUND(ROUND(((double)iVar7 / (double)this_ptr->grid_cols) * 16515072 +
                            131072));
      local_14 = (int)ROUND(ROUND(dVar3 + ((double)(this_ptr->grid_rows - local_20) /
                                          (double)this_ptr->grid_rows) * dVar2));
      pCVar5 = pCVar5 + 1;
      *(int *)(pcVar9 + 0x648) = local_14;
      iVar4 = iVar4 + 1;
  ...
  if (0 < this_ptr->grid_rows) {
    local_2c = this_ptr->broken_quads;
    do {
      iVar4 = 0;
      if (0 < this_ptr->grid_cols) {
        pSVar6 = local_2c + local_18;
        do {
          (pSVar6->base).base.count = 4;
          (pSVar6->base).surface_normal.D = 0;
          iVar7 = (pSVar6->base).surface_normal.D;
          (pSVar6->base).surface_normal.C = iVar7;
          (pSVar6->base).surface_normal.B = iVar7;
          (pSVar6->base).surface_normal.A = iVar7;
          iVar7 = (this_ptr->grid_cols + 1) * local_1c + iVar4;
  ...
  iVar4 = stricmp(this_ptr->break_event,"none");
  if (iVar4 == 0) {
  ...
  if (this_ptr->background_flag != 0) {
    ...
    iVar4 = strcmp(pcVar9,"7YEARS.RAW");
    if (iVar4 == 0) {
      iVar4 = stricmp
