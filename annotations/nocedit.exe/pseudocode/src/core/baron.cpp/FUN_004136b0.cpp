// Name: core_baron.cpp_FUN_004136b0
// Address: 004136b0
// Address Range: [[004136b0, 004139f7]]
// Convention: unknown
// Signature: undefined core_baron.cpp_FUN_004136b0()

#include "nocturne.h"

/* Signature: byte actors_hero_baron.cpp_FUN_004136b0(uint param_1) */

void core_baron_cpp_FUN_004136b0(void)

{
  CDemonActor *this_ptr;
  CVector3f *pCVar1;
  float fVar2;
  int iVar3;
  BADSPACEBASE *in_ESP;
  int unaff_EDI;
  CDemonActor *in_stack_00000004;
  float in_stack_fffffeb4;
  SDamageInfo local_d0;
  CDemonActor *pCStack_94;
  byte auStack_90 [32];
  CVector3f CStack_70;
  byte auStack_5c [12];
  byte auStack_50 [8];
  float fStack_48;
  float fStack_44;
  CVector3f local_3c [2];
  CVector3f CStack_20;
  
  this_ptr = (CDemonActor *)core_hero_cpp_FUN_004f3960();
  if (this_ptr == (CDemonActor *)0x0) {
    return;
  }
  core_actor_cpp_CDemonActor_worldToLocalPoint_FUN_00408f10
            (in_stack_00000004,(CVector3f *)(auStack_5c + 8),&(this_ptr->location).position);
  pCVar1 = core_vehicle_cpp_convertDirectionVectorToEulerAngles_FUN_005e7830
                     ((CVector3f *)(auStack_90 + 4),(CVector3f *)auStack_50);
  if ((CVector3f *)(auStack_50 + 4) != pCVar1) {
    auStack_50._4_4_ = pCVar1->x;
    fStack_48 = pCVar1->y;
    fStack_44 = pCVar1->z;
  }
  fStack_44 = core_actor_cpp_normalizeAngleToPi_FUN_0040cd70(fStack_48);
  CStack_20.y = fStack_44;
  if ((fStack_44 < (float)-0.78539816337500001) || (0.78539816337500001 < (double)fStack_44)) {
    core_fire_cpp_CFireEffect_FUN_004c8f50(g_CFireEffectPtr);
    core_fire_cpp_CFireEffect_FUN_004c8ea0(g_CFireEffectPtr);
  }
  else {
    crt_memory_c_constructObjectArray_DefaultCtor_FUN_005fe667
              (&stack0xfffffeb0,10,&g_CVectorTypeInfo);
    fVar2 = (float)(*this_ptr->vtable->getTargetPoints)(this_ptr,(CVector3f *)&stack0xfffffeb4);
    CStack_20.z = fVar2;
    iVar3 = crt_stdlib_c_rand_FUN_005feb5c();
    pCVar1 = core_actor_cpp_CDemonActor_localToWorldPoint_FUN_00408ec0
                       (this_ptr,(CVector3f *)auStack_90,
                        (CVector3f *)(&stack0xfffffebc + (iVar3 % (int)fVar2) * 0xc));
    if (&CStack_20 != pCVar1) {
      CStack_20.x = pCVar1->x;
      CStack_20.y = pCVar1->y;
      CStack_20.z = pCVar1->z;
    }
    iVar3 = crt_stdlib_c_rand_FUN_005feb5c();
    pCVar1 = core_actor_cpp_CDemonActor_localToWorldPoint_FUN_00408ec0
                       (this_ptr,(CVector3f *)(auStack_50 + 4),
                        (CVector3f *)(&stack0xfffffec4 + (iVar3 % unaff_EDI) * 0xc));
    if (local_3c != pCVar1) {
      local_3c[0].x = pCVar1->x;
      local_3c[0].y = pCVar1->y;
      local_3c[0].z = pCVar1->z;
    }
    pCVar1 = core_xform_cpp_transformVector3x4_FUN_005f4dc0
                       ((CVector3f *)(auStack_5c + 8),&g_ZeroVector,
                        (CMatrix3x4f *)&(&in_stack_00000004[0xb].scale)[DAT_008224cc * 4].z);
    core_actor_cpp_CDemonActor_localToWorldPoint_FUN_00408ec0
              (in_stack_00000004,(CVector3f *)auStack_5c,pCVar1);
    pCVar1 = core_xform_cpp_transformVector3x4_FUN_005f4dc0
                       (&CStack_70,&g_ZeroVector,
                        (CMatrix3x4f *)&(&in_stack_00000004[0xb].scale)[DAT_008224d0 * 4].z);
    core_actor_cpp_CDemonActor_localToWorldPoint_FUN_00408ec0
              (in_stack_00000004,(CVector3f *)&CStack_20.z,pCVar1);
    in_stack_fffffeb4 = 5.990007e-39;
    core_fire_cpp_CFireEffect_FUN_004c8fd0(g_CFireEffectPtr);
    core_fire_cpp_CFireEffect_FUN_004c8fd0(g_CFireEffectPtr);
    (*in_stack_00000004->vtable->playSound)(in_stack_00000004,"baron-attack.wav");
  }
  core_charactr_cpp_SDamageInfo_ctor_FUN_00427db0(&local_d0);
  local_d0.attacker = (CDemonActor *)0x6;
  local_d0.damage_flags = 0x42c80000;
  local_d0.wielder = in_stack_00000004;
  pCStack_94 = *(CDemonActor **)(in_stack_00000004[0x17a].create_event + 0x44);
  if (pCStack_94 == (CDemonActor *)0x0) {
    pCStack_94 = in_stack_00000004;
  }
  (*this_ptr->vtable[1].playAmbientSoundWithVolume)
            (this_ptr,(char *)&local_d0.damage_amount,in_stack_fffffeb4);
  this_ptr[0x88].field22_0x120 = 0x40400000;
  return;
}
