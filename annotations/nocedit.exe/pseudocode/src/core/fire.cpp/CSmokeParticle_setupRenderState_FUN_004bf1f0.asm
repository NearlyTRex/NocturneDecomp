; *****************************************************************************
;                               FUNCTION
; *****************************************************************************
; void __cdecl core_fire_cpp_CSmokeParticle_setupRenderState_FUN_004bf1f0(CSmokeParticle *this_ptr)
;
; Parameters:
; CSmokeParticle * Stack[0x4]:4   this_ptr
; Local Variables:
; float[997]       Stack[-0x1000]:3988  afStackY_1000
; float            Stack[-0x60]:4  local_60
; float            Stack[-0x54]:4  local_54
; float            Stack[-0x48]:4  local_48
; undefined1       Stack[-0x40]:1  local_40
; CVector3i        Stack[-0x18]:12  local_18
;
; XREF[1]:
;   core_fire.cpp_CFireEffect_render_FUN_004c7180 at 004c72a7
;
; Referenced Globals:
;   double DOUBLE_00629c03 = -65536
;   CDemonRenderer* g_CDemonRendererPtr2 = 02c6d578
;   CDemonCamera* g_CurrentSceneCamera
;   CDemonRenderer g_CDemonRendererInstance
;   CVector3i g_BillboardCameraRight
;   undefined4 g_BillboardCameraRight.y
;   CVector3i g_BillboardCameraUp
;   undefined4 g_BillboardCameraUp.y
;   undefined4 g_BillboardCameraUp.z
;   undefined4 g_BillboardPrimitive.base.base.count
;   undefined4 g_BillboardPrimitive.base.surface_normal.A
;   undefined4 g_BillboardPrimitive.base.surface_normal.B
;   undefined4 g_BillboardPrimitive.base.surface_normal.C
;   undefined4 g_BillboardPrimitive.base.surface_normal.D
;   undefined4 g_BillboardPrimitive.vertices[0]
;   ... and 3 more
;
; Called Functions:
;   crt_math.c_round_FUN_005fe6b0
;   engine_drender.cpp_CDemonRenderer_getCameraRotationFixed_FUN_0048c7e0
;
; *****************************************************************************

section .text

    PUSH EBX                            ; 004bf1f0
        ;   Label: core_fire.cpp_CSmokeParticle_setupRenderState_FUN_004bf1f0
    PUSH ESI                            ; 004bf1f1
    PUSH EDI                            ; 004bf1f2
    SUB ESP,0x5c                        ; 004bf1f3
    MOV EDX,0x4                         ; 004bf1f6
    MOV EAX,0x1                         ; 004bf1fb
    LEA ESI,[ESP + 0x50]                ; 004bf200
    MOV EBX,dword ptr [0x006703ec]      ; 004bf204 | g_CDemonRendererInstance | g_CDemonRendererPtr2
    MOV EDI,0x2d12db4                   ; 004bf20a | g_BillboardCameraRight
    XOR ECX,ECX                         ; 004bf20f
    MOV dword ptr [0x02d12dd0],EDX      ; 004bf211 | g_BillboardPrimitive.base.base.count
    MOV dword ptr [0x02d12dd4],ECX      ; 004bf217 | g_BillboardPrimitive.base.surface_normal.A
    MOV dword ptr [0x02d12dd8],ECX      ; 004bf21d | g_BillboardPrimitive.base.surface_normal.B
    MOV dword ptr [0x02d12ddc],ECX      ; 004bf223 | g_BillboardPrimitive.base.surface_normal.C
    MOV dword ptr [0x02d12de0],ECX      ; 004bf229 | g_BillboardPrimitive.base.surface_normal.D
    MOV dword ptr [0x02d12de4],ECX      ; 004bf22f | g_BillboardPrimitive.vertices[0]
    MOV [0x02d12de8],EAX                ; 004bf235 | g_BillboardPrimitive.vertices[1]
    PUSH EBX                            ; 004bf23a | g_CDemonRendererInstance
    MOV EDX,0x2                         ; 004bf23b
    MOV ECX,0x3                         ; 004bf240
    MOV dword ptr [0x02d12dec],EDX      ; 004bf245 | g_BillboardPrimitive.vertices[2]
    MOV dword ptr [0x02d12df0],ECX      ; 004bf24b | g_BillboardPrimitive.vertices[3]
    CALL engine_drender.cpp_CDemonRenderer_getCameraRotationFixed_FUN_0048c7e0 ; 004bf251
        ;   XREF to: 0048c7e0 (UNCONDITIONAL_CALL)  ; CVector3i * engine_drender.cpp_CDemonRenderer_getCameraRotationFixed_FUN_0048c7e0(CDemonRenderer * this_ptr, CVector3i * output)
    LEA ESI,[ESP + 0x54]                ; 004bf256
    ADD ESP,0x4                         ; 004bf25a
    JMP 0x00610903                      ; 004bf25d
        ;   XREF to: 00610903 (UNCONDITIONAL_JUMP)  ; LAB_00610903
    MOV dword ptr [0x02d12db8],ESI      ; 004bf262 | g_BillboardCameraRight.y
        ;   Label: LAB_004bf262
    MOV ESI,dword ptr [0x00823a74]      ; 004bf268 | g_CurrentSceneCamera
    JMP 0x03fc553f                      ; 004bf26e
        ;   XREF to: 03fc553f (UNCONDITIONAL_JUMP)  ; LAB_03fc553f
    JMP 0x03fc5592                      ; 004bf27c
        ;   XREF to: 03fc5592 (UNCONDITIONAL_JUMP)  ; LAB_03fc5592
        ;   Label: LAB_004bf27c
    FLD double ptr [0x00629c03]         ; 004bf289 | DOUBLE_00629c03
        ;   Label: LAB_004bf289
    FLD float ptr [ESP + 0x8]           ; 004bf28f
    FMUL ST1                            ; 004bf293
    FLD float ptr [ESP + 0x14]          ; 004bf295
    FMUL ST2                            ; 004bf299
    FLD float ptr [ESP + 0x20]          ; 004bf29b
    FMULP ST3                           ; 004bf29f
    FXCH                                ; 004bf2a1
    CALL crt_math.c_round_FUN_005fe6b0  ; 004bf2a3
        ;   XREF to: 005fe6b0 (UNCONDITIONAL_CALL)  ; double crt_math.c_round_FUN_005fe6b0(double value)
    FXCH                                ; 004bf2a8
    CALL crt_math.c_round_FUN_005fe6b0  ; 004bf2aa
        ;   XREF to: 005fe6b0 (UNCONDITIONAL_CALL)  ; double crt_math.c_round_FUN_005fe6b0(double value)
    FXCH ST2                            ; 004bf2af
    CALL crt_math.c_round_FUN_005fe6b0  ; 004bf2b1
        ;   XREF to: 005fe6b0 (UNCONDITIONAL_CALL)  ; double crt_math.c_round_FUN_005fe6b0(double value)
    FXCH                                ; 004bf2b6
    FISTP dword ptr [0x02d12dc0]        ; 004bf2b8 | g_BillboardCameraUp
    FXCH                                ; 004bf2be
    FISTP dword ptr [0x02d12dc4]        ; 004bf2c0 | g_BillboardCameraUp.y
    FISTP dword ptr [0x02d12dc8]        ; 004bf2c6 | g_BillboardCameraUp.z
    ADD ESP,0x5c                        ; 004bf2cc
    POP EDI                             ; 004bf2cf
    POP ESI                             ; 004bf2d0
    POP EBX                             ; 004bf2d1
    RET                                 ; 004bf2d2
    MOV ECX,dword ptr [ESI]             ; 00610903
        ;   Label: LAB_00610903
    MOV dword ptr [EDI],ECX             ; 00610905
    MOV ECX,dword ptr [ESI + 0x4]       ; 00610907
    MOV dword ptr [EDI + 0x4],ECX       ; 0061090a
    MOV ECX,dword ptr [ESI + 0x8]       ; 0061090d
    MOV dword ptr [EDI + 0x8],ECX       ; 00610910
    ADD ESI,0xc                         ; 00610913
    ADD EDI,0xc                         ; 00610916
    XOR ESI,ESI                         ; 00610919
    JMP 0x004bf262                      ; 0061091b
        ;   XREF to: 004bf262 (UNCONDITIONAL_JUMP)  ; LAB_004bf262
    MOV ECX,0xa                         ; 03fc553f
        ;   Label: LAB_03fc553f
    LEA EDI,[ESP + 0x28]                ; 03fc5544
    LEA ESI,[ESI + 0x10]                ; 03fc5548
    MOV ECX,dword ptr [ESI]             ; 03fc554b
    MOV dword ptr [EDI],ECX             ; 03fc554d
    MOV ECX,dword ptr [ESI + 0x4]       ; 03fc554f
    MOV dword ptr [EDI + 0x4],ECX       ; 03fc5552
    MOV ECX,dword ptr [ESI + 0x8]       ; 03fc5555
    MOV dword ptr [EDI + 0x8],ECX       ; 03fc5558
    MOV ECX,dword ptr [ESI + 0xc]       ; 03fc555b
    MOV dword ptr [EDI + 0xc],ECX       ; 03fc555e
    MOV ECX,dword ptr [ESI + 0x10]      ; 03fc5561
    MOV dword ptr [EDI + 0x10],ECX      ; 03fc5564
    MOV ECX,dword ptr [ESI + 0x14]      ; 03fc5567
    MOV dword ptr [EDI + 0x14],ECX      ; 03fc556a
    MOV ECX,dword ptr [ESI + 0x18]      ; 03fc556d
    MOV dword ptr [EDI + 0x18],ECX      ; 03fc5570
    MOV ECX,dword ptr [ESI + 0x1c]      ; 03fc5573
    MOV dword ptr [EDI + 0x1c],ECX      ; 03fc5576
    MOV ECX,dword ptr [ESI + 0x20]      ; 03fc5579
    MOV dword ptr [EDI + 0x20],ECX      ; 03fc557c
    MOV ECX,dword ptr [ESI + 0x24]      ; 03fc557f
    MOV dword ptr [EDI + 0x24],ECX      ; 03fc5582
    ADD ESI,0x28                        ; 03fc5585
    ADD EDI,0x28                        ; 03fc5588
    XOR ECX,ECX                         ; 03fc558b
    JMP 0x004bf27c                      ; 03fc558d
        ;   XREF to: 004bf27c (UNCONDITIONAL_JUMP)  ; LAB_004bf27c
    MOV ECX,0xa                         ; 03fc5592
        ;   Label: LAB_03fc5592
    MOV EDI,ESP                         ; 03fc5597
    LEA ESI,[ESP + 0x28]                ; 03fc5599
    MOV ECX,dword ptr [ESI]             ; 03fc559d
    MOV dword ptr [EDI],ECX             ; 03fc559f
    MOV ECX,dword ptr [ESI + 0x4]       ; 03fc55a1
    MOV dword ptr [EDI + 0x4],ECX       ; 03fc55a4
    MOV ECX,dword ptr [ESI + 0x8]       ; 03fc55a7
    MOV dword ptr [EDI + 0x8],ECX       ; 03fc55aa
    MOV ECX,dword ptr [ESI + 0xc]       ; 03fc55ad
    MOV dword ptr [EDI + 0xc],ECX       ; 03fc55b0
    MOV ECX,dword ptr [ESI + 0x10]      ; 03fc55b3
    MOV dword ptr [EDI + 0x10],ECX      ; 03fc55b6
    MOV ECX,dword ptr [ESI + 0x14]      ; 03fc55b9
    MOV dword ptr [EDI + 0x14],ECX      ; 03fc55bc
    MOV ECX,dword ptr [ESI + 0x18]      ; 03fc55bf
    MOV dword ptr [EDI + 0x18],ECX      ; 03fc55c2
    MOV ECX,dword ptr [ESI + 0x1c]      ; 03fc55c5
    MOV dword ptr [EDI + 0x1c],ECX      ; 03fc55c8
    MOV ECX,dword ptr [ESI + 0x20]      ; 03fc55cb
    MOV dword ptr [EDI + 0x20],ECX      ; 03fc55ce
    MOV ECX,dword ptr [ESI + 0x24]      ; 03fc55d1
    MOV dword ptr [EDI + 0x24],ECX      ; 03fc55d4
    ADD ESI,0x28                        ; 03fc55d7
    ADD EDI,0x28                        ; 03fc55da
    XOR ECX,ECX                         ; 03fc55dd
    JMP 0x004bf289                      ; 03fc55df
        ;   XREF to: 004bf289 (UNCONDITIONAL_JUMP)  ; LAB_004bf289

