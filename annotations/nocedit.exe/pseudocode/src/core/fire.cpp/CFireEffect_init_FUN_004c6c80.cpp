...

{
  ...
  do {
    ...
    pCVar1 = pCVar2;
  } while (pCVar2 != g_SparkPool + sizeof(g_SparkPool) / sizeof(g_SparkPool[0]));
  pCVar3 = g_MuzzleFlashPool;
  do {
    pCVar3->frames_remaining = 0;
    pCVar3 = pCVar3 + 1;
  } while (pCVar3 != g_MuzzleFlashPool + sizeof(g_MuzzleFlashPool) / sizeof(g_MuzzleFlashPool[0]));
  ...
  do {
    ...
    pCVar4 = pCVar5;
  } while (pCVar5 != g_GlassParticlePool + sizeof(g_GlassParticlePool) / sizeof(g_GlassParticlePool[0]));
  pCVar6 = g_BulletTrailPool;
  do {
    pCVar7 = pCVar6 + 1;
    pCVar6->frames_remaining = 0;
    pCVar6 = pCVar7;
  } while (pCVar7 != g_BulletTrailPool + sizeof(g_BulletTrailPool) / sizeof(g_BulletTrailPool[0]));
  ...
  do {
    ...
    pCVar8 = pCVar9;
  } while (pCVar9 != g_FireballPool + sizeof(g_FireballPool) / sizeof(g_FireballPool[0]));
  g_RockAllocIndex = 0;
  pCVar10 = g_RockPool;
  do {
    pCVar11 = pCVar10 + 1;
    ...
    pCVar10 = pCVar11;
  } while (pCVar11 != g_RockPool + sizeof(g_RockPool) / sizeof(g_RockPool[0]));
  g_ExplosionPoolIndex = 0;
  this_ptr_01 = g_ExplosionPool;
  do {
    core_fire_cpp_CExplosion_ctor_FUN_004c38c0(this_ptr_01);
    this_ptr_01 = this_ptr_01 + 1;
  } while (this_ptr_01 != g_ExplosionPool + sizeof(g_ExplosionPool) / sizeof(g_ExplosionPool[0]));
  this_ptr_02 = g_TossPool;
  g_TossAllocIndex = 0;
  do {
    core_fire_cpp_CToss_reset_FUN_004c3ed0(this_ptr_02);
    this_ptr_02 = this_ptr_02 + 1;
  } while (this_ptr_02 != g_TossPool + sizeof(g_TossPool) / sizeof(g_TossPool[0]));
  ...
  } while (this_ptr_03 != g_CraterPool + sizeof(g_CraterPool) / sizeof(g_CraterPool[0]));
  ...
  } while (this_ptr_04 != g_GunFlamePool + sizeof(g_GunFlamePool) / sizeof(g_GunFlamePool[0]));
  ...
  } while (this_ptr_05 != g_LightningBoltPool + sizeof(g_LightningBoltPool) / sizeof(g_LightningBoltPool[0]));
  this_ptr_06 = g_TrailPool;
  g_TrailAllocIndex = 0;
  do {
    core_fire_cpp_CTrail_reset_FUN_004c5de0(this_ptr_06);
    this_ptr_06 = this_ptr_06 + 1;
  } while (this_ptr_06 != g_TrailPool + sizeof(g_TrailPool) / sizeof(g_TrailPool[0]));
  g_ShellAllocIndex = 0;
  pCVar12 = g_ShellPool;
  do {
    pCVar13 = pCVar12 + 1;
    ...
    pCVar12 = pCVar13;
  } while (pCVar13 != g_ShellPool + sizeof(g_ShellPool) / sizeof(g_ShellPool[0]));
  ...
  do {
    ...
    pCVar14 = pCVar15;
  } while (pCVar15 != g_PopcornPool + sizeof(g_PopcornPool) / sizeof(g_PopcornPool[0]));
  ...
  do {
    ...
    pCVar16 = pCVar17;
  } while (pCVar17 != g_RainDropPool + sizeof(g_RainDropPool) / sizeof(g_RainDropPool[0]));
  return;
}
