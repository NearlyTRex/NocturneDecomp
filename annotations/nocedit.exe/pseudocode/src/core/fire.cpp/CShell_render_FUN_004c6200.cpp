...

{
  ...
  if (0.0 < (this_ptr->base).lifetime_remaining) {
    ...
    local_1c.x = (int)ROUND((this_ptr->base).position.x * 256f);
    local_1c.y = (int)ROUND((this_ptr->base).position.y * 256f);
    local_1c.z = (int)ROUND((this_ptr->base).position.z * 256f);
