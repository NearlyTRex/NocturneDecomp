// Name: core_skeleton.cpp_CDeformableModelInstance_applyRotationToHierarchy_FUN_0059ff20
// Address: 0059ff20
// Address Range: [[0059ff20, 005a00ed]]
// Convention: __cdecl
// Signature: void core_skeleton.cpp_CDeformableModelInstance_applyRotationToHierarchy_FUN_0059ff20(CDeformableModelInstance * this_ptr, CQuaternion4f * rotation_quat, float blend_weight, int bone_index, CDeformableModel_MotionBlendWeightFunc * blend_callback)

#include "nocturne.h"

void __cdecl
core_skeleton_cpp_CDeformableModelInstance_applyRotationToHierarchy_FUN_0059ff20
          (CDeformableModelInstance *this_ptr,CQuaternion4f *rotation_quat,float blend_weight,
          int bone_index,CDeformableModel_MotionBlendWeightFunc *blend_callback)

{
  CSkeleton *pCVar1;
  int hierarchy_distance;
  int iVar2;
  CQuaternion4f *unaff_ESI;
  CQuaternion4f *pCVar3;
  int unaff_EDI;
  uint *puVar4;
  uint *puVar5;
  float *pfVar6;
  byte bVar7;
  float afStackY_1850 [1493];
  float fStack_e8;
  float fStack_e0;
  CQuaternion4f local_b4 [4];
  byte local_74 [8];
  uint auStack_6c [6];
  byte local_54 [36];
  CQuaternion4f CStack_30;
  CVector3f local_20;
  float fStack_14;
  
  bVar7 = 0;
  pCVar1 = core_skeleton_cpp_CDeformableModelInstance_getSkeletonPtr_FUN_005a0820(this_ptr);
  iVar2 = 0;
  CStack_30.z = (float)pCVar1;
  core_xform_cpp_quaternionToAxisAngle_FUN_005f7730
            (rotation_quat,&CStack_30.y,(CVector3f *)(local_54 + 0x20));
  if (0 < pCVar1->bone_count) {
    local_20.y = (float)(this_ptr->bone_transform).bone_rotations;
    local_20.x = (float)this_ptr;
    do {
      hierarchy_distance =
           core_skeleton_cpp_CSkeleton_getHierarchyDistance_FUN_0059a100
                     ((CSkeleton *)CStack_30.z,iVar2,bone_index);
      if (-1 < hierarchy_distance) {
        fStack_e8 = (*blend_callback)(iVar2,bone_index,blend_weight,hierarchy_distance,this_ptr);
        fStack_e8 = fStack_e8 * fStack_14;
        if ((float)1.0000000000000001e-05 < ABS(fStack_e8)) {
          fStack_e0 = 8.265272e-39;
          core_xform_cpp_quaternionFromAxisAngle_FUN_005f7a70(fStack_e8,&local_20,&CStack_30);
          local_54._4_4_ = CStack_30.w;
          puVar4 = (uint *)((int)&CStack_30 + (uint)bVar7 * -8 + (uint)bVar7 * -8 + 8);
          *(uint *)(local_54 + (uint)bVar7 * -8 + 8) =
               *(uint *)((int)&CStack_30 + (uint)bVar7 * -8 + 4);
          *(uint *)(local_54 + (uint)bVar7 * -8 + (uint)bVar7 * -8 + 0xc) = *puVar4;
          *(uint *)
           ((int)(local_54 + (uint)bVar7 * -8 + (uint)bVar7 * -8 + 0xc) + ((uint)bVar7 * -2 + 1) * 4
           ) = puVar4[(uint)bVar7 * -2 + 1];
          fStack_e0 = 8.265326e-39;
          core_xform_cpp_multiplyQuaternion_FUN_005f7640
                    (unaff_ESI,(CQuaternion4f *)(local_54 + 4),(CQuaternion4f *)(local_74 + 4));
          puVar4 = (uint *)(unaff_EDI + 0x6b4 + (uint)bVar7 * -8);
          *(uint *)(unaff_EDI + 0x6b0) = local_74._4_4_;
          puVar5 = puVar4 + (uint)bVar7 * -2 + 1;
          *puVar4 = auStack_6c[(uint)bVar7 * -2];
          *puVar5 = auStack_6c[(uint)bVar7 * -2 + (uint)bVar7 * -2 + 1];
          puVar5[(uint)bVar7 * -2 + 1] =
               (auStack_6c + (uint)bVar7 * -2 + (uint)bVar7 * -2 + 1)[(uint)bVar7 * -2 + 1];
        }
      }
      local_20.x = (float)&((CMotionController *)local_20.x)->tween_speed;
      iVar2 = iVar2 + 1;
      local_20.y = (float)((int)local_20.y + 0x10);
    } while (iVar2 < *(int *)((int)CStack_30.z + 0x28558));
  }
  if (-1 < bone_index) {
    return;
  }
  core_xform_cpp_quaternionFromAxisAngle_FUN_005f7a70
            (CStack_30.y * blend_weight,(CVector3f *)(local_54 + 0x20),(CQuaternion4f *)local_74);
  local_54._0_4_ = local_74._0_4_;
  *(uint *)(local_54 + (uint)bVar7 * -8 + 4) = auStack_6c[(uint)bVar7 * -2 + -1];
  *(uint *)(local_54 + (uint)bVar7 * -8 + (uint)bVar7 * -8 + 8) =
       auStack_6c[(uint)bVar7 * -2 + (uint)bVar7 * -2];
  *(uint *)
   ((int)(local_54 + (uint)bVar7 * -8 + (uint)bVar7 * -8 + 8) + ((uint)bVar7 * -2 + 1) * 4) =
       (auStack_6c + (uint)bVar7 * -2 + (uint)bVar7 * -2)[(uint)bVar7 * -2 + 1];
  core_xform_cpp_quaternionToMatrix3x4_FUN_005f73e0((CMatrix3x4f *)local_54,local_b4);
  pCVar3 = local_b4;
  pfVar6 = (float *)&stack0xffffff1c;
  for (iVar2 = 0xc; iVar2 != 0; iVar2 = iVar2 + -1) {
    *pfVar6 = pCVar3->w;
    pCVar3 = (CQuaternion4f *)((int)pCVar3 + ((uint)bVar7 * -2 + 1) * 4);
    pfVar6 = pfVar6 + (uint)bVar7 * -2 + 1;
  }
  core_xform_cpp_transformVector3x4InPlace_FUN_005f4e20
            (&(this_ptr->bone_transform).root_position,(CMatrix3x4f *)&stack0xffffff1c);
  return;
}
