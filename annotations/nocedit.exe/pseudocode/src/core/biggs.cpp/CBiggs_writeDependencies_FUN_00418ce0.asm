; *****************************************************************************
;                               FUNCTION
; *****************************************************************************
; __cdecl void __cdecl core_biggs_cpp_CBiggs_writeDependencies_FUN_00418ce0(CBiggs *this_ptr,_FILE *file_handle)
;
; Parameters:
; CBiggs *         Stack[0x4]:4   this_ptr
; _FILE *          Stack[0x8]:4   file_handle
;
; Called Functions:
;   core_actor.cpp_CDemonActor_writeDependencies_FUN_0040cfd0
;   core_skeledit.cpp_CDeformableModelInstance_writeDependencies_FUN_0058a260
;
; *****************************************************************************

section .text

    PUSH EBX                            ; 00418ce0
        ;   Label: core_biggs.cpp_CBiggs_writeDependencies_FUN_00418ce0
    MOV EDX,dword ptr [ESP + 0xc]       ; 00418ce1
    PUSH EDX                            ; 00418ce5
    MOV ECX,dword ptr [ESP + 0xc]       ; 00418ce6
    PUSH ECX                            ; 00418cea
    CALL core_actor.cpp_CDemonActor_writeDependencies_FUN_0040cfd0 ; 00418ceb
        ;   XREF to: 0040cfd0 (UNCONDITIONAL_CALL)  ; void core_actor.cpp_CDemonActor_writeDependencies_FUN_0040cfd0(CDemonActor * this_ptr, _FILE * file_handle)
    ADD ESP,0x8                         ; 00418cf0
    MOV EBX,dword ptr [ESP + 0xc]       ; 00418cf3
    MOV EAX,dword ptr [ESP + 0x8]       ; 00418cf7
    PUSH EBX                            ; 00418cfb
    ADD EAX,0x158                       ; 00418cfc
    PUSH EAX                            ; 00418d01
    CALL core_skeledit.cpp_CDeformableModelInstance_writeDependencies_FUN_0058a260 ; 00418d02
        ;   XREF to: 0058a260 (UNCONDITIONAL_CALL)  ; void core_skeledit.cpp_CDeformableModelInstance_writeDependencies_FUN_0058a260(CDeformableModelInstance * this_ptr, _FILE * file_handle)
    ADD ESP,0x8                         ; 00418d07
    POP EBX                             ; 00418d0a
    RET                                 ; 00418d0b

