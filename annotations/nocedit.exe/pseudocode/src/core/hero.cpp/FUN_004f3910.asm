; *****************************************************************************
;                               FUNCTION
; *****************************************************************************
; unknown undefined core_hero.cpp_FUN_004f3910()
;
;
; XREF[1]:
;   core_script.cpp_CScript_step_FUN_0055a810 at 0055e4ec
;
; Called Functions:
;   core_inv.cpp_CInventory_removeMatchingKeys_FUN_005014a0
;
; *****************************************************************************

section .text

    MOV EDX,dword ptr [ESP + 0x8]       ; 004f3910
        ;   Label: core_hero.cpp_FUN_004f3910
    MOV EAX,dword ptr [ESP + 0x4]       ; 004f3914
    PUSH EDX                            ; 004f3918
    ADD EAX,0x1f738                     ; 004f3919
    PUSH EAX                            ; 004f391e
    CALL core_inv.cpp_CInventory_removeMatchingKeys_FUN_005014a0 ; 004f391f | void core_inv.cpp_CInventory_removeMatchingKeys_FUN_005014a0(CInventory * this_ptr, uint key_mask)
        ;   XREF to: 005014a0 (UNCONDITIONAL_CALL)
    ADD ESP,0x8                         ; 004f3924
    RET                                 ; 004f3927

