           ...
           (((double)local_3c.z <= 3 && (ABS(local_3c.x) <= (float)3))))
        {
          ...
          if ((ABS(pCVar3->y) <= (float)60) &&
            ...
            if ((CLocation *)&this_ptr->target_position != input_world_point) {
              (this_ptr->target_position).x = (input_world_point->position).x;
              (this_ptr->target_position).y = (this_ptr_00->base).location.position.y;
              (this_ptr->target_position).z = (this_ptr_00->base).location.position.z;
            }
            pUVar4 = &(this_ptr_00->base).orient;
            if (&this_ptr->target_orientation != pUVar4) {
              (this_ptr->target_orientation).vec.x = (pUVar4->vec).x;
              (this_ptr->target_orientation).vec.y = (this_ptr_00->base).orient.vec.y;
              (this_ptr->target_orientation).vec.z = (this_ptr_00->base).orient.vec.z;
            }
            return 1;
          }
        }
