...
LAB_004f3db9:
  ...
  (pCVar2->base).base.location.position.x = (this_ptr->base).location.position.x;
  (pCVar2->base).base.location.position.y = (this_ptr->base).location.position.y;
  (pCVar2->base).base.location.position.z = (this_ptr->base).location.position.z;
  (pCVar2->base).base.location.area_id = (this_ptr->base).location.area_id;
  pUVar1 = &(pCVar2->base).base.orient;
  pUVar2 = &(this_ptr->base).orient;
  if (pUVar1 != pUVar2) {
    (pUVar1->vec).x = (pUVar2->vec).x;
    (pCVar2->base).base.orient.vec.y = (this_ptr->base).orient.vec.y;
    (pCVar2->base).base.orient.vec.z = (this_ptr->base).orient.vec.z;
  }
