// Name: core_hero.cpp_CHero_FUN_004f3960
// Address: 004f3960
// Address Range: [[004f3960, 004f3ae7]]
// Convention: __cdecl
// Signature: float __cdecl core_hero_cpp_CHero_FUN_004f3960(CHero *this_ptr)

#include "nocturne.h"

float __cdecl core_hero_cpp_CHero_FUN_004f3960(CHero *this_ptr)

{
  CCharacter *this_ptr_00;
  float fVar1;
  CDemonActor *pCVar2;
  int iVar3;
  float *unaff_EBX;
  int iVar4;
  int iVar5;
  float *in_stack_00000008;
  CVector3f aCStack_a0 [10];
  float fStack_20;
  float local_1c;
  float fStack_18;
  CLocation *local_14;
  
  iVar5 = 0;
  *in_stack_00000008 = 9999.9;
  iVar4 = 0;
  local_14 = &(this_ptr->base).base.location;
  local_1c = 0.0;
  while( true ) {
    if (*(int *)(g_CDemonSetPtr->unk4 + 0x1f3c) <= iVar5) break;
    this_ptr_00 = *(CCharacter **)(g_CDemonSetPtr->unk4 + iVar4 + 8000);
    pCVar2 = core_actor_cpp_castToClassHash_FUN_0040c790
                       (&this_ptr_00->base,g_CVehicleClassInfo.name_hash);
    if (pCVar2 == (CDemonActor *)0x0) {
      pCVar2 = core_actor_cpp_castToClassHash_FUN_0040c790
                         (&this_ptr_00->base,g_CMobsterClassInfo.name_hash);
      if ((pCVar2 == (CDemonActor *)0x0) || (*(int *)(pCVar2[0x8e].actor_name + 0x10) == 0)) {
        iVar3 = (*(((this_ptr_00->base).vtable._uc)->_uc).getDeathState)(this_ptr_00);
        if (iVar3 == 0) {
          iVar3 = (*((this_ptr_00->base).vtable._ub)->shouldIgnoreForTargeting)(&this_ptr_00->base);
          if (iVar3 == 0) {
            __arrinit(aCStack_a0,10,&g_CVectorTypeInfo);
            iVar3 = (*((this_ptr_00->base).vtable._ub)->getTargetPoints)
                              (&this_ptr_00->base,aCStack_a0);
            if (0 < iVar3) {
              fStack_20 = *unaff_EBX - (this_ptr_00->base).location.position.x;
              local_1c = (unaff_EBX[1] - (this_ptr_00->base).location.position.y) *
                         (float)2;
              fStack_18 = unaff_EBX[2] - (this_ptr_00->base).location.position.z;
              fVar1 = fStack_18 * fStack_18 + fStack_20 * fStack_20 + local_1c * local_1c;
              if (fVar1 < *in_stack_00000008) {
                *in_stack_00000008 = fVar1;
              }
            }
          }
        }
      }
    }
    iVar5 = iVar5 + 1;
    iVar4 = iVar4 + 4;
  }
  if (local_1c == 0.0) {
    return 0.0;
  }
  *in_stack_00000008 = SQRT(*in_stack_00000008);
  return local_1c;
}
