// Name: core_dstrender.cpp_blendBilerpLightmapToTexture64BitOutputOffset_FUN_00491c9a
// Address: 00491c9a
// MANUAL RECONSTRUCTION
// Address Range: [[00491c9a, 00491ed0] [00491ee0, 00491f8f]]
// Convention: __cdecl
// Signature: void __cdecl core_dstrender_cpp_blendBilerpLightmapToTexture64BitOutputOffset_FUN_00491c9a(ulonglong *output_buffer,ulonglong *texture_buffer,byte *texture_indices,byte *lightmap_indices,int pixel_count)

#include "nocturne.h"

void __cdecl core_dstrender_cpp_blendBilerpLightmapToTexture64BitOutputOffset_FUN_00491c9a(ulonglong *output_buffer,ulonglong *texture_buffer,byte *texture_indices,byte *lightmap_indices,int pixel_count)
{
  ushort solid_ws[4];
  uint solid_raw;
  ...
  do {
    ...
    pal_idx0 = ((uint)texture_indices[0] + (uint)texture_indices[0x140]) >> 1;
    pal_idx1 = ((uint)texture_indices[1] + (uint)texture_indices[0x141]) >> 1;
    palette0 = g_LightmapTexturePalette[pal_idx0];
    palette1 = g_LightmapTexturePalette[pal_idx1];

    lm_idx = (uint)lightmap_indices[0] +
             (((uint)texture_indices[1] + (uint)texture_indices[0x141]) >> 2);
    lm_data = *(ulonglong *)((char *)g_LightmapData + lm_idx * 8);
    lm_xored = lm_data ^ xor_mask_bits;

    out64 = 0;
    for (pixel = 0; pixel < 2; pixel = pixel + 1) {
      ambient_val = (pixel == 0) ? g_AmbientLightMMX1.mm : g_AmbientLightMMX2.mm;
      for (c = 0; c < 4; c = c + 1) {
        byte_pos = pixel * 4 + c;
        ...
        ambient_w = (short)((ambient_val >> (c * 16)) & 0xffff);

        sum16 = (short)(mul1_hi + solid_hi + ambient_w);
