...
LAB_004d3af9:
    ...
    if ((this_ptr->base).player_input.action_state.fire != 0) {
      ...
      pCVar16 = core_vecdir_cpp_convertDirectionVectorToEulerAngles_FUN_005e7830
                          (&local_dc,&local_b8);
      local_1c = core_actor_cpp_normalizeAngleToPi_FUN_0040cd70(pCVar16->y);
      ...
      if ((((local_b8.z - SQRT(fVar10 * fVar10 + fVar11 * fVar11) < (float)3) &&
         ...
         (ABS(local_1c) < (float)0.78539816337500001)) {
        float fVarClamp = delta_time * (float)3.1415926535000001;
        if (local_1c < -fVarClamp) {
          local_1c = -fVarClamp;
        }
        if (fVarClamp < local_1c) {
          local_1c = fVarClamp;
        }
        (this_ptr->base).base.base.orient.vec.y =
             (this_ptr->base).base.base.orient.vec.y + local_1c;
        goto LAB_004d321d;
      }
    }
    core_motion_cpp_CMotionController_setDesiredState_FUN_0052db00
