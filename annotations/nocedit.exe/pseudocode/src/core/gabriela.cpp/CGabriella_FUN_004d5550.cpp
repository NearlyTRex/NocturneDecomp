...

{
  ...
  if (this_ptr->draw_blend < 0.64f) {
    (*(((((this_ptr->base).inventory.selected_weapon)->base).vtable._uc)->_uc).cfunc2)();
    core_xform_cpp_buildMatrixFromEulerAndPositionDirect_FUN_005f54c0
  ...
  pCVar1 = (this_ptr->base).inventory.selected_weapon;
  pCVar5 = &(pCVar1->base).orient;
  if (pCVar5 != (COrientation *)pCVar3) {
    pCVar5->pitch = pCVar3->x;
    (pCVar1->base).orient.bank = pCVar3->y;
    (pCVar1->base).orient.heading = pCVar3->z;
  }
