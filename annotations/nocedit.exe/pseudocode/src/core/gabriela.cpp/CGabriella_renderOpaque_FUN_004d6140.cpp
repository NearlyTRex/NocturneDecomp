...

{
  ...
  if (iVar3 != 0) {
    ...
    if (((this_ptr_00 != (CWeapon *)0x0) &&
        ((*((this_ptr_00->base_actor).vtable._ub)->renderOpaque)(&this_ptr_00->base_actor),
        (float)0.90000000000000002 < *(float *)(this_ptr->field1_0x1fbd4 + 8))) &&
       ((pCVar4 = core_actor_cpp_castToClassHash_FUN_0040c790
        ...
        0.0 < SQRT(fVar2 * fVar2 +
                   *(float *)(pCVar4[4].actor_name + 0x1c) * *(float *)(pCVar4[4].actor_name + 0x1c)
                   + fVar1 * fVar1))))) {
      (*(((((this_ptr->base_hero).inventory.selected_weapon)->base_actor).vtable._uc)->_uc).cfunc7)
                ();
      return iVar3;
    }
  }
