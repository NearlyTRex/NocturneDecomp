...
// =============================================================================
//
// See gl_blit.h for why this is renderer-agnostic and lives here rather than in
// tridx7's gl_shader.cpp.
...
// #version 120, matching gl_shader.cpp: the lowest version that has everything
// needed, so driver variation is minimised. Nothing here needs more.
//
// The quad's positions are already in clip space, which is what replaces the
// glOrtho(0, 1, 1, 0, -1, 1) the fixed-function path set up. That projection
// mapped the unit square onto NDC as (2x - 1, 1 - 2y) — a top-left origin so the
// texture's first row lands at the top of the viewport — with z = 0 passing
// through unchanged. Baking it into four static vertices is the whole of it.
...
bool build_program() {
    ...
    // That cost the draw path an iteration (research/17, phase 3); pin it here
    // rather than let the linker choose.
...
extern "C" int nocturne_gl_blit_quad(unsigned int texture) {
    ...
    // texture on — nothing here calls glActiveTexture, and neither did the
    // fixed-function quad this replaces.
