; *****************************************************************************
;                               FUNCTION
; *****************************************************************************
; void FUN_005492b0(undefined4 *param_1)
;
;
; XREF[1]:
;   FUN_00506f10 at 0050725e
;
; Referenced Globals:
;   string s_junk.jnk_00596cc5
;   undefined4 DAT_005ba8ec
;   undefined4 DAT_02dd10cc
;
; Called Functions:
;   FUN_004b13d0
;   FUN_004b14d0
;
; *****************************************************************************

section .text

    PUSH EBX                            ; 005492b0
        ;   Label: FUN_005492b0
    MOV EDX,dword ptr [0x005ba8ec]      ; 005492b1 | DAT_005ba8ec
    MOV EAX,dword ptr [ESP + 0x8]       ; 005492b7
    PUSH EDX                            ; 005492bb
    MOV dword ptr [EAX],0x0             ; 005492bc
    CALL FUN_004b13d0                   ; 005492c2
        ;   XREF to: 004b13d0 (UNCONDITIONAL_CALL)  ; undefined FUN_004b13d0()
    ADD ESP,0x4                         ; 005492c7
    PUSH 0x596cc5                       ; 005492ca | = "junk.jnk"
    MOV ECX,dword ptr [0x005ba8ec]      ; 005492cf | DAT_005ba8ec
    PUSH ECX                            ; 005492d5
    MOV EBX,0x1                         ; 005492d6
    CALL FUN_004b14d0                   ; 005492db
        ;   XREF to: 004b14d0 (UNCONDITIONAL_CALL)  ; undefined FUN_004b14d0()
    ADD ESP,0x8                         ; 005492e0
    MOV dword ptr [0x02dd10cc],EBX      ; 005492e3 | DAT_02dd10cc
    POP EBX                             ; 005492e9
    RET                                 ; 005492ea

