// Name: FUN_004f4bb0
// Address: 004f4bb0
// Address Range: [[004f4bb0, 004f4c7c]]
// Convention: unknown
// Signature: void FUN_004f4bb0(int param_1,int param_2)

#include "nocturne.h"

/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

void FUN_004f4bb0(int param_1,int param_2)

{
  int iVar1;
  uint *puVar2;
  int *piVar3;
  int aiStack_2c [6];
  int iStack_14;
  
  puVar2 = (uint *)0x5be17c;
  piVar3 = aiStack_2c;
  for (iVar1 = 1; iVar1 != 0; iVar1 = iVar1 + -1) {
    *piVar3 = *puVar2;
    puVar2 = puVar2 + 1;
    piVar3 = piVar3 + 1;
  }
  for (iVar1 = 0; iVar1 != 0; iVar1 = iVar1 + -1) {
    *(byte *)piVar3 = *(byte *)puVar2;
    puVar2 = (uint *)((int)puVar2 + 1);
    piVar3 = (uint *)((int)piVar3 + 1);
  }
  aiStack_2c[1] = *(uint *)(param_1 + 0x18);
  aiStack_2c[2] = *(uint *)(param_1 + 0x1c);
  aiStack_2c[3] = 0;
  aiStack_2c[4] = 0;
  aiStack_2c[5] = *(int *)(param_1 + 0x18) + -1;
  iStack_14 = *(int *)(param_1 + 0x1c) + -1;
  FUN_00563a50(aiStack_2c,0x1c,1,param_2);
  FUN_00563a50(*(uint *)(param_1 + 0x20),4,*(int *)(param_1 + 0x1c) + 1,param_2);
  FUN_00563a50(*(uint *)(param_1 + 0x14),
               *(uint *)(*(int *)(param_1 + 0x20) + *(int *)(param_1 + 0x1c) * 4),1,param_2);
  if ((*(byte *)(param_2 + 0xc) & 0x20) == 0) {
    return;
  }
  _DAT_01cc4800 = "..\\cockpit\\pkbitmap.cpp";
  _DAT_01cc4804 = 0x49d;
  FUN_004c8440("Error writing PBM");
  return;
}
