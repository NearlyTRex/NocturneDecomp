; *****************************************************************************
;                               FUNCTION
; *****************************************************************************
; void FUN_005543c0(int param_1)
;
;
; Referenced Globals:
;   undefined4 DAT_00597c09
;   undefined4 DAT_00597c0d
;   undefined4 DAT_00597c11
;   undefined4 DAT_005b9354
;   undefined4 DAT_005be368
;   undefined4 DAT_01c74640
;   undefined4 DAT_01c74740
;   undefined4 DAT_01c74744
;   undefined4 DAT_01c74748
;   undefined4 DAT_01c7474c
;   undefined4 DAT_01c74750
;   undefined4 DAT_01c74778
;   undefined4 DAT_01c762f4
;   undefined4 DAT_01c762fc
;   undefined4 DAT_01c76310
;   ... and 7 more
;
; Called Functions:
;   FUN_0040a240
;   FUN_0040d890
;   FUN_0044cd90
;   FUN_0044cde0
;   FUN_0044d7a0
;   FUN_00452080
;   FUN_0050a9f0
;
; *****************************************************************************

section .text

    PUSH EBX                            ; 005543c0
        ;   Label: FUN_005543c0
    PUSH ESI                            ; 005543c1
    PUSH EBP                            ; 005543c2
    SUB ESP,0x2c                        ; 005543c3
    MOV EBX,dword ptr [ESP + 0x3c]      ; 005543c6
    MOV EDX,ESP                         ; 005543ca
    PUSH EDX                            ; 005543cc
    MOV EAX,dword ptr [EBX + 0x14c]     ; 005543cd
    PUSH EBX                            ; 005543d3
    CALL dword ptr [EAX + 0xd8]         ; 005543d4
    ADD ESP,0x8                         ; 005543da
    PUSH EAX                            ; 005543dd
    LEA EAX,[ESP + 0x1c]                ; 005543de
    PUSH EAX                            ; 005543e2
    PUSH EBX                            ; 005543e3
    CALL FUN_0040a240                   ; 005543e4
        ;   XREF to: 0040a240 (UNCONDITIONAL_CALL)  ; undefined FUN_0040a240()
    MOV EDX,0x1                         ; 005543e9
    LEA EAX,[ESP + 0x24]                ; 005543ee
    ADD ESP,0xc                         ; 005543f2
    MOV dword ptr [0x01c762f4],EDX      ; 005543f5 | DAT_01c762f4
    MOV dword ptr [0x01c762fc],EDX      ; 005543fb | DAT_01c762fc
    CMP EAX,0x1c74744                   ; 00554401
    JZ 0x00554423                       ; 00554406
        ;   XREF to: 00554423 (CONDITIONAL_JUMP)  ; LAB_00554423
    MOV EAX,dword ptr [ESP + 0x18]      ; 00554408
    MOV [0x01c74744],EAX                ; 0055440c | DAT_01c74744
    MOV EAX,dword ptr [ESP + 0x1c]      ; 00554411
    MOV [0x01c74748],EAX                ; 00554415 | DAT_01c74748
    MOV EAX,dword ptr [ESP + 0x20]      ; 0055441a
    MOV [0x01c7474c],EAX                ; 0055441e | DAT_01c7474c
    LEA EDX,[EBX + 0x30]                ; 00554423
        ;   Label: LAB_00554423
    MOV EAX,dword ptr [EDX]             ; 00554426
    MOV dword ptr [ESP + 0xc],EAX       ; 00554428
    LEA EAX,[EDX + 0x4]                 ; 0055442c
    MOV EAX,dword ptr [EAX]             ; 0055442f
    MOV dword ptr [ESP + 0x10],EAX      ; 00554431
    LEA EAX,[EDX + 0x8]                 ; 00554435
    MOV ESI,0x42000000                  ; 00554438
    MOV EAX,dword ptr [EAX]             ; 0055443d
    MOV EBP,0x1                         ; 0055443f
    MOV dword ptr [ESP + 0x14],EAX      ; 00554444
    LEA EAX,[ESP + 0xc]                 ; 00554448
    FLD float ptr [ESP + 0xc]           ; 0055444c
    PUSH EAX                            ; 00554450
    FADD float ptr [0x00597c09]         ; 00554451 | DAT_00597c09
    PUSH 0x1c74750                      ; 00554457 | DAT_01c74750
    FSTP float ptr [ESP + 0x14]         ; 0055445c
    CALL FUN_0044d7a0                   ; 00554460
        ;   XREF to: 0044d7a0 (UNCONDITIONAL_CALL)  ; undefined FUN_0044d7a0()
    ADD ESP,0x8                         ; 00554465
    MOV EDX,dword ptr [0x01cae124]      ; 00554468 | DAT_01cae124
    MOV EAX,0x3f800000                  ; 0055446e
    PUSH EDX                            ; 00554473
    MOV dword ptr [0x01c74740],ESI      ; 00554474 | DAT_01c74740
    MOV dword ptr [ESP + 0x2c],EAX      ; 0055447a
    PUSH EBX                            ; 0055447e
    MOV EAX,dword ptr [EBX + 0x14c]     ; 0055447f
    MOV dword ptr [0x01c74778],ESI      ; 00554485 | DAT_01c74778
    MOV dword ptr [0x01c76310],EBP      ; 0055448b | DAT_01c76310
    CALL dword ptr [EAX + 0x8c]         ; 00554491
    ADD ESP,0x4                         ; 00554497
    PUSH EAX                            ; 0055449a
    CALL FUN_0040d890                   ; 0055449b
        ;   XREF to: 0040d890 (UNCONDITIONAL_CALL)  ; undefined FUN_0040d890()
    ADD ESP,0x8                         ; 005544a0
    TEST EAX,EAX                        ; 005544a3
    JZ 0x005544ce                       ; 005544a5
        ;   XREF to: 005544ce (CONDITIONAL_JUMP)  ; LAB_005544ce
    MOV EAX,dword ptr [EAX + 0x1f5a0]   ; 005544a7
    MOV dword ptr [ESP + 0x24],EAX      ; 005544ad
    FLD float ptr [ESP + 0x24]          ; 005544b1
    FCOMP float ptr [0x00597c0d]        ; 005544b5 | DAT_00597c0d
    FNSTSW AX                           ; 005544bb
    SAHF                                ; 005544bd
    JNC 0x005544ce                      ; 005544be
        ;   XREF to: 005544ce (CONDITIONAL_JUMP)  ; LAB_005544ce
    FLD float ptr [ESP + 0x24]          ; 005544c0
    FMUL float ptr [0x00597c11]         ; 005544c4 | DAT_00597c11
    FSTP float ptr [ESP + 0x28]         ; 005544ca
    PUSH dword ptr [ESP + 0x28]         ; 005544ce
        ;   Label: LAB_005544ce
    PUSH 0x1c74640                      ; 005544d2 | DAT_01c74640
    CALL FUN_00452080                   ; 005544d7
        ;   XREF to: 00452080 (UNCONDITIONAL_CALL)  ; undefined FUN_00452080()
    MOV EAX,[0x005b9354]                ; 005544dc | DAT_005b9354 | DAT_01c775ec
    MOV ECX,dword ptr [EAX + 0x244]     ; 005544e1 | DAT_01c77830
    ADD ESP,0x8                         ; 005544e7
    TEST ECX,ECX                        ; 005544ea
    JNZ 0x005544f5                      ; 005544ec
        ;   XREF to: 005544f5 (CONDITIONAL_JUMP)  ; LAB_005544f5
    ADD ESP,0x2c                        ; 005544ee
    POP EBP                             ; 005544f1
    POP ESI                             ; 005544f2
    POP EBX                             ; 005544f3
    RET                                 ; 005544f4
    PUSH EDI                            ; 005544f5
        ;   Label: LAB_005544f5
    LEA EAX,[ESP + 0x1c]                ; 005544f6
    PUSH EAX                            ; 005544fa
    PUSH 0x2ddf9ac                      ; 005544fb
    CALL FUN_0044cd90                   ; 00554500
        ;   XREF to: 0044cd90 (UNCONDITIONAL_CALL)  ; undefined FUN_0044cd90()
    ADD ESP,0x8                         ; 00554505
    MOV EBX,0x8000                      ; 00554508
    PUSH 0x40400000                     ; 0055450d
    MOV AH,0x20                         ; 00554512
    PUSH 0x2ddf9ac                      ; 00554514
    MOV dword ptr [0x02ddf9e8],EBX      ; 00554519 | DAT_02ddf9e8
    MOV byte ptr [0x02ddf9c8],AH        ; 0055451f | DAT_02ddf9c8
    CALL FUN_0044cde0                   ; 00554525
        ;   XREF to: 0044cde0 (UNCONDITIONAL_CALL)  ; undefined FUN_0044cde0()
    ADD ESP,0x8                         ; 0055452a
    PUSH 0x2ddf9ac                      ; 0055452d
    MOV EDI,dword ptr [0x005be368]      ; 00554532 | DAT_005be368
    XOR ESI,ESI                         ; 00554538
    PUSH EDI                            ; 0055453a | DAT_01e57284
    MOV dword ptr [0x02ddf9cc],ESI      ; 0055453b | DAT_02ddf9cc
    CALL FUN_0050a9f0                   ; 00554541
        ;   XREF to: 0050a9f0 (UNCONDITIONAL_CALL)  ; undefined FUN_0050a9f0()
    ADD ESP,0x8                         ; 00554546
    POP EDI                             ; 00554549
    ADD ESP,0x2c                        ; 0055454a
    POP EBP                             ; 0055454d
    POP ESI                             ; 0055454e
    POP EBX                             ; 0055454f
    RET                                 ; 00554550

