; *****************************************************************************
;                               FUNCTION
; *****************************************************************************
; void FUN_004df5b0(undefined4 param_1,int param_2)
;
;
; XREF[1]:
;   FUN_004e0170 at 004e019b
;
; Called Functions:
;   FUN_004df460
;   FUN_0051da50
;   FUN_0051e020
;
; *****************************************************************************

section .text

    PUSH EBX                            ; 004df5b0
        ;   Label: FUN_004df5b0
    PUSH ESI                            ; 004df5b1
    PUSH EDI                            ; 004df5b2
    PUSH EBP                            ; 004df5b3
    MOV EBX,dword ptr [ESP + 0x18]      ; 004df5b4
    PUSH 0x0                            ; 004df5b8
    PUSH EBX                            ; 004df5ba
    CALL FUN_0051da50                   ; 004df5bb
        ;   XREF to: 0051da50 (UNCONDITIONAL_CALL)  ; undefined FUN_0051da50()
    ADD ESP,0x8                         ; 004df5c0
    PUSH EBX                            ; 004df5c3
    CALL FUN_0051e020                   ; 004df5c4
        ;   XREF to: 0051e020 (UNCONDITIONAL_CALL)  ; undefined FUN_0051e020()
    MOV EDX,EAX                         ; 004df5c9
    MOV EAX,dword ptr [EBX + 0x2230]    ; 004df5cb
    SHL EAX,0x2                         ; 004df5d1
    ADD EAX,EDX                         ; 004df5d4
    ADD ESP,0x4                         ; 004df5d6
    MOV ECX,dword ptr [EAX + 0x90]      ; 004df5d9
    PUSH ECX                            ; 004df5df
    ADD EDX,0xc0                        ; 004df5e0
    PUSH EDX                            ; 004df5e6
    MOV ESI,dword ptr [EAX + 0x7c]      ; 004df5e7
    PUSH ESI                            ; 004df5ea
    MOV EDI,dword ptr [EAX + 0x54]      ; 004df5eb
    PUSH EDI                            ; 004df5ee
    MOV EBP,dword ptr [EBX + 0x2234]    ; 004df5ef
    PUSH EBP                            ; 004df5f5
    MOV EDX,dword ptr [EAX + 0x2c]      ; 004df5f6
    PUSH EDX                            ; 004df5f9
    MOV ECX,dword ptr [ESP + 0x2c]      ; 004df5fa
    PUSH ECX                            ; 004df5fe
    CALL FUN_004df460                   ; 004df5ff
        ;   XREF to: 004df460 (UNCONDITIONAL_CALL)  ; undefined FUN_004df460()
    ADD ESP,0x1c                        ; 004df604
    POP EBP                             ; 004df607
    POP EDI                             ; 004df608
    POP ESI                             ; 004df609
    POP EBX                             ; 004df60a
    RET                                 ; 004df60b

