...

{
  ...
  local_18 = 0;
  if (this_ptr->stop_after_fade == 1.4013e-45) {
    engine_console_cpp_CConsole_printf_FUN_0043ac60(PTR_DAT_005ad350,"SFXDBG: sample = %s\n");
  }
  if ((this_ptr->hardware_buffer_handle != 0) && (0.0 <= (this_ptr->options).trigger_time)) {
    if ((0.0 < delta_time) && (0.0 <= this_ptr->fade_target_volume)) {
      if (this_ptr->fade_target_volume <= delta_time) {
        if (ABS(this_ptr->fade_time_remaining) != 0.0) goto LAB_00524b03;
        (this_ptr->options).current_volume = this_ptr->max_distance;
      }
      else {
        fVar1 = (this_ptr->options).current_volume;
        fVar2 = this_ptr->fade_target_volume;
        fVar3 = (this_ptr->options).current_volume;
        this_ptr->fade_target_volume = this_ptr->fade_target_volume - delta_time;
        (this_ptr->options).current_volume =
             (delta_time / fVar2) * (this_ptr->max_distance - fVar1) + fVar3;
      }
      local_18 = 8;
    }
    ...
    this_ptr->sample = (CSfxSample *)(this_ptr->options).base_frequency;
    if (this_ptr->stop_after_fade == 1.4013e-45) {
      engine_console_cpp_CConsole_printf_FUN_0043ac60
                (PTR_DAT_005ad350,"  freq = %5.2f\n",(double)(float)this_ptr->sample);
    }
    if (((this_ptr->options).flags & 1) == 0) {
      ...
      this_ptr->sample = (CSfxSample *)((float)this_ptr->sample * (float)local_60);
    }
    else if (this_ptr->stop_after_fade == 1.4013e-45) {
      engine_console_cpp_CConsole_printf_FUN_0043ac60(PTR_DAT_005ad350);
    }
    ...
    if ((this_ptr->stop_after_fade == 1.4013e-45) &&
       (iVar11 = 0, pCVar13 = this_ptr, 0 < DAT_005bea68)) {
      do {
        iVar12 = iVar11 + 1;
        engine_console_cpp_CConsole_printf_FUN_0043ac60
                  (PTR_DAT_005ad350,"  channelVol[%d] = %5.2f\n",iVar11,
                   (double)((float)(1 << ((char)*(uint *)(this_ptr->playback_state + 0x104) -
                                          1U & 0x1f)) * pCVar13->distance_to_speakers[7]));
        iVar11 = iVar12;
        pCVar13 = (CSfxSlot *)&(pCVar13->options).position;
      } while (iVar12 < DAT_005bea68);
    }
    if ((float)this_ptr->sample < (float)(1.0 / _FLOAT_005a2160)) {
      this_ptr->sample = (CSfxSample *)(float)(1.0 / _FLOAT_005a2160);
    }
    if ((float)_FLOAT_005a2160 < (float)this_ptr->sample) {
      this_ptr->sample = (CSfxSample *)(float)_FLOAT_005a2160;
    }
    if (this_ptr->stop_after_fade == 1.4013e-45) {
      engine_console_cpp_CConsole_printf_FUN_0043ac60
                (PTR_DAT_005ad350,"  effFreq = %5.2f\n",(double)(float)this_ptr->sample);
    }
    if (((local_18 != 0) && (_DAT_02dc8318 != (int *)0x0)) && ((this_ptr->options).dead != 0)) {
