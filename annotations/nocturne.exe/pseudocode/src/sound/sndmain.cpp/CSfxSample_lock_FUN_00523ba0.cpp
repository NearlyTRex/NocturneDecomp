...

{
  ...
  if (this_ptr->locked_length != 0) {
    ...
    g_CurrentLineNumber = 1869;
    core_main_c_displayErrorAndQuit_FUN_004c8440("SfxSample::lock - already locked!");
  }
  if ((lock_offset < 0) || (this_ptr->streaming_buffer_size < lock_offset + lock_length)) {
    g_CurrentFilename = "..\\sound\\sndmain.cpp";
    g_CurrentLineNumber = 1874;
    core_main_c_displayErrorAndQuit_FUN_004c8440("SfxSample::lock - invalid region");
  }
  if (this_ptr->buffer_id == 0) {
    if (this_ptr->sample_data == (short *)0x0) {
      g_CurrentFilename = "..\\sound\\sndmain.cpp";
      g_CurrentLineNumber = 1881;
      core_main_c_displayErrorAndQuit_FUN_004c8440("SfxSample::lock - nothing allocated!");
    }
