; *****************************************************************************
;                               FUNCTION
; *****************************************************************************
; void __cdecl engine_2d_c_loadPaletteFile_FUN_00401bd0(char *filename)
;
; Parameters:
; char *           Stack[0x4]:4   filename
;
; XREF[1]:
;   engine_2d.c_initGraphicsSystem_FUN_00401010 at 0040105c
;
; Referenced Globals:
;   TerminatedCString s_rb_00577154
;   TerminatedCString s_art_00577157
;   TerminatedCString s_engine_2d_c_0057715b
;   TerminatedCString s_Unable_to_open_VGA_palet_0057716a
;   char* g_CurrentFilename
;   int g_CurrentLineNumber
;
; Called Functions:
;   core_main.c_displayErrorAndQuit_FUN_004c8440
;   crt_stdio.c_fclose_FUN_00563380
;   crt_stdio.c_fread_FUN_005636d0
;   engine_dosio.cpp_getFile_FUN_00456a60
;
; *****************************************************************************

section .text

    PUSH EBX                            ; 00401bd0
        ;   Label: engine_2d.c_loadPaletteFile_FUN_00401bd0
    PUSH 0x577154                       ; 00401bd1 | = "rb"
    MOV EDX,dword ptr [ESP + 0xc]       ; 00401bd6
    PUSH EDX                            ; 00401bda
    PUSH 0x577157                       ; 00401bdb | = "art"
    CALL engine_dosio.cpp_getFile_FUN_00456a60 ; 00401be0
        ;   XREF to: 00456a60 (UNCONDITIONAL_CALL)  ; _FILE * engine_dosio.cpp_getFile_FUN_00456a60(char * directory, char * filename, char * mode)
    ADD ESP,0xc                         ; 00401be5
    MOV EBX,EAX                         ; 00401be8
    TEST EAX,EAX                        ; 00401bea
    JZ 0x00401c0e                       ; 00401bec
        ;   XREF to: 00401c0e (CONDITIONAL_JUMP)  ; LAB_00401c0e
    PUSH EBX                            ; 00401bee
        ;   Label: LAB_00401bee
    PUSH 0x1                            ; 00401bef
    PUSH 0x300                          ; 00401bf1
    PUSH 0x1c00648                      ; 00401bf6
    CALL crt_stdio.c_fread_FUN_005636d0 ; 00401bfb
        ;   XREF to: 005636d0 (UNCONDITIONAL_CALL)  ; SIZE_T crt_stdio.c_fread_FUN_005636d0(void * buffer, SIZE_T size, SIZE_T count, _FILE * file)
    ADD ESP,0x10                        ; 00401c00
    PUSH EBX                            ; 00401c03
    CALL crt_stdio.c_fclose_FUN_00563380 ; 00401c04
        ;   XREF to: 00563380 (UNCONDITIONAL_CALL)  ; int crt_stdio.c_fclose_FUN_00563380(_FILE * file_handle)
    ADD ESP,0x4                         ; 00401c09
    POP EBX                             ; 00401c0c
    RET                                 ; 00401c0d
    PUSH ESI                            ; 00401c0e
        ;   Label: LAB_00401c0e
    MOV ECX,0x57715b                    ; 00401c0f | = "..\\engine\\2d.c"
    MOV ESI,0x2c1                       ; 00401c14
    PUSH 0x57716a                       ; 00401c19 | = "Unable to open VGA palette"
    MOV dword ptr [0x01cc4800],ECX      ; 00401c1e | g_CurrentFilename
    MOV dword ptr [0x01cc4804],ESI      ; 00401c24 | g_CurrentLineNumber
    CALL core_main.c_displayErrorAndQuit_FUN_004c8440 ; 00401c2a
        ;   XREF to: 004c8440 (UNCONDITIONAL_CALL)  ; void core_main.c_displayErrorAndQuit_FUN_004c8440(char * format)
    ADD ESP,0x4                         ; 00401c2f
    POP ESI                             ; 00401c32
    JMP 0x00401bee                      ; 00401c33
        ;   XREF to: 00401bee (UNCONDITIONAL_JUMP)  ; LAB_00401bee

