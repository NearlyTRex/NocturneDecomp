// Name: engine_3d.c_badMRGLStruct_FUN_004044a0
// Address: 004044a0
// Address Range: [[004044a0, 004044c7]]
// Convention: __cdecl
// Signature: SMRGLHeaderExtended * __cdecl engine_3d_c_badMRGLStruct_FUN_004044a0(SMRGLHeaderExtended *prim)

#include "nocturne.h"

SMRGLHeaderExtended * __cdecl engine_3d_c_badMRGLStruct_FUN_004044a0(SMRGLHeaderExtended *prim)

{
  g_CHAR_PTR_01cc4800 = "..\\engine\\3d.c";
  g_INT_01cc4804 = 0x119;
  core_main_c_FUN_004c8440("Bad MRGL struct");
  return prim;
}
