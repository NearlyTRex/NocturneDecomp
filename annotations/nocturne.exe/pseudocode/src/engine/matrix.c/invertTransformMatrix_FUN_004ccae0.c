// Name: engine_matrix.c_invertTransformMatrix_FUN_004ccae0
// Address: 004ccae0
// Address Range: [[004ccae0, 004ccdb9]]
// Convention: __cdecl
// Signature: void __cdecl engine_matrix_c_invertTransformMatrix_FUN_004ccae0(void)

#include "nocturne.h"

/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

void __cdecl engine_matrix_c_invertTransformMatrix_FUN_004ccae0(void)

{
  float fVar1;
  float fVar2;
  float fVar3;
  float fVar4;
  float fVar5;
  float fVar6;
  float fVar7;
  float fVar8;
  float10 fVar9;
  float10 fVar10;
  float10 fVar11;
  float10 fVar12;
  float10 fVar13;
  float10 fVar14;
  float10 fVar15;
  uint uStack_54;
  
  fVar2 = (float)_DAT_01c039e8;
  fVar1 = (float)_DAT_005881db;
  fVar6 = (float)_DAT_01c039ec;
  fVar5 = (float)_DAT_005881db;
  fVar8 = (float)_DAT_01c03a00;
  fVar7 = (float)_DAT_005881db;
  fVar4 = (float)_DAT_01c03a04;
  fVar3 = (float)_DAT_005881db;
  _DAT_01cc5140 = _DAT_01c039e8;
  _DAT_01cc5144 = _DAT_01c039ec;
  _DAT_01cc5148 = _DAT_01c039f0;
  _DAT_01cc514c = _DAT_01c039f4;
  _DAT_01cc5150 = _DAT_01c039f8;
  _DAT_01cc5154 = _DAT_01c039fc;
  _DAT_01cc5158 = _DAT_01c03a00;
  _DAT_01cc515c = _DAT_01c03a04;
  _DAT_01cc5160 = _DAT_01c03a08;
  fVar9 = (float10)round();
  _DAT_01cc5118 = (int)ROUND(fVar9);
  fVar9 = (float10)round();
  _DAT_01cc511c = (int)ROUND(fVar9);
  fVar9 = (float10)round();
  fVar10 = (float10)round();
  fVar11 = (float10)round();
  fVar12 = (float10)round();
  fVar13 = (float10)round();
  fVar14 = (float10)round((float10)uStack_54 * (float10)7.053105e-39);
  fVar15 = (float10)round
                              ((float10)-(fVar2 * fVar1 * fVar4 * fVar3 -
                                         fVar6 * fVar5 * fVar8 * fVar7) * (float10)7.053133e-39);
  _DAT_01cc5120 = (int)ROUND(fVar9);
  _DAT_01cc5124 = (int)ROUND(fVar10);
  _DAT_01cc5128 = (int)ROUND(fVar11);
  _DAT_01cc512c = (int)ROUND(fVar12);
  _DAT_01cc5130 = (int)ROUND(fVar13);
  _DAT_01cc5134 = (int)ROUND(fVar14);
  _DAT_01cc5138 = (int)ROUND(fVar15);
  return;
}
