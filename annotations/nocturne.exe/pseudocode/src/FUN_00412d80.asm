; *****************************************************************************
;                               FUNCTION
; *****************************************************************************
; void FUN_00412d80(int param_1)
;
;
; Referenced Globals:
;   undefined4 s_speed_00578a35+2
;   string s_modelName_00578a3d
;   string s_motion_state_00578a47
;
; Called Functions:
;   FUN_0040c880
;   FUN_0040ca80
;   FUN_0040cb00
;   FUN_004796c0
;
; *****************************************************************************

section .text

    PUSH EBX                            ; 00412d80
        ;   Label: FUN_00412d80
    PUSH ESI                            ; 00412d81
    MOV ESI,dword ptr [ESP + 0xc]       ; 00412d82
    PUSH ESI                            ; 00412d86
    CALL FUN_004796c0                   ; 00412d87
        ;   XREF to: 004796c0 (UNCONDITIONAL_CALL)  ; undefined FUN_004796c0()
    ADD ESP,0x4                         ; 00412d8c
    PUSH 0x578a37                       ; 00412d8f | s_speed_00578a35+2
    LEA EBX,[ESI + 0xbc8c]              ; 00412d94
    PUSH EBX                            ; 00412d9a
    CALL FUN_0040c880                   ; 00412d9b
        ;   XREF to: 0040c880 (UNCONDITIONAL_CALL)  ; undefined FUN_0040c880()
    ADD ESP,0x8                         ; 00412da0
    PUSH 0x578a3d                       ; 00412da3 | = "modelName"
    LEA EBX,[ESI + 0x150]               ; 00412da8
    PUSH EBX                            ; 00412dae
    CALL FUN_0040ca80                   ; 00412daf
        ;   XREF to: 0040ca80 (UNCONDITIONAL_CALL)  ; undefined FUN_0040ca80()
    ADD ESP,0x8                         ; 00412db4
    PUSH 0x578a47                       ; 00412db7 | = "motion state"
    PUSH EBX                            ; 00412dbc
    CALL FUN_0040cb00                   ; 00412dbd
        ;   XREF to: 0040cb00 (UNCONDITIONAL_CALL)  ; undefined FUN_0040cb00()
    ADD ESP,0x8                         ; 00412dc2
    POP ESI                             ; 00412dc5
    POP EBX                             ; 00412dc6
    RET                                 ; 00412dc7

