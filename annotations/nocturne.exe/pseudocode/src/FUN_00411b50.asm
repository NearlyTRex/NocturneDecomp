; *****************************************************************************
;                               FUNCTION
; *****************************************************************************
; void FUN_00411b50(int param_1)
;
;
; Referenced Globals:
;   string s_courseFilename_00578790
;   string s_param_0057879f
;   string s_speed_005787a5
;   string s_flapSpeedMult_005787ab
;   string s_courseSpeed_005787b9
;   string s_modelName_005787c5
;   string s_periodicSoundTimerMin_005787cf
;   string s_periodicSoundTimerMax_005787e5
;   undefined4 DAT_005787ff
;   undefined4 DAT_005acf94
;
; Called Functions:
;   FUN_0040c6d0
;   FUN_0040c880
;   FUN_0040ca00
;   FUN_0040d2d0
;
; *****************************************************************************

section .text

    PUSH EBX                            ; 00411b50
        ;   Label: FUN_00411b50
    MOV EBX,dword ptr [ESP + 0x8]       ; 00411b51
    PUSH EBX                            ; 00411b55
    CALL FUN_0040d2d0                   ; 00411b56
        ;   XREF to: 0040d2d0 (UNCONDITIONAL_CALL)  ; undefined FUN_0040d2d0()
    ADD ESP,0x4                         ; 00411b5b
    PUSH 0x578790                       ; 00411b5e | = "courseFilename"
    LEA EAX,[EBX + 0x150]               ; 00411b63
    PUSH EAX                            ; 00411b69
    CALL FUN_0040c6d0                   ; 00411b6a
        ;   XREF to: 0040c6d0 (UNCONDITIONAL_CALL)  ; undefined FUN_0040c6d0()
    ADD ESP,0x8                         ; 00411b6f
    PUSH 0x57879f                       ; 00411b72 | = "param"
    LEA EAX,[EBX + 0x2fc]               ; 00411b77
    PUSH EAX                            ; 00411b7d
    CALL FUN_0040c880                   ; 00411b7e
        ;   XREF to: 0040c880 (UNCONDITIONAL_CALL)  ; undefined FUN_0040c880()
    MOV EDX,dword ptr [0x005acf94]      ; 00411b83 | DAT_005acf94
    ADD ESP,0x8                         ; 00411b89
    LEA EAX,[EBX + 0x304]               ; 00411b8c
    CMP EDX,0x3                         ; 00411b92
    JL 0x00411be1                       ; 00411b95
        ;   XREF to: 00411be1 (CONDITIONAL_JUMP)  ; LAB_00411be1
    PUSH 0x5787ab                       ; 00411b97 | = "flapSpeedMult"
    PUSH EAX                            ; 00411b9c
    CALL FUN_0040c880                   ; 00411b9d
        ;   XREF to: 0040c880 (UNCONDITIONAL_CALL)  ; undefined FUN_0040c880()
    ADD ESP,0x8                         ; 00411ba2
    PUSH 0x5787b9                       ; 00411ba5 | = "courseSpeed"
    LEA EAX,[EBX + 0x300]               ; 00411baa
    PUSH EAX                            ; 00411bb0
    CALL FUN_0040c880                   ; 00411bb1
        ;   XREF to: 0040c880 (UNCONDITIONAL_CALL)  ; undefined FUN_0040c880()
    ADD ESP,0x8                         ; 00411bb6
    CMP dword ptr [0x005acf94],0x2      ; 00411bb9 | DAT_005acf94
        ;   Label: LAB_00411bb9
    JL 0x00411bd6                       ; 00411bc0
        ;   XREF to: 00411bd6 (CONDITIONAL_JUMP)  ; LAB_00411bd6
    PUSH 0x5787c5                       ; 00411bc2 | = "modelName"
    LEA EAX,[EBX + 0x180]               ; 00411bc7
    PUSH EAX                            ; 00411bcd
    CALL FUN_0040ca00                   ; 00411bce
        ;   XREF to: 0040ca00 (UNCONDITIONAL_CALL)  ; undefined FUN_0040ca00()
    ADD ESP,0x8                         ; 00411bd3
    CMP dword ptr [0x005acf94],0x4      ; 00411bd6 | DAT_005acf94
        ;   Label: LAB_00411bd6
    JGE 0x00411c03                      ; 00411bdd
        ;   XREF to: 00411c03 (CONDITIONAL_JUMP)  ; LAB_00411c03
    POP EBX                             ; 00411bdf
    RET                                 ; 00411be0
    PUSH 0x5787a5                       ; 00411be1 | = "speed"
        ;   Label: LAB_00411be1
    PUSH EAX                            ; 00411be6
    CALL FUN_0040c880                   ; 00411be7
        ;   XREF to: 0040c880 (UNCONDITIONAL_CALL)  ; undefined FUN_0040c880()
    FLD float ptr [EBX + 0x304]         ; 00411bec
    FMUL double ptr [0x005787ff]        ; 00411bf2 | DAT_005787ff
    ADD ESP,0x8                         ; 00411bf8
    FSTP float ptr [EBX + 0x300]        ; 00411bfb
    JMP 0x00411bb9                      ; 00411c01
        ;   XREF to: 00411bb9 (UNCONDITIONAL_JUMP)  ; LAB_00411bb9
    PUSH 0x0                            ; 00411c03
        ;   Label: LAB_00411c03
    LEA EAX,[EBX + 0x30c]               ; 00411c05
    PUSH EAX                            ; 00411c0b
    CALL FUN_0040c6d0                   ; 00411c0c
        ;   XREF to: 0040c6d0 (UNCONDITIONAL_CALL)  ; undefined FUN_0040c6d0()
    ADD ESP,0x8                         ; 00411c11
    PUSH 0x5787cf                       ; 00411c14 | = "periodicSoundTimerMin"
    LEA EAX,[EBX + 0x370]               ; 00411c19
    PUSH EAX                            ; 00411c1f
    CALL FUN_0040c880                   ; 00411c20
        ;   XREF to: 0040c880 (UNCONDITIONAL_CALL)  ; undefined FUN_0040c880()
    ADD ESP,0x8                         ; 00411c25
    PUSH 0x5787e5                       ; 00411c28 | = "periodicSoundTimerMax"
    ADD EBX,0x374                       ; 00411c2d
    PUSH EBX                            ; 00411c33
    CALL FUN_0040c880                   ; 00411c34
        ;   XREF to: 0040c880 (UNCONDITIONAL_CALL)  ; undefined FUN_0040c880()
    ADD ESP,0x8                         ; 00411c39
    POP EBX                             ; 00411c3c
    RET                                 ; 00411c3d

