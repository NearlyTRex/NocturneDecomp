; *****************************************************************************
;                               FUNCTION
; *****************************************************************************
; int FUN_005506c0(int param_1,byte param_2)
;
;
; Referenced Globals:
;   undefined4 DAT_005a3e60
;
; Called Functions:
;   FUN_00409ea0
;   FUN_0056445f
;   FUN_00564486
;   FUN_00564494
;
; *****************************************************************************

section .text

    PUSH EBX                            ; 005506c0
        ;   Label: FUN_005506c0
    MOV EBX,dword ptr [ESP + 0x8]       ; 005506c1
    TEST byte ptr [ESP + 0xc],0x4       ; 005506c5
    JNZ 0x005506fd                      ; 005506ca
        ;   XREF to: 005506fd (CONDITIONAL_JUMP)  ; LAB_005506fd
    PUSH 0x1                            ; 005506cc
    ADD EBX,0x3bc                       ; 005506ce
    PUSH EBX                            ; 005506d4
    CALL FUN_00409ea0                   ; 005506d5
        ;   XREF to: 00409ea0 (UNCONDITIONAL_CALL)  ; undefined FUN_00409ea0()
    ADD ESP,0x8                         ; 005506da
    PUSH 0x1                            ; 005506dd
    LEA EBX,[EAX + 0xfffffc44]          ; 005506df
    PUSH EBX                            ; 005506e5
    CALL FUN_00409ea0                   ; 005506e6
        ;   XREF to: 00409ea0 (UNCONDITIONAL_CALL)  ; undefined FUN_00409ea0()
    ADD ESP,0x8                         ; 005506eb
    MOV DL,byte ptr [ESP + 0xc]         ; 005506ee
    MOV EBX,EAX                         ; 005506f2
    TEST DL,0x2                         ; 005506f4
    JNZ 0x00550718                      ; 005506f7
        ;   XREF to: 00550718 (CONDITIONAL_JUMP)  ; LAB_00550718
    MOV EAX,EBX                         ; 005506f9
    POP EBX                             ; 005506fb
    RET                                 ; 005506fc
    PUSH 0x5a3e60                       ; 005506fd | DAT_005a3e60
        ;   Label: LAB_005506fd
    PUSH EBX                            ; 00550702
    CALL FUN_0056445f                   ; 00550703
        ;   XREF to: 0056445f (UNCONDITIONAL_CALL)  ; undefined FUN_0056445f()
    ADD ESP,0x8                         ; 00550708
    PUSH EAX                            ; 0055070b
    CALL FUN_00564486                   ; 0055070c
        ;   XREF to: 00564486 (UNCONDITIONAL_CALL)  ; undefined FUN_00564486()
    ADD ESP,0x4                         ; 00550711
    MOV EAX,EBX                         ; 00550714
    POP EBX                             ; 00550716
    RET                                 ; 00550717
    PUSH EAX                            ; 00550718
        ;   Label: LAB_00550718
    CALL FUN_00564494                   ; 00550719
        ;   XREF to: 00564494 (UNCONDITIONAL_CALL)  ; undefined FUN_00564494()
    ADD ESP,0x4                         ; 0055071e
    MOV EAX,EBX                         ; 00550721
    POP EBX                             ; 00550723
    RET                                 ; 00550724

