...

{
  ...
  if (p_Var2 == (_FILE *)0x0) {
    ...
    core_main_c_displayErrorAndQuit_FUN_004c8440("CDemonFilter::load - Bad filename : %s");
  }
  _fclose(p_Var2);
  ...
  if (this_ptr->size != this_ptr->count) {
    ...
    core_main_c_displayErrorAndQuit_FUN_004c8440("CDemonFilter::load - Non-square filter %dx%d");
  }
  if ((this_ptr->size < 0x40) || (0x100 < this_ptr->size)) {
    g_CurrentFilename = "..\\core\\dfilter.cpp";
    g_CurrentLineNumber = 185;
    core_main_c_displayErrorAndQuit_FUN_004c8440("CDemonFilter::load - Bad filter size %d");
  }
