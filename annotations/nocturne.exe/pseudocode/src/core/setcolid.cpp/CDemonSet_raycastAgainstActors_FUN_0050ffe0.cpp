...

{
  ...
  if (-1 < this_ptr->ignore_list_count) {
    ...
    if (0 < this_ptr->collidable_actor_count) {
      local_18 = (SActorRayHit *)&this_ptr->collision_normal;
      local_20 = this_ptr;
      do {
        pCVar1 = local_20->collidable_actors[0];
        pCVar3 = (CDeformableModelInstance *)
                 core_setcolid_cpp_CDemonSet_isActorIgnored_FUN_005103f0(this_ptr,pCVar1);
        if (pCVar3 == (CDeformableModelInstance *)0x0) {
          ...
          if (bbox_type != COLLISION_TYPE_NONE) {
            fStack_14 = core_actor_cpp_CDemonActor_rayIntersect_FUN_0040a740
                                  (pCVar1,ray_origin,&local_64,&SStack_a4,&local_cc,bbox_type,
                                   &local_88);
            if (((fStack_14 <= 1.0) && (fVar2 = fStack_14 * local_24, fVar2 < max_t)) &&
              ...
              this_ptr->collision_part_index = SStack_a4.part_index;
              this_ptr->collision_bone_index = SStack_a4.bone_index;
