...

{
  ...
  for (iVar3 = 0; iVar3 < *(int *)(0x01E57284 + 0x14cd6c); iVar3 = iVar3 + 1) {
    ...
    if (this_ptr_00 != (CLever *)0x0) {
      ...
      if ((((ABS(local_3c.y) <= (float)5) && (0.0 <= (double)local_3c.z)) &&
        ...
        iVar1 = core_vecdir_cpp_convertDirectionVectorToEulerAngles_FUN_0054e4a0(local_30,&local_3c)
        ;
        if (ABS(*(float *)(iVar1 + 4)) <= (float)1.04719755116667) {
          iVar1 = core_lever_cpp_CLever_isAccessibleFrom_FUN_004c68f0(this_ptr_00,local_1c);
          if (iVar1 != 0) {
  ...
  if (this_ptr->lever_to_pull != (CLever *)0x0) {
    iVar2 = core_event_cpp_CRuleList_evaluateAndRun_FUN_00481890(&this_ptr->lever_to_pull->rule);
    if (iVar2 == 0) {
