// Name: core_shotgun.cpp_CShotgun_process_FUN_00515db0
// Address: 00515db0
// Address Range: [[00515db0, 00515e99]]
// Convention: __cdecl
// Signature: void __cdecl core_shotgun_cpp_CShotgun_process_FUN_00515db0(CShotgun *this_ptr,float delta_time)

#include "nocturne.h"

void __cdecl core_shotgun_cpp_CShotgun_process_FUN_00515db0(CShotgun *this_ptr,float delta_time)

{
  CVector3f *input_local_point;
  float in_stack_0000000c;
  CVector3f CStack_1c;
  
  if (this_ptr->muzzle_flash_active != 0) {
    g_CDemonLight_01c74640.light_enabled_flag = 1;
    input_local_point =
         (CVector3f *)(*(((this_ptr->base).base.vtable._uc)->_uc).canWalk)((CCharacter *)this_ptr);
    core_actor_cpp_CDemonActor_localToWorldPoint_FUN_0040a240
              ((CDemonActor *)this_ptr,&CStack_1c,input_local_point);
    g_CDemonLight_01c74640.volumetric_enabled = 0;
    if ((float *)&stack0x00000000 != &g_CDemonLight_01c74640.base.rotation_matrix.m[1].y) {
      g_CDemonLight_01c74640.base.position.x = CStack_1c.x;
      g_CDemonLight_01c74640.base.position.y = CStack_1c.y;
      g_CDemonLight_01c74640.base.position.z = CStack_1c.z;
    }
    core_dirmat_cpp_CMatrix3x3f_buildRotationMatrix_FUN_0044d7a0
              (&g_CDemonLight_01c74640.base.rotation_matrix,&(this_ptr->base).base.orient.vec);
    g_CDemonLight_01c74640.base.fixed_point_scale = this_ptr->muzzle_flash_distance;
    g_CDemonLight_01c74640.base.focal_length = 112.0;
    core_dlight_cpp_CDemonLight_setVolumetricIntensity_FUN_00452080(&g_CDemonLight_01c74640,1.0);
    g_CDemonLight_01c74640.antialiasing_enabled = 0;
    core_set_cpp_CDemonSet_addDynamicLight_FUN_0050a970
              (g_CDemonSet_PTR_005be368,&g_CDemonLight_01c74640);
  }
  this_ptr->muzzle_flash_active = 0;
  core_weapon_cpp_CWeapon_process_FUN_00554030(&this_ptr->base,in_stack_0000000c);
  return;
}
