// Name: core_boxactor.cpp_CLightActor_process_FUN_0041f3d0
// Address: 0041f3d0
// Address Range: [[0041f3d0, 0041f5f3]]
// Convention: __cdecl
// Signature: void __cdecl core_boxactor_cpp_CLightActor_process_FUN_0041f3d0(CLightActor *this_ptr,float delta_time)

#include "nocturne.h"

void __cdecl core_boxactor_cpp_CLightActor_process_FUN_0041f3d0(CLightActor *this_ptr,float delta_time)

{
  CVector3f *pCVar1;
  float fVar2;
  CDemonSet *this_ptr_00;
  uint uVar3;
  int iVar4;
  CDemonGlobe *this_ptr_01;
  CMatrix3x4f *pCVar5;
  CMatrix3x4f *pCVar6;
  byte bVar7;
  int filter_index;
  int filter_pos_x;
  int filter_pos_y;
  CMatrix3x4f local_e4;
  CMatrix3x4f local_b4;
  CMatrix3x4f local_84;
  CMatrix3x4f local_54;
  CVector3f local_24;
  CVector3f local_18;
  float local_c;
  
  bVar7 = 0;
  core_boxactor_cpp_CBoxActor_process_FUN_0041e5e0(&this_ptr->base,delta_time);
  if ((this_ptr->light_actor_type == LIGHT_TYPE_LANTERN) &&
     (fVar2 = this_ptr->flicker_timer - delta_time, this_ptr->flicker_timer = fVar2, fVar2 <= 0.0))
  {
    filter_pos_y = 0;
    filter_pos_x = 0;
    filter_index = 0;
    iVar4 = core_actor_cpp_getRandomInt_FUN_0040de00(0,7);
    core_dlight_cpp_CDemonLight_applyFilter_FUN_004501c0
              (&this_ptr->light,g_CDemonFilter_ARRAY_0076483c + iVar4,filter_index,filter_pos_x,
               filter_pos_y);
    local_c = core_actor_cpp_getRandomFloatFromRange_FUN_0040dda0(0.0,0.15);
    this_ptr->flicker_timer = local_c;
  }
  if (this_ptr->light_actor_type != LIGHT_TYPE_GLOBE) {
    if ((this_ptr->light).light_enabled_flag == 0) {
      return;
    }
    core_actor_cpp_CDemonActor_localToWorldPoint_FUN_0040a240
              ((CDemonActor *)this_ptr,&local_18,&this_ptr->blight_pos);
    pCVar1 = &(this_ptr->light).base.position;
    if (pCVar1 != &local_18) {
      pCVar1->x = local_18.x;
      (this_ptr->light).base.position.y = local_18.y;
      (this_ptr->light).base.position.z = local_18.z;
    }
    core_xform_cpp_buildMatrixFromEulerAndPositionDirect_FUN_0055afb0
              (&local_54,(CVector3f *)&DAT_02dd1184,&(this_ptr->light_orient).vec);
    core_xform_cpp_buildMatrixFromEulerAndPositionDirect_FUN_0055afb0
              (&local_b4,(CVector3f *)&DAT_02dd1184,&(this_ptr->base).base.orient.vec);
    core_xform_cpp_multiplyMatrix3x4_FUN_0055aa00(&local_54,&local_b4,&local_e4);
    pCVar5 = &local_e4;
    pCVar6 = &local_84;
    for (iVar4 = 0xc; iVar4 != 0; iVar4 = iVar4 + -1) {
      pCVar6->m[0].w = *(float *)pCVar5;
      pCVar5 = (CMatrix3x4f *)((int)pCVar5 + ((uint)bVar7 * -2 + 1) * 4);
      pCVar6 = (CMatrix3x4f *)((int)pCVar6 + ((uint)bVar7 * -2 + 1) * 4);
    }
    core_xform_cpp_matrixToEulerAngles_FUN_0055b180(&local_84,&local_24);
    core_dirmat_cpp_CMatrix3x3f_buildRotationMatrix_FUN_0044d7a0
              (&(this_ptr->light).base.rotation_matrix,&local_24);
    core_set_cpp_CDemonSet_addDynamicLight_FUN_0050a970(g_CDemonSet_PTR_005be368,&this_ptr->light);
    return;
  }
  this_ptr_01 = &this_ptr->globe;
  core_dglobe_cpp_CDemonGlobe_setPosition_FUN_0044cd90
            (this_ptr_01,&(this_ptr->base).base.location.position);
  uVar3 = rand();
  (this_ptr->globe).intensity_multiplier = uVar3 & 0x7fff;
  (this_ptr->globe).intensity.bytes[0] = (uchar)((int)(uVar3 & 0x7fff) >> 10);
  core_dglobe_cpp_CDemonGlobe_precomputeAttenuation_FUN_0044cde0(this_ptr_01,2.0);
  this_ptr_00 = g_CDemonSet_PTR_005be368;
  (this_ptr->globe).corona_mode = 0;
  core_set_cpp_CDemonSet_addCoronaGlobe_FUN_0050a9f0(this_ptr_00,this_ptr_01);
  return;
}
