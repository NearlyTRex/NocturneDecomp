// Name: core_fire.cpp_CGunFlame_arrdtor_FUN_0048cd40
// Address: 0048cd40
// Address Range: [[0048cd40, 0048cd57]]
// Convention: __cdecl
// Signature: CGunFlame * __cdecl core_fire_cpp_CGunFlame_arrdtor_FUN_0048cd40(CGunFlame *this_ptr,uint flags)

#include "nocturne.h"

CGunFlame * __cdecl core_fire_cpp_CGunFlame_arrdtor_FUN_0048cd40(CGunFlame *this_ptr,uint flags)

{
  CGunFlame *pCVar1;
  
  pCVar1 = (CGunFlame *)__arrfini(this_ptr,500,&g_CGunFlameTypeInfo_0059d4a0);
  return pCVar1;
}
