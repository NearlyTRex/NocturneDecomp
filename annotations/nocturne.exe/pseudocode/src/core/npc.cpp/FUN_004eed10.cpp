// Name: core_npc.cpp_FUN_004eed10
// Address: 004eed10
// Address Range: [[004eed10, 004eedb4]]
// Convention: unknown
// Signature: void core_npc_cpp_FUN_004eed10(CCharacter *param_1)

#include "nocturne.h"

void core_npc_cpp_FUN_004eed10(CCharacter *param_1)

{
  if (INT_005be074 < 2) {
    core_actor_cpp_CDemonActor_archive_FUN_0040d2d0(&param_1->base);
    core_actor_cpp_archiveFloat_FUN_0040c880(&param_1->hit_points,"hitPoints");
    return;
  }
  core_charactr_cpp_CCharacter_archive_FUN_004244b0(param_1);
  if (2 < INT_005be074) {
    core_actor_cpp_archiveDeformableModelInstance_FUN_0040ca80(&param_1->model,"modelName")
    ;
    core_actor_cpp_archiveMotionState_FUN_0040cb00
              (&(param_1->model).motion_controller,"motion state");
  }
  if (3 < INT_005be074) {
    core_actor_cpp_archiveInteger_FUN_0040c900
              ((int *)(param_1[2].flames[0x1b].off_event + 4),"shootMe");
  }
  if (INT_005be074 < 5) {
    return;
  }
  core_actor_cpp_archiveInteger_FUN_0040c900
            ((int *)(param_1[2].flames[0x1b].off_event + 8),"poolMe");
  return;
}
