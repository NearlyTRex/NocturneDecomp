// Name: core_dmodel.cpp_CKeyFramedModel_ctor_FUN_00452590
// Address: 00452590
// Address Range: [[00452590, 0045262d]]
// Convention: __cdecl
// Signature: void __cdecl core_dmodel_cpp_CKeyFramedModel_ctor_FUN_00452590(undefined1 *param_1)

#include "nocturne.h"

void __cdecl core_dmodel_cpp_CKeyFramedModel_ctor_FUN_00452590(byte *param_1)

{
  *(uint *)(param_1 + 0x104) = 0;
  *(uint *)(param_1 + 0x10c) = 0;
  *(uint *)(param_1 + 0x108) = 0;
  *(uint *)(param_1 + 0x110) = 0;
  *(uint *)(param_1 + 0x114) = 0;
  *(uint *)(param_1 + 0x118) = 0;
  *(uint *)(param_1 + 0x11c) = 0;
  *(uint *)(param_1 + 0x120) = 0;
  *(uint *)(param_1 + 0x244) = 0;
  *(uint *)(param_1 + 0x350) = 0;
  *(uint *)(param_1 + 0x100) = 0;
  *(uint *)(param_1 + 0x354) = 0;
  *(uint *)(param_1 + 0x358) = 0;
  *(uint *)(param_1 + 0x35c) = 0;
  *(uint *)(param_1 + 0x360) = 0;
  *param_1 = 0;
  return;
}
