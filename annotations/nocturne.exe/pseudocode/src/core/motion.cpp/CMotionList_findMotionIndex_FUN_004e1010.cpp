// Name: core_motion.cpp_CMotionList_findMotionIndex_FUN_004e1010
// Address: 004e1010
// Address Range: [[004e1010, 004e1092]]
// Convention: __cdecl
// Signature: int __cdecl core_motion_cpp_CMotionList_findMotionIndex_FUN_004e1010(CMotionList *this_ptr,char *motion_name,int error_on_not_found)

#include "nocturne.h"

int __cdecl core_motion_cpp_CMotionList_findMotionIndex_FUN_004e1010(CMotionList *this_ptr,char *motion_name,int error_on_not_found)

{
  int iVar1;
  int iVar2;
  SMotion *str1;
  
  iVar2 = 0;
  if (0 < this_ptr->motion_count) {
    str1 = this_ptr->motions;
    do {
      iVar1 = _stricmp(str1->motion_name,motion_name);
      if (iVar1 == 0) {
        return iVar2;
      }
      iVar2 = iVar2 + 1;
      str1 = str1 + 1;
    } while (iVar2 < this_ptr->motion_count);
  }
  if (error_on_not_found == 0) {
    return -1;
  }
  g_CurrentFilename = "..\\core\\motion.cpp";
  g_CurrentLineNumber = 256;
  core_main_c_displayErrorAndQuit_FUN_004c8440("Can't find motion \"%s\" in motion list",motion_name);
  return -1;
}
