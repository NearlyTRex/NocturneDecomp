#pragma once

// Function prototypes for engine/console.cpp
// Generated from Ghidra function signatures

// Original: engine_console.cpp_CConsole_ctor_FUN_0043abe0
// Address: 0043abe0
undefined4 * CConsole::ctor(undefined4 *param_1,undefined4 param_2,undefined4 param_3,undefined4 param_4,undefined4 param_5);

// Original: engine_console.cpp_FUN_0043ac50
// Address: 0043ac50
undefined4 FUN_0043ac50(undefined4 param_1);

// Original: engine_console.cpp_CConsole_printf_FUN_0043ac60
// Address: 0043ac60
void CConsole::printf(int *param_1,undefined4 param_2);

// Original: engine_console.cpp_CConsole_writeChar_FUN_0043ad30
// Address: 0043ad30
void CConsole::writeChar(int param_1,char param_2);

// Original: engine_console.cpp_CConsole_reset_FUN_0043ae00
// Address: 0043ae00
void __cdecl CConsole::reset(int param_1);

// Original: engine_console.cpp_CConsole_scrollUp_FUN_0043ae40
// Address: 0043ae40
void __cdecl CConsole::scrollUp(int param_1);

// Original: engine_console.cpp_CConsole_render_FUN_0043aec0
// Address: 0043aec0
void __cdecl CConsole::render(int param_1);
