#pragma once

// Function prototypes for core/batcreat.cpp
// Generated from Ghidra function signatures

// Original: core_batcreat.cpp_CBatCreature_setup_FUN_00412160
// Address: 00412160
void CBatCreature::setup(int param_1);

// Original: core_batcreat.cpp_CBatCreature_process_FUN_00412480
// Address: 00412480
void CBatCreature::process(int param_1,float param_2);

// Original: core_batcreat.cpp_CBatCreature_processDismemberment_FUN_00412de0
// Address: 00412de0
void __cdecl CBatCreature::processDismemberment(int param_1,int *param_2);

// Original: core_batcreat.cpp_CBatCreature_getTargetPoints_FUN_00413250
// Address: 00413250
undefined4 CBatCreature::getTargetPoints(int param_1,undefined4 *param_2);
