#pragma once

// Function prototypes for core/cow.cpp
// Generated from Ghidra function signatures

// Original: core_cow.cpp_staticInit_FUN_0043bb80
// Address: 0043bb80
void __cdecl staticInit(void);

// Original: core_cow.cpp_FUN_0043bbb0
// Address: 0043bbb0
void FUN_0043bbb0(void);

// Original: core_cow.cpp_FUN_0043bbd0
// Address: 0043bbd0
CDemonActorType * FUN_0043bbd0(void);

// Original: core_cow.cpp_FUN_0043bbe0
// Address: 0043bbe0
int FUN_0043bbe0(undefined4 param_1);

// Original: core_cow.cpp_CZombieCow_setup_FUN_0043bc50
// Address: 0043bc50
void CZombieCow::setup(int param_1);

// Original: core_cow.cpp_CZombieCow_process_FUN_0043bdb0
// Address: 0043bdb0
void CZombieCow::process(int param_1,float param_2);

// Original: core_cow.cpp_FUN_0043c2e0
// Address: 0043c2e0
void FUN_0043c2e0(int param_1);

// Original: core_cow.cpp_CZombieCow_processDismemberment_FUN_0043c360
// Address: 0043c360
void __cdecl CZombieCow::processDismemberment(int param_1,int *param_2);

// Original: core_cow.cpp_CZombieCow_processDamage_FUN_0043c5e0
// Address: 0043c5e0
void CZombieCow::processDamage(int param_1,int param_2);

// Original: core_cow.cpp_FUN_0043c6a0
// Address: 0043c6a0
undefined4 FUN_0043c6a0(int param_1,undefined4 *param_2);

// Original: core_cow.cpp_FUN_0043c6f0
// Address: 0043c6f0
int FUN_0043c6f0(int param_1,byte param_2);
