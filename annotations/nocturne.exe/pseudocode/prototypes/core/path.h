#pragma once

// Function prototypes for core/path.cpp
// Generated from Ghidra function signatures

// Original: core_path.cpp_CPathMap_ctor_FUN_004efd50
// Address: 004efd50
int __cdecl CPathMap::ctor(int param_1);

// Original: core_path.cpp_CPathMap_dtor_FUN_004efdd0
// Address: 004efdd0
int __cdecl CPathMap::dtor(int param_1);

// Original: core_path.cpp_CPathMap_getDirection_FUN_004efeb0
// Address: 004efeb0
float CPathMap::getDirection(float *param_1,int param_2,int param_3,int param_4,float *param_5);

// Original: core_path.cpp_CPathMap_getCachedVoxelHeight_FUN_004f04a0
// Address: 004f04a0
undefined4 __cdecl CPathMap::getCachedVoxelHeight(int param_1,int param_2,int param_3,int param_4);

// Original: core_path.cpp_CPathMap_isLineWalkable_FUN_004f0540
// Address: 004f0540
undefined4 __cdecl CPathMap::isLineWalkable(undefined4 param_1,int param_2,int param_3,int param_4,int param_5,int param_6,int param_7);

// Original: core_path.cpp_CPathMap_checkAxisAlignedPath_FUN_004f0790
// Address: 004f0790
undefined4 __cdecl CPathMap::checkAxisAlignedPath(int param_1,int param_2,int param_3,int param_4,int param_5);

// Original: core_path.cpp_CPathMap_findPathToDestination_FUN_004f0c20
// Address: 004f0c20
undefined4 __cdecl CPathMap::findPathToDestination(float *param_1,float *param_2,undefined4 *param_3,int param_4);

// Original: core_path.cpp_CPathMap_previewPathfinding_FUN_004f1500
// Address: 004f1500
undefined4 CPathMap::previewPathfinding(void);

// Original: core_path.cpp_CPathMap_findPathWithRetry_FUN_004f1600
// Address: 004f1600
undefined4 __cdecl CPathMap::findPathWithRetry(int param_1,float *param_2,undefined4 *param_3,undefined4 param_4);

// Original: core_path.cpp_getOrCreatePathMap_FUN_004f1c90
// Address: 004f1c90
int __cdecl getOrCreatePathMap(undefined4 *param_1);

// Original: core_path.cpp_getPathMap_FUN_004f1e00
// Address: 004f1e00
void __cdecl getPathMap(undefined4 param_1);

// Original: core_path.cpp_CPathMap_reset_FUN_004f1e10
// Address: 004f1e10
void CPathMap::reset(int param_1);

// Original: core_path.cpp_resetAllPathMaps_FUN_004f1e90
// Address: 004f1e90
void __cdecl resetAllPathMaps(void);

// Original: core_path.cpp_CPathMap_searchGrid_FUN_004f20a0
// Address: 004f20a0
uint __cdecl CPathMap::searchGrid(undefined4 param_1,undefined4 param_2);
